...
T2PReferenceArea::T2PReferenceArea(PangoRectangle rectangle_in,
                                   cairo_t *cairo_in, bool right_to_left_in)
...
// Destructor.
{
  ...
  for (unsigned int i = 0; i < table_of_contents_layout_containers.size();
       i++) {
...
// Print the data.
{
  ...
  for (unsigned int blk = 0; blk < body_blocks.size(); blk++) {
    ...
    // While laying out text, the block's x and y values are relative to the
    // parent's values.
  ...
  // Print the notes. While doing that reposition them to the bottom of the
  // page.
  ...
  for (unsigned int i = 0; i < note_layout_containers.size(); i++) {
    ...
    layout_container->rectangle.y +=
        rectangle.y + rectangle.height - notes_height;
...
void T2PReferenceArea::print(unsigned int page_number, bool print_date,
                             const ustring &left_running_header,
                             const ustring &right_running_header,
                             bool suppress_header,
                             unsigned int left_first_chapter,
                             unsigned int left_last_chapter,
                             unsigned int right_first_chapter,
...
// Print the page number, the date, the running header.
{
  ...
  header_paragraph.font_size_points =
  ...
  if (!(page_number % 2)) {
    // Even.
    page_number_layout_container.rectangle.x +=
        rectangle.width - page_number_layout_container.rectangle.width;
  }
  ...
  if (print_date) {
    string s(date_time_julian_human_readable(date_time_julian_day_get_current(),
                                             false));
  ...
  if ((page_number % 2)) {
    ...
    // Even.
    date_layout_container.rectangle.x +=
        rectangle.width - date_layout_container.rectangle.width -
        millimeters_to_pango_units(5) -
        page_number_layout_container.rectangle.width;
  }
  ...
  if (!header.empty()) {
    ...
    if ((page_number % 2)) {
      // Odd page.
      running_header_layout_container.rectangle.x +=
          rectangle.width - running_header_layout_container.rectangle.width;
    } else {
      // Even page.
    }
...
// Fits the blocks into the reference area.
{
  ...
  while (!input_blocks.empty()) {
    ...
    if (input_blocks[0]->column_count != n_columns ||
      ...
      // If there are still blocks left, that means that these didn't fit on the
      // page.
  ...
  // Look for any last blocks on the page that have their "keep_with_next"
  // property set.
  // If these are there, that means these should be removed from here, and be
  // put back into
  // the input stream, so that they can be kept with paragraphs on the next
  // page.
  // But this only should be done if there's indeed goint to be a next page.
  // If there's no next paragraph to be kept with, then the last block is just
  // left where it is.
  while (!body_blocks.empty() &&
         body_blocks[body_blocks.size() - 1]->keep_with_next &&
         !input_blocks.empty()) {
...
/*
 ...
 If there are any input blocks available for fitting in, then instead of
 considering the
 ...
 Then it keeps shifting blocks from the first to the second column, and
 vice-versa,
 ...
 Whether white space before the paragraph disappears at the top of the reference
 area.
 ...
 */
{
  ...
  while (!input_blocks.empty()) {
    ...
    // Get the next lot of blocks that stay together, fit it, and remove it from
    // the input stream.
    T2PBigBlock big_block =
        get_next_big_block_to_be_kept_together(input_blocks, column_count);
    ...
    for (unsigned int i = 0; i < big_block.blocks.size(); i++) {
      ...
      for (unsigned int i2 = 0; i2 < block->layoutcontainers.size(); i2++) {
        ...
        for (unsigned int i3 = 0; i3 < layout_container->note_paragraphs.size();
             i3++) {
    ...
    // See if the column(s) didn't get too high after adding this last big
    // block.
    ...
    bool columns_too_high =
        (start_stacking_y + MAX(first_column_height, last_column_height)) >
        (rectangle.height - get_note_height());
    if (columns_too_high) {

      // If there's only one block on the page, special handling is needed to
      ...
      // If the last big block caused any notes to be added, normally put the
      // text of these notes back into their
      ...
      while (!added_notes.empty()) {
        ...
        // If there's only one block on the page, we have special treatment so
        // that not all note data is removed,
        // but only excess data. This causes that bits of notes are not printed.
        if (one_block_on_page) {
          columns_too_high = (start_stacking_y +
                              MAX(first_column_height, last_column_height)) >
                             (rectangle.height - get_note_height());
          if (!columns_too_high)
            break;
        }
  ...
  // When text runs from right to left, the columns get swapped, the intrusions
  // flush to the right,
  // The last lines of the paragraphs are flushed to the right.
  // Copy the blocks from the columns into the object.
  int second_x =
      rectangle.width - ((rectangle.width - column_spacing_pango_units) / 2);
  ...
  // Store maximum height of any column as the starting point for the next
  // column.
...
// The big blocks returned could contain one block in the normal case, and more
// if these are kept
// together.
...
// Gets the next free rectangle where a note could be laid out.
{
  ...
  // If there's not enough space for the note on the x-axis, move it to the next
  // line.
  double remaining_millimeters =
      pango_units_to_millimeters(rectangle.width - next_x);
...
// Two-way balancing means that content can be moved from the first column to
// the last one,
...
// Move block from the first column to the last till the last column is higher
...
int T2PReferenceArea::balance_first_column_higher_than_or_equal_to_last_column(
    deque<T2PBigBlock> &first_column, deque<T2PBigBlock> &last_column)
// Move blocks from the last column to the first till the first column is higher
// than or equal to the last one.
// Returns the maximum height of both columns.
{
  int first_column_height = get_column_height(first_column, start_stacking_y);
  int last_column_height = get_column_height(last_column, start_stacking_y);
...
// Returns true if there's any flag in this page that the headers should be
// suppressed.
{
  bool suppress = false;
  for (unsigned int blk = 0; blk < body_blocks.size(); blk++) {
  ...
  for (unsigned int i = 0; i < body_blocks.size(); i++) {
    ...
    if (!block->reference.empty()) {
      ...
      if (!referent.empty()) {
        ...
        int available_width =
            block->maximum_width_pango_units - block->rectangle.x -
            block->rectangle.width - dot_distance_pango_units -
            dot_distance_pango_units - page_number_layout->rectangle.width;
        ...
        while (available_width > 0) {
          ...
          start_x +=
              (dotted_line_layout->rectangle.width + dot_distance_pango_units);
          available_width -=
              (dotted_line_layout->rectangle.width + dot_distance_pango_units);
        }
      }

...
// box: the parent box where it goes in.
{
  ...
  heavy_weight_tag =
      gtk_text_buffer_create_tag(textbuffer, NULL, "weight", PANGO_WEIGHT_HEAVY,
                                 "background", "hot pink", NULL);
  strike_through_tag = gtk_text_buffer_create_tag(
      textbuffer, NULL, "strikethrough", TRUE, "background", "hot pink", NULL);
}

DisplayChangesGui::~DisplayChangesGui() { gtk_widget_destroy(vbox); }
...
void DisplayChangesGui::display(const vector<ustring> &differences) {
  ...
  // Temporally removing the view from the buffer speeds loading text up a huge
  // lot.
  ...
  for (unsigned int i = 0; i < differences.size(); i++) {
    ...
    while (!line.empty()) {
      ...
      gtk_text_buffer_insert_with_tags(textbuffer, &iter, character.c_str(), -1,
                                       tag, NULL);
      line.erase(0, 1);
    }
    // End of line.
  ...
  // The window will then show the same block of text as before loading. This
  // makes editing the text easier.
...
void DisplayChangesGui::clear() { gtk_text_buffer_set_text(textbuffer, "", 0); }

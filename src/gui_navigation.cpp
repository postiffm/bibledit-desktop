...
GuiNavigation::GuiNavigation(int dummy) : reference(0), track(0) {
...
void GuiNavigation::build(GtkWidget *toolbar) {
  ...
  gtk_toolbar_insert(GTK_TOOLBAR(toolbar), GTK_TOOL_ITEM(toolitem_list_back),
                     -1);
  ...
  gtk_toolbar_insert(GTK_TOOLBAR(toolbar), GTK_TOOL_ITEM(toolitem_list_forward),
                     -1);
  ...
  spinbutton_book =
      gtk_spin_button_new(GTK_ADJUSTMENT(spinbutton_book_adj), 1, 0);
  ...
  spinbutton_chapter =
      gtk_spin_button_new(GTK_ADJUSTMENT(spinbutton_chapter_adj), 1, 0);
  ...
  spinbutton_verse =
      gtk_spin_button_new(GTK_ADJUSTMENT(spinbutton_verse_adj), 1, 0);
  ...
  // Jump to verse (basically same as Go to reference dialog, built right into
  // the main window navigation toolbar
  ...
  g_signal_connect((gpointer)entry_free, "activate",
                   G_CALLBACK(on_entry_free_activate), gpointer(this));

  g_signal_connect((gpointer)button_list_back, "clicked",
                   G_CALLBACK(on_button_list_back_clicked), gpointer(this));
  g_signal_connect((gpointer)button_back, "clicked",
                   G_CALLBACK(on_button_back_clicked), gpointer(this));
  g_signal_connect((gpointer)button_forward, "clicked",
                   G_CALLBACK(on_button_forward_clicked), gpointer(this));
  g_signal_connect((gpointer)button_list_forward, "clicked",
                   G_CALLBACK(on_button_list_forward_clicked), gpointer(this));
  g_signal_connect((gpointer)combo_book, "changed",
                   G_CALLBACK(on_combo_book_changed), gpointer(this));
  g_signal_connect((gpointer)spinbutton_book, "value_changed",
                   G_CALLBACK(on_spinbutton_book_value_changed),
                   gpointer(this));
  g_signal_connect((gpointer)combo_chapter, "changed",
                   G_CALLBACK(on_combo_chapter_changed), gpointer(this));
  g_signal_connect((gpointer)spinbutton_chapter, "value_changed",
                   G_CALLBACK(on_spinbutton_chapter_value_changed),
                   gpointer(this));
  g_signal_connect((gpointer)combo_verse, "changed",
                   G_CALLBACK(on_combo_verse_changed), gpointer(this));
  g_signal_connect((gpointer)spinbutton_verse, "value_changed",
                   G_CALLBACK(on_spinbutton_verse_value_changed),
                   gpointer(this));
}
...
// the project.
{
  ...
  if (!project_book_exists(project, reference.book_get())) {
    ...
    if (books.empty()) {
      ...
      reference.book_set(
          clamp(reference.book_get(), books[0], books[books.size() - 1]));
    }
  }
  // Clamp the chapter.
  vector<unsigned int> chapters =
      project_get_chapters(project, reference.book_get());
  ...
  // Clamp the verse.
  vector<ustring> verses = project_get_verses(project, reference.book_get(),
                                              reference.chapter_get());
...
Reference GuiNavigation::get_current_ref(void) {
  unsigned int currentbook =
      books_name_to_id(language, combobox_get_active_string(combo_book));
  unsigned int currentchapter =
      convert_to_int(combobox_get_active_string(combo_chapter));
...
void GuiNavigation::on_button_list_back_clicked(GtkButton *button,
                                                gpointer user_data) {
  ((GuiNavigation *)user_data)->on_list_back();
}

void GuiNavigation::on_button_back_clicked(GtkButton *button,
                                           gpointer user_data) {
  ((GuiNavigation *)user_data)->on_back();
}

void GuiNavigation::on_button_forward_clicked(GtkButton *button,
                                              gpointer user_data) {
  ((GuiNavigation *)user_data)->on_forward();
}

void GuiNavigation::on_button_list_forward_clicked(GtkButton *button,
                                                   gpointer user_data) {
  ((GuiNavigation *)user_data)->on_list_forward();
}

void GuiNavigation::on_combo_book_changed(GtkComboBox *combobox,
                                          gpointer user_data) {
  ((GuiNavigation *)user_data)->on_combo_book();
}

void GuiNavigation::on_combo_chapter_changed(GtkComboBox *combobox,
                                             gpointer user_data) {
  ((GuiNavigation *)user_data)->on_combo_chapter();
}

void GuiNavigation::on_combo_verse_changed(GtkComboBox *combobox,
                                           gpointer user_data) {
  ((GuiNavigation *)user_data)->on_combo_verse();
}

void GuiNavigation::on_spinbutton_book_value_changed(GtkSpinButton *spinbutton,
                                                     gpointer user_data) {
  ((GuiNavigation *)user_data)->on_spinbutton_book();
}

void GuiNavigation::on_spinbutton_chapter_value_changed(
    GtkSpinButton *spinbutton, gpointer user_data) {
  ((GuiNavigation *)user_data)->on_spinbutton_chapter();
}

void GuiNavigation::on_spinbutton_verse_value_changed(GtkSpinButton *spinbutton,
                                                      gpointer user_data) {
...
void GuiNavigation::on_spinbutton_book() {
  if (settingcombos)
    return;
  int value =
      int(gtk_adjustment_get_value(GTK_ADJUSTMENT(spinbutton_book_adj)));
...
void GuiNavigation::on_spinbutton_chapter() {
  if (settingcombos)
    return;
  int value =
      int(gtk_adjustment_get_value(GTK_ADJUSTMENT(spinbutton_chapter_adj)));
...
void GuiNavigation::on_spinbutton_verse() {
  if (settingcombos)
    return;
  int value =
      int(gtk_adjustment_get_value(GTK_ADJUSTMENT(spinbutton_verse_adj)));
...
void GuiNavigation::signal(bool track) {
  ...
  delayer_event_id =
      g_timeout_add_full(G_PRIORITY_DEFAULT, 100, GSourceFunc(signal_delayer),
                         gpointer(this), NULL);
  ...
  if (track) {
    track_event_id =
        g_timeout_add_full(G_PRIORITY_DEFAULT, 2000, GSourceFunc(signal_track),
                           gpointer(this), NULL);
  }
...
void GuiNavigation::signal_tracking() { track.store(reference); }
...
// the boundaries of a book or a chapter.
{
  ...
  for (unsigned int i = previousbookindex; i <= nextbookindex; i++) {
    ...
    vector<unsigned int> bookchapters =
        project_get_chapters(project, allbooks[i]);
    for (unsigned int i2 = 0; i2 < bookchapters.size(); i2++) {
      vector<ustring> chapterverses =
          project_get_verses(project, allbooks[i], bookchapters[i2]);
...
void GuiNavigation::crossboundarieschapter(bool forward) {
  ...
  for (unsigned int i = previousbookindex; i <= nextbookindex; i++) {
    ...
    for (unsigned int i2 = 0; i2 < bookchapters.size(); i2++) {
      ...
      vector<ustring> chapterverses =
          project_get_verses(project, allbooks[i], bookchapters[i2]);
      // We take the first verse of each chapter, if available, else we take v
      // 0.
...
void GuiNavigation::tracker_sensitivity() {
  // Buttons.
  gtk_widget_set_sensitive(button_list_back,
                           track.previous_reference_available());
  ...
  gtk_widget_set_sensitive(button_list_forward,
                           track.next_reference_available());
}
...
void GuiNavigation::on_list_back() {
  ...
  RadiobuttonDialog dialog(
      _("Go back"), _("Where would you like to go back to?"), labels, 0, true);
...
void GuiNavigation::on_list_forward() {
  ...
  RadiobuttonDialog dialog(_("Go forward"),
                           _("Where would you like to go forward to?"), labels,
                           0, true);
  if (dialog.run() == GTK_RESPONSE_OK) {
    for (unsigned int i = 0; i <= dialog.selection; i++) {
      on_forward();
...
void GuiNavigation::on_entry_free_activate(GtkEntry *entry_box,
                                           gpointer user_data) {
...
void GuiNavigation::on_entry_free() {
  ...
  if (reference_discover(oldRef, gtk_entry_get_text(GTK_ENTRY(entry_free)),
                         reference, true)) {

...
void SystemlogDialog::setup(void) {
  ...
  gtk_builder_add_from_file(gtkbuilder,
                            gw_build_filename(Directories->get_package_data(),
                                              "gtkbuilder.systemlogdialog.xml")
                                .c_str(),
                            NULL);
  ...
  // Experimental - trying to make this window not modal, so it can float always
  // to the side.
  ...
  checkbutton_session =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "checkbutton_session"));
  shortcuts.button(checkbutton_session);
  g_signal_connect((gpointer)checkbutton_session, "toggled",
                   G_CALLBACK(on_checkbutton1_toggled), gpointer(this));

  GSList *radiobuttongroup = NULL;

  radiobutton_main =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "radiobutton_main"));
  shortcuts.button(radiobutton_main);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_main),
                             radiobuttongroup);
  radiobuttongroup =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_main));
  g_signal_connect((gpointer)radiobutton_main, "toggled",
                   G_CALLBACK(on_radiobutton_toggled), gpointer(this));

  radiobutton_shutdown =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "radiobutton_shutdown"));
  shortcuts.button(radiobutton_shutdown);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_shutdown),
                             radiobuttongroup);
  radiobuttongroup =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_shutdown));
  g_signal_connect((gpointer)radiobutton_shutdown, "toggled",
                   G_CALLBACK(on_radiobutton_toggled), gpointer(this));

  radiobutton_settings =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "radiobutton_settings"));
  shortcuts.button(radiobutton_settings);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_settings),
                             radiobuttongroup);
  radiobuttongroup =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_settings));
  g_signal_connect((gpointer)radiobutton_settings, "toggled",
                   G_CALLBACK(on_radiobutton_toggled), gpointer(this));

  /* button_diag = GTK_WIDGET (gtk_builder_get_object (gtkbuilder,
  "button_diag"));
  g_signal_connect((gpointer) button_diag, "clicked",
  G_CALLBACK(on_button_diagnostics_clicked), gpointer(this));
  shortcuts.button (button_diag);

  InDialogHelp * indialoghelp = new InDialogHelp(dialog, gtkbuilder, &shortcuts,
  NULL);
  ...
  event_source_id = g_timeout_add_full(G_PRIORITY_DEFAULT, 500,
                                       GSourceFunc(show_script_dialog_load),
                                       gpointer(this), NULL);
}
...
int SystemlogDialog::run() {
  ...
  g_signal_connect((gpointer)(getDialog()), "close",
                   G_CALLBACK(on_system_log_close_activate), gpointer(this));
  // This handles clicking the X of the window
  g_signal_connect((gpointer)(getDialog()), "delete-event",
                   G_CALLBACK(on_system_log_delete_event_activate),
                   gpointer(this));
  // This handles the "secondary" destroy signal that is emitted as a result of
  // return FALSE from each of the above signal
  // handlers.
  g_signal_connect((gpointer)(getDialog()), "destroy",
                   G_CALLBACK(on_system_log_destroy_activate), gpointer(this));

  gtk_widget_show_all(dialog);
  return 0;
}

gboolean SystemlogDialog::on_system_log_close_activate(GtkDialog *dlg,
                                                       gpointer user_data) {
...
gboolean
SystemlogDialog::on_system_log_delete_event_activate(GtkDialog *dlg,
                                                     gpointer user_data) {
...
void SystemlogDialog::on_system_log_destroy_activate(GtkDialog *dlg,
                                                     gpointer user_data) {
...
void SystemlogDialog::load(bool force) {
  ...
  // Write contents of settings file (yes, I know...inefficient) (was called
  // "diagnostics")
  ...
  // In the case that the user has asked for the previous session information,
  // AND
  // we did not just get a click on one of the radio or check buttons (force is
  // true in those cases), then we can be lazy
  // and not do anything because no change is possible to the log files on disk.
  // This also has the benefit of helping me (Postiff) track down a memory leak
  // that is
  // occurring when the System log dialog box stays open on the main and
  // settings log
  // files. It uses about 24KB every second while it sits open.
  gboolean showPreviousSession =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(checkbutton_session));
  if (!force && showPreviousSession) {
    // Since we are showing the previous session's log file, there will be no
    // change in it, so we can
    // skip all this work.
    return;
  }

  // In cases that message keep streaming in, it may happen that the user tries
  // to copy
  // the messages. He selects the text, but before he can copy it to the
  // clipboard,
  // the new text being loaded erases his selection. The result is that he fails
  // to copy the text to the clipboard. This problem is resolved by introducing
  // the
  ...
  if (!loadSuccess) { // Also contents is set to NULL and length (third
  ...
  gchar *oldContents = gtk_text_buffer_get_text(
      textbuffer, &begin, &end, false); // this call allocates memory
  if (g_ascii_strcasecmp(contents, oldContents) != 0) {
    ...
    gtk_text_buffer_get_end_iter(
        gtk_text_view_get_buffer(GTK_TEXT_VIEW(textview)), &end);
...
void SystemlogDialog::on_checkbutton1_toggled(GtkToggleButton *togglebutton,
                                              gpointer user_data) {
...
ustring SystemlogDialog::logfilename() {
  ...
  return log_file_name(type, gtk_toggle_button_get_active(
                                 GTK_TOGGLE_BUTTON(checkbutton_session)));
}
...
LogFileType SystemlogDialog::currentLogFileType() {
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(radiobutton_shutdown))) {
    type = lftShutdown;
                 ...
                 GTK_TOGGLE_BUTTON(radiobutton_settings))) {
    type = lftSettings;
  }
  return type;
}
...
void SystemlogDialog::writeSettings() {
  ...
  lines.push_back(_("Note that the above settings are always from the current "
                    "run of Bibledit, not the previous session\n."));

  // Add the settings info to the temp directory.
  ustring settingsfile =
      gw_build_filename(Directories->get_temp(),
                        _("settings.log")); // should pull settings.log out and
                                            // use log_file_name, and then fix
                                            // that so that it is not a
                                            // stand-alone function
  write_lines(settingsfile, lines);         // This closes the file when done.
  return;
}

void SystemlogDialog::on_radiobutton_toggled(GtkToggleButton *togglebutton,
                                             gpointer user_data) {

...
WindowCheckKeyterms::WindowCheckKeyterms(GtkWidget *parent_layout,
                                         GtkAccelGroup *accelerator_group,
                                         bool startup)
    : FloatingWindow(parent_layout, widCheckKeyterms, _("Check keyterms"),
                     startup),
      myreference(0)
// Window for checking keyterms.
{
  ...
  gtk_box_pack_start(GTK_BOX(hbox_collection), label_collection, FALSE, FALSE,
                     0);
  ...
  gtk_box_pack_start(GTK_BOX(hbox_collection), combobox_collection, TRUE, TRUE,
                     0);
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolledwindow_terms),
                                 GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
  gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(scrolledwindow_terms),
                                      GTK_SHADOW_IN);
  ...
  treestore_renderings = gtk_tree_store_new(4, G_TYPE_BOOLEAN, G_TYPE_BOOLEAN,
                                            G_TYPE_STRING, G_TYPE_BOOLEAN);

  treeview_renderings =
      gtk_tree_view_new_with_model(GTK_TREE_MODEL(treestore_renderings));
  ...
  g_signal_connect(renderer_renderings, "toggled",
                   G_CALLBACK(keyterm_whole_word_toggled), gpointer(this));
  ...
  g_signal_connect(renderer_renderings, "toggled",
                   G_CALLBACK(keyterm_case_sensitive_toggled), gpointer(this));
  column = gtk_tree_view_column_new_with_attributes(
      _("Case\nsensitive"), renderer_renderings, "active", 1, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview_renderings), column);
  renderer_renderings = gtk_cell_renderer_text_new();
  g_signal_connect(renderer_renderings, "edited", G_CALLBACK(cell_edited),
                   gpointer(this));
  column = gtk_tree_view_column_new_with_attributes(
      _("Rendering"), renderer_renderings, "text", 2, "editable", 3, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview_renderings), column);
  treeselect_renderings =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(treeview_renderings));
  gtk_tree_selection_set_mode(treeselect_renderings, GTK_SELECTION_SINGLE);

  g_signal_connect(
      (gpointer)webview_terms, "navigation-policy-decision-requested",
      G_CALLBACK(on_navigation_policy_decision_requested), gpointer(this));
  g_signal_connect((gpointer)combobox_collection, "changed",
                   G_CALLBACK(on_combobox_keyterm_collection_changed),
                   gpointer(this));

  gtk_label_set_mnemonic_widget(GTK_LABEL(label_collection),
                                combobox_collection);
...
void WindowCheckKeyterms::load_renderings() {
  ...
  keyterms_retrieve_renderings(project, keyterm, category, renderings,
...
void WindowCheckKeyterms::save_renderings() {
  ...
  extern Settings *settings;
  ustring project = settings->genconfig.project_get();
  keyterms_store_renderings(project, keyterm, category, renderings, wholewords,
                            casesensitives);
  load_renderings();
...
void WindowCheckKeyterms::on_rendering_toggle(GtkCellRendererToggle *cell,
                                              gchar *path_str,
                                              bool first_toggle) {
...
void WindowCheckKeyterms::on_cell_edited(GtkCellRendererText *cell,
                                         const gchar *path_string,
...
void WindowCheckKeyterms::add_to_renderings(const ustring &rendering,
                                            bool wholeword)
...
bool WindowCheckKeyterms::find_renderings(const ustring &text,
...
// rendering starts at.
// lengths: If non-NULL, will be filled with the lengths of the renderings
// found.
...
void WindowCheckKeyterms::get_renderings(vector<ustring> &renderings,
                                         vector<bool> &wholewords,
                                         vector<bool> &casesensitives) {
  GtkTreeModel *model = (GtkTreeModel *)treestore_renderings;
  GtkTreeIter iter;
...
gboolean WindowCheckKeyterms::on_navigation_policy_decision_requested(
  ...
  ((WindowCheckKeyterms *)user_data)
      ->navigation_policy_decision_requested(request, navigation_action,
                                             policy_decision);
  return true;
}

void WindowCheckKeyterms::navigation_policy_decision_requested(
    WebKitNetworkRequest *request, WebKitWebNavigationAction *navigation_action,
...
// Callback for clicking a link.
{
  ...
  GtkAdjustment *adjustment = gtk_scrolled_window_get_vadjustment(
      GTK_SCROLLED_WINDOW(scrolledwindow_terms));
...
void WindowCheckKeyterms::html_link_clicked(const gchar *url) {
  ...
  if (active_url.find(_("keyterm ")) == 0) {
    ...
    // Load the renderings.
    // To be done before displaying the verses themselves since the latter
    ...
    if (collection().find(_("Biblical")) != string::npos) {
      if (collection().find(_("Hebrew")) != string::npos) {
        ...
        htmlwriter.text_add(
            _("Key Terms in Biblical Hebrew: The entries are an experimental "
              "sample set, not yet fully reviewed and approved. The KTBH team "
              "would welcome feed-back to christopher_samuel@sil.org."));
    ...
    for (unsigned int i = 0; i < terms.size(); i++) {
      htmlwriter.paragraph_open();
      htmlwriter.hyperlink_add(_("keyterm ") + convert_to_string(ids[i]),
                               terms[i]);
      htmlwriter.paragraph_close();
    }
    display_another_page = true;
  ...
  if (display_another_page) {
    // Load the page.
    webkit_web_view_load_string(WEBKIT_WEB_VIEW(webview_terms),
...
void WindowCheckKeyterms::html_write_keyterms(HtmlWriter2 &htmlwriter,
    ...
    while (pos != string::npos) {
      ...
      if (pos != string::npos) {
        ...
        // Remap the reference.
        {
          ...
          vector<int> chapters;
          vector<int> verses;
          mapping.original_to_me(reference.chapter_get(), reference.verse_get(),
                                 chapters, verses);
          if (!chapters.empty()) {
        ...
        htmlwriter.hyperlink_add("goto " + remapped_reference_text,
                                 remapped_reference_text);
        ...
        if (find_renderings(verse, renderings, wholewords, casesensitives,
          ...
          for (unsigned int i = 0; i < startpositions.size(); i++) {
            htmlwriter.text_add(
                verse.substr(0, startpositions[i] - processposition));
            htmlwriter.bold_open();
            htmlwriter.text_add(
                verse.substr(startpositions[i] - processposition, lengths[i]));
            htmlwriter.bold_close();
            verse.erase(0, startpositions[i] - processposition + lengths[i]);
            processposition = startpositions[i] + lengths[i];
          }
          // Add whatever is left over of the verse. This could be the full
          // verse in case it wasn't processed.
...
{
  gw_destroy_source(text_changed_event_id);
  text_changed_event_id = g_timeout_add_full(
      G_PRIORITY_DEFAULT, 500, GSourceFunc(on_text_changed_timeout),
      gpointer(this), NULL);
  my_editor = editor;
}
...
void WindowCheckKeyterms::set_fonts() {
  ...
  if (!settings->genconfig.text_editor_font_default_get()) {
    ...
    WebKitWebSettings *webkit_settings =
        webkit_web_view_get_settings(WEBKIT_WEB_VIEW(webview_terms));

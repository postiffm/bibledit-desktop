...
// Formats the project and shows it in a pdf viewer.
{
  ...
  if (scriptureportions->books.empty()) {
    gtkw_dialog_info(
        NULL,
        _("There were no books to print\nSelect some books and try again"));
    return;
  }
  // Settings.
  extern Settings *settings;
  ProjectConfiguration *projectconfig =
      settings->projectconfig(myproject->name);
  ...
  // Line spacing...was in project config, but better to have it in general
  // config
  ...
  for (unsigned int i = 0; i < scriptureportions->books.size(); i++) {
    ...
    select_portion_get_values(portionproject,
                              books_english_to_id(scriptureportions->books[i]),
                              scriptureportions->portions[i], chapters_from,
                              verses_from, chapters_to, verses_to);
    usfm2text.add_print_portion(
        books_english_to_id(scriptureportions->books[i]), chapters_from,
        verses_from, chapters_to, verses_to);
  }

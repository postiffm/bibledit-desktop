...
bool robinson_define_parsing_suffix(ustring &definition, ustring &parsing,
                                    bool silent)
// Parse the suffix.
...
In the 2000 Maurice Robinson's coding manual version, there were two
interpretations of a final -C (comparative or crasis).
In the 2004 version -C should always indicate comparative and -K should indicate
crasis.
There may be old content that uses -C for crasis, but at the time of writing
this Bibledit does not use it.
  */
  ...
  if (parsing == "S") {
    ...
    definition.append(_(" merged by crasis with a second word; declension is "
                        "that of the second word"));
  } else if (parsing == "ATT") {

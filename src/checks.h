...
public:
  ...
  void word_inventory(const vector<ustring> &words,
                      const vector<unsigned int> &count, int total, int unique,
                      int filteredtotal, int filteredunique, int excludelimit);
  void usfm_count(const vector<ustring> &usfms,
                  const vector<unsigned int> &count, const ustring &stylesheet);
  void character_count(const vector<ustring> &chars,
                       const vector<unsigned int> &count);
  void references_inventory(const vector<ustring> &verses,
                            const vector<ustring> &references);
  void ot_quotations_in_nt(const vector<ustring> nt_refs,
                           const vector<ustring> nt_texts,
                           const vector<VectorUstring> &ot_refs,
                           const vector<VectorUstring> &ot_texts,
                           const gchar *mainheading);
  void parallel_passages(const vector<OtNtParallelDataSection> &data,
                         const gchar *mainheading);

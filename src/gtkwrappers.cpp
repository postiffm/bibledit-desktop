...
void gtkw_dialog_info(GtkWidget *parent, const ustring &info) {
  ...
  GtkWidget *dialog = gtk_message_dialog_new(
      ...
      GTK_MESSAGE_INFO, GTK_BUTTONS_OK, "%s", info.c_str());
  // Add cancel button so that pressing the Escape button will also close the
  // window,
  // but hide this button so it's not visible, but Escape remains working.
  GtkWidget *cancel =
      gtk_dialog_add_button(GTK_DIALOG(dialog), "", GTK_RESPONSE_CANCEL);
...
int gtkw_dialog_question(GtkWidget *parent, const ustring &question,
                         int default_response) {
  int result;
  GtkWidget *dialog = gtk_message_dialog_new(
      GTK_WINDOW(parent), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_QUESTION,
      GTK_BUTTONS_YES_NO, "%s", question.c_str());
...
void gtkw_dialog_warning(GtkWidget *parent, const ustring &warning) {
  GtkWidget *dialog = gtk_message_dialog_new(
      GTK_WINDOW(parent), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_WARNING,
      GTK_BUTTONS_OK, "%s", warning.c_str());
  gtk_dialog_run(GTK_DIALOG(dialog));
  gtk_widget_destroy(dialog);
}

ustring gtkw_file_chooser_select_folder(GtkWidget *parent, const ustring &title,
...
// Returns the folder selected, or is empty if no selection was made.
{
  ...
  dialog = gtk_file_chooser_dialog_new(
      mytitle.c_str(), GTK_WINDOW(parent),
...
ustring gtkw_file_chooser_open(GtkWidget *parent, const ustring &title,
                               const ustring &file)
...
// Returns the file that was selected, or is empty if no selection was made.
{
  ...
  GtkWidget *dialog;
  dialog = gtk_file_chooser_dialog_new(
      mytitle.c_str(), GTK_WINDOW(parent), GTK_FILE_CHOOSER_ACTION_OPEN,
...
vector<ustring> gtkw_file_chooser_open_multiple(GtkWidget *parent,
                                                const ustring &title,
                                                ustring directory)
...
ustring gtkw_file_chooser_save(GtkWidget *parent, const ustring &title,
                               const ustring &file)
...
// Returns the file that was selected, or is empty if no selection was made.
{
  ...
  GtkWidget *dialog;
  dialog = gtk_file_chooser_dialog_new(
      mytitle.c_str(), GTK_WINDOW(parent), GTK_FILE_CHOOSER_ACTION_SAVE,
...
void gtkw_show_uri(ustring uri, bool internet) {
  ...
  // We do something similar to what is done at
  // https://build.opensuse.org/package/view_file?file=0012-Work-around-disfunctional-gtk_show_uri-on-Windows.patch&package=mingw32-evince&project=home%3Ahiberis%3Awix&rev=93e9eb22b7da0de079015d2b2014e02c

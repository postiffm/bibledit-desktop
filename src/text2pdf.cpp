...
// Converts text code to a pdf file.
{
  ...
  surface = cairo_pdf_surface_create(
      pdffile.c_str(), pango_units_to_points(page_width_pango_units),
...
void Text2Pdf::page_size_set(double width_centimeters,
                             double height_centimeters)
// Set the page size in centimeters.
{
  if (text2pdf_intermediary_2_double(intermediate_text_pointer, __func__,
                                     width_centimeters, height_centimeters))
    return;
...
void Text2Pdf::page_margins_set(double inside_margin_centimeters,
...
{
  if (text2pdf_intermediary_4_double(
          intermediate_text_pointer, __func__, inside_margin_centimeters,
          right_margin_centimeters, top_margin_centimeters,
          bottom_margin_centimeters))
    return;
...
{
  if (text2pdf_intermediary_1_double(intermediate_text_pointer, __func__,
                                     size_centimeters))
    return;
...
// Lays out one paragraph.
{
  ...
  if (input_paragraph->space_before_mm != 0) {
    ...
    int column_count = get_column_count_rectangle_width(
        input_paragraph->column_count, rectangle.width);
...
// Gets the next layout container to be used.
{
  ...
  block->keep_with_next =
      input_paragraph->keep_with_next || keep_data_together || intrusion;
  if (intrusion) {
...
// Fits the block onto the pages, and creates new pages if need be.
{
  ...
  while (!input_blocks.empty() && infinite_loop_counter < 5000) {
    ...
    // Progress.
    double fraction =
        double(initial_input_size - input_blocks.size()) / initial_input_size;
...
int Text2Pdf::get_column_count_rectangle_width(int column_count_in,
                                               int &width) {
...
// Produces the next page and related objects.
{
  ...
  page =
      new T2PPage(page_number, page_width_pango_units, page_height_pango_units,
                  inside_margin_pango_units, outside_margin_pango_units,
...
{
  if (text2pdf_intermediary_1_bool(intermediate_text_pointer, __func__,
                                   underline))
    return;
  ensure_open_paragraph();
...
{
  if (text2pdf_intermediary_1_int(intermediate_text_pointer, __func__,
                                  alignment))
    return;
  ensure_open_paragraph();
...
{
  if (text2pdf_intermediary_1_ustring(intermediate_text_pointer, __func__,
                                      text))
    return;
...
{
  if (text2pdf_intermediary_2_int(intermediate_text_pointer, __func__, left,
                                  right))
    return;
...
// Interpretes the intermediate commands it finds.
{
  ...
  for (unsigned int i = 0; i < intermediate_text.size(); i++) {
    ...
    // In case of the command to add text, don't interprete the | separater
    // character, but pass it through.
    ...
    if (command == "page_size_set") {
      ...
      text2pdf_intermediary_4_double_get(parameters, double1, double2, double3,
                                         double4);

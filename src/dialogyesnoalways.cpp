...
bool yes_no_always_dialog(const ustring &message, YesNoAlwaysDialogType type,
                          bool default_yes, bool default_always)
/*
This is a dialog that asks the user for either Yes or No, and optional the user
can choose to always give this answer
for the remainder of the session.
message: message.
type : dialog type.
default_yes: Make the Yes button the default response for the dialog.
default_always: Set the dialog so that the answer the user now chooses is
...
*/
{
  ...
  if (!settings->session.yes_no_always_dialog_enabled[type]) {
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dialog.checkbutton),
                                 default_always);
    response = dialog.run();
    settings->session.yes_no_always_dialog_enabled[type] =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(dialog.checkbutton));
...
YesNoAlwaysDialog::YesNoAlwaysDialog(const ustring &message, bool default_yes) {
  ...
  gtk_builder_add_from_file(
      gtkbuilder, gw_build_filename(Directories->get_package_data(),
                                    "gtkbuilder.yesnoalwaysdialog.xml")
                      .c_str(),
      NULL);
  ...
  // Add the buttons. In the standard question dialog, the Yes is at the right,
  // then the No following to the left.
...
int YesNoAlwaysDialog::run() { return gtk_dialog_run(GTK_DIALOG(dialog)); }

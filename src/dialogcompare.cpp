...
CompareDialog::CompareDialog(WindowReferences *references_window) {
  ...
  gtk_window_set_position(GTK_WINDOW(comparedialog),
  ...
  label1 = gtk_label_new(_("Select a project to compare the current project "
                           "with\nand then press OK"));
  ...
  selectprojectgui->build(vbox1, "",
                          settings->genconfig.project_to_compare_with_get());

  checkbutton1 =
      gtk_check_button_new_with_mnemonic(_("Print changed verses only"));
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbutton1),
                               settings->genconfig.print_changes_only_get());
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(comparedialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(comparedialog), okbutton,
                               GTK_RESPONSE_OK);
  ...
  g_signal_connect((gpointer)button_change, "clicked",
...
int CompareDialog::run() { return gtk_dialog_run(GTK_DIALOG(comparedialog)); }

bool CompareDialog::on_combobox1_key_press_event(GtkWidget *widget,
                                                 GdkEventKey *event,
                                                 gpointer user_data) {
...
void CompareDialog::static_on_okbutton_clicked(GtkButton *button,
                                               gpointer user_data) {
...
// Ok handler: do the comparison.
{
  ...
  settings->genconfig.print_changes_only_set(
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(checkbutton1)));
  // Run comparison.
  compare_with(my_references_window,
               settings->genconfig.project_to_compare_with_get(),
               settings->genconfig.project_get(),
               settings->genconfig.print_changes_only_get());
}

void CompareDialog::on_button_portion_clicked(GtkButton *button,
                                              gpointer user_data) {

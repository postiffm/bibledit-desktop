...
// Backup assistant.
{
  ...
  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), vbox_select_type,
                               _("What would you like to backup?"));
  ...
  radiobutton_select_type_resource =
      gtk_radio_button_new_with_mnemonic(NULL, _("Resource"));
  gtk_widget_show(radiobutton_select_type_resource);
  gtk_box_pack_start(GTK_BOX(vbox_select_type),
                     radiobutton_select_type_resource, FALSE, FALSE, 0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_select_type_resource),
                             radiobutton_select_type_group);
  radiobutton_select_type_group = gtk_radio_button_get_group(
      GTK_RADIO_BUTTON(radiobutton_select_type_resource));
  ...
  gtk_box_pack_start(GTK_BOX(vbox_select_type),
                     radiobutton_select_type_everything, FALSE, FALSE, 0);
  gtk_radio_button_set_group(
      GTK_RADIO_BUTTON(radiobutton_select_type_everything),
      radiobutton_select_type_group);
  radiobutton_select_type_group = gtk_radio_button_get_group(
      GTK_RADIO_BUTTON(radiobutton_select_type_everything));
  ...
  gtk_box_pack_start(GTK_BOX(vbox_bible_name), label_bible_name, FALSE, FALSE,
                     0);
  ...
  page_number_resource_name =
      gtk_assistant_append_page(GTK_ASSISTANT(assistant), vbox_resource_name);
  gtk_container_set_border_width(GTK_CONTAINER(vbox_resource_name), 10);

  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), vbox_resource_name,
                               _("Is this the right Resource?"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant), vbox_resource_name,
                              GTK_ASSISTANT_PAGE_CONTENT);
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant), vbox_resource_name,
                                  true);

  label_resource_name = gtk_label_new(_("Resource name"));
  gtk_widget_show(label_resource_name);
  gtk_box_pack_start(GTK_BOX(vbox_resource_name), label_resource_name, FALSE,
                     FALSE, 0);

  button_resource_name = gtk_button_new();
  gtk_widget_show(button_resource_name);
  gtk_box_pack_start(GTK_BOX(vbox_resource_name), button_resource_name, FALSE,
                     FALSE, 0);

  g_signal_connect((gpointer)button_resource_name, "clicked",
                   G_CALLBACK(on_button_resource_name_clicked), gpointer(this));
  ...
  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), label_summary,
                               _("The backup was completed successfully."));
...
void BackupAssistant::on_assistant_prepare_signal(GtkAssistant *assistant,
                                                  GtkWidget *page,
                                                  gpointer user_data) {
...
void BackupAssistant::on_assistant_prepare(GtkWidget *page) {
  ...
  if (page == vbox_resource_name) {
    ...
    if (resource_name.empty()) {
      gtk_label_set_text(GTK_LABEL(label_resource_name),
                         _("No Resource selected"));
    }
    gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant),
                                    vbox_resource_name, !resource_name.empty());
  }
...
void BackupAssistant::on_assistant_apply_signal(GtkAssistant *assistant,
                                                gpointer user_data) {
...
void BackupAssistant::on_assistant_apply() {
  ...
  // summary_page_number);
  // The forward_function logic handles which page is next; don't be setting it
  // ourselves here!
}

gint BackupAssistant::assistant_forward_function(gint current_page,
                                                 gpointer user_data) {
...
gint BackupAssistant::assistant_forward(gint current_page) {
  ...
  /*   do {
      ...
      if (current_page > (gint) summary_page_number) current_page =
    summary_page_number; */
...
void BackupAssistant::on_button_bible_name_clicked(GtkButton *button,
                                                   gpointer user_data) {
...
void BackupAssistant::on_button_resource_name_clicked(GtkButton *button,
                                                      gpointer user_data) {
...
void BackupAssistant::on_button_file_clicked(GtkButton *button,
                                             gpointer user_data) {
...
BackupType BackupAssistant::get_type() {
  ...
  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(radiobutton_select_type_resource))) {
    return btResource;
  }
  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(radiobutton_select_type_everything))) {
    return btAll;
  }
  return btAll;
}

...
USFMView::USFMView(GtkWidget *vbox, const ustring &project_in) {
  ...
  markup_tag = gtk_text_buffer_create_tag(GTK_TEXT_BUFFER(sourcebuffer), NULL,
                                          "foreground", "red", NULL);
  ...
  g_signal_connect(G_OBJECT(sourcebuffer), "changed",
                   G_CALLBACK(on_textbuffer_changed), gpointer(this));

  // The view's signal handlers.
  g_signal_connect_after((gpointer)sourceview, "move_cursor",
                         G_CALLBACK(on_textview_move_cursor), gpointer(this));
  g_signal_connect_after((gpointer)sourceview, "grab_focus",
                         G_CALLBACK(on_textview_grab_focus), gpointer(this));
  g_signal_connect((gpointer)sourceview, "button_press_event",
                   G_CALLBACK(on_textview_button_press_event), gpointer(this));
  g_signal_connect((gpointer)sourceview, "key-press-event",
  ...
  GtkTextTagTable *texttagtable =
      gtk_text_buffer_get_tag_table(GTK_TEXT_BUFFER(sourcebuffer));
...
// Loads a chapter with the number "chapter_in".
{
  ...
  // Because the buffer has just been cleared, this marker will always be at the
  // end.
...
ustring USFMView::chapter_get_ustring() {
  ...
  gchar *text = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(sourcebuffer),
                                         &startiter, &enditer, true);
...
bool USFMView::can_undo() { return gtk_source_buffer_can_undo(sourcebuffer); }

void USFMView::undo() { gtk_source_buffer_undo(sourcebuffer); }

bool USFMView::can_redo() { return gtk_source_buffer_can_redo(sourcebuffer); }

void USFMView::redo() { gtk_source_buffer_redo(sourcebuffer); }
...
void USFMView::font_set() {
  ...
  if (projectconfig->editor_default_color_get()) {
    ...
    color_widget_set(sourceview, projectconfig->editor_normal_text_color_get(),
...
void USFMView::on_textbuffer_changed(GtkTextBuffer *textbuffer,
                                     gpointer user_data) {
...
void USFMView::on_textview_move_cursor(GtkTextView *textview,
...
void USFMView::on_textview_cursor_moved() { restart_verse_tracker(); }

void USFMView::on_textview_grab_focus(GtkWidget *widget, gpointer user_data) {
  ((USFMView *)user_data)->textview_grab_focus();
}

void USFMView::textview_grab_focus() { restart_verse_tracker(); }

void USFMView::restart_verse_tracker() {
  gw_destroy_source(verse_tracker_event_id);
  verse_tracker_event_id = g_timeout_add_full(
      G_PRIORITY_DEFAULT, 100, GSourceFunc(on_verse_tracker_timeout),
      gpointer(this), NULL);
}
...
bool USFMView::on_verse_tracker() {
  ...
  gtk_text_buffer_get_iter_at_mark(
      GTK_TEXT_BUFFER(sourcebuffer), &startiter,
      gtk_text_buffer_get_insert(GTK_TEXT_BUFFER(sourcebuffer)));
  ustring verse = "0";
  ...
  gtk_text_iter_forward_chars(&startiter, 3);
  if (gtk_text_iter_backward_search(&startiter, "\\v ", GtkTextSearchFlags(0),
                                    &startiter, NULL, NULL)) {
    GtkTextIter enditer = startiter;
    gtk_text_iter_forward_chars(&enditer, 3);
    enditer = startiter;
...
// Moves the cursor to the verse
{
  ...
  gtk_text_iter_forward_search(&startiter, usfmcode.c_str(),
                               GtkTextSearchFlags(0), &startiter, NULL, NULL);
  gtk_text_buffer_place_cursor(GTK_TEXT_BUFFER(sourcebuffer), &startiter);
  textview_scroll_to_mark(
      GTK_TEXT_VIEW(sourceview),
      gtk_text_buffer_get_insert(GTK_TEXT_BUFFER(sourcebuffer)), true);
}
...
{
  GtkTextIter startiter, enditer;
  gtk_text_buffer_get_selection_bounds(GTK_TEXT_BUFFER(sourcebuffer),
                                       &startiter, &enditer);
  gchar *text = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(sourcebuffer),
                                         &startiter, &enditer, true);
...
// If text is selected, this is erased first.
{
  ...
  // Erase selected text.
  gtk_text_buffer_delete_selection(GTK_TEXT_BUFFER(sourcebuffer), true,
                                   editable);

  // Insert the new text.
  gtk_text_buffer_insert_at_cursor(GTK_TEXT_BUFFER(sourcebuffer), text.c_str(),
                                   -1);
}

gboolean USFMView::on_textview_button_press_event(GtkWidget *widget,
...
void USFMView::on_texteditor_click(GtkWidget *widget, GdkEventButton *event) {
  ...
  if (event->type == GDK_2BUTTON_PRESS) {
    ...
    GtkTextIter enditer;
    gtk_text_buffer_get_iter_at_mark(
        GTK_TEXT_BUFFER(sourcebuffer), &enditer,
        gtk_text_buffer_get_insert(GTK_TEXT_BUFFER(sourcebuffer)));
    if (!gtk_text_iter_ends_word(&enditer))
      gtk_text_iter_forward_word_end(&enditer);
    startiter = enditer;
    gtk_text_iter_backward_word_start(&startiter);
    gchar *txt = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(sourcebuffer),
...
/*
 ...
 rawtext:   The raw text of the note, e.g. "+ Mat 1.1.". This excludes the note
 opener and note closer.
 */
{
...
void USFMView::cut() {
  ...
  gtk_text_buffer_cut_clipboard(GTK_TEXT_BUFFER(sourcebuffer), clipboard,
                                editable);
}
...
void USFMView::paste() {
  GtkClipboard *clipboard = gtk_clipboard_get(GDK_SELECTION_CLIPBOARD);
  gtk_text_buffer_paste_clipboard(GTK_TEXT_BUFFER(sourcebuffer), clipboard,
                                  NULL, editable);
}

gboolean USFMView::on_textview_key_press_event(GtkWidget *widget,
                                               GdkEventKey *event,
                                               gpointer user_data) {
  return ((USFMView *)user_data)->textview_key_press_event(widget, event);
}

gboolean USFMView::textview_key_press_event(GtkWidget *widget,
                                            GdkEventKey *event) {
...
{
  bool moved = spellingchecker->move_cursor_to_spelling_error(
      GTK_TEXT_BUFFER(sourcebuffer), next, extremity);
  if (moved) {
    textview_scroll_to_mark(
        GTK_TEXT_VIEW(sourceview),
        gtk_text_buffer_get_insert(GTK_TEXT_BUFFER(sourcebuffer)), true);
...
void USFMView::on_button_spelling_recheck_clicked(GtkButton *button,
                                                  gpointer user_data) {

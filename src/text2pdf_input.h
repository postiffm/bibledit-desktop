...
enum T2PInputType {
  t2pitParagraph,
  t2pitOpenKeepTogether,
  t2pitCloseKeepTogether,
  t2pitNewPage,
  t2pitNewOddPage
};
...
public:
  T2PInputParagraph(const ustring &font, unsigned int line_spacing_in,
  ...
  bool inline_get_font_size(unsigned int index, bool &in_range, int &size,
                            int &start_index, int &end_index);
  void inline_set_italic(T2PMarkupType italic, bool cleanup_only = false);
  bool inline_get_italic(unsigned int index, bool &in_range, bool &value,
                         int &start_index, int &end_index);
  void inline_set_bold(T2PMarkupType bold, bool cleanup_only = false);
  bool inline_get_bold(unsigned int index, bool &in_range, bool &value,
                       int &start_index, int &end_index);
  void inline_set_underline(T2PMarkupType underline, bool cleanup_only = false);
  bool inline_get_underline(unsigned int index, bool &in_range, bool &value,
                            int &start_index, int &end_index);
  void inline_set_small_caps(T2PMarkupType small_caps,
                             bool cleanup_only = false);
  bool inline_get_small_caps(unsigned int index, bool &in_range, bool &value,
                             int &start_index, int &end_index);
  void inline_set_superscript(bool superscript);
  bool inline_get_superscript(unsigned int index, bool &in_range,
                              bool &superscript, int &start_index,
                              int &end_index, size_t text_length);
  void inline_set_colour(int colour);
  bool inline_get_colour(unsigned int index, bool &in_range, int &value,
                         int &start_index, int &end_index);
  void inline_set_strike_through(T2PMarkupType strike_through,
                                 bool cleanup_only = false);
  bool inline_get_strike_through(unsigned int index, bool &in_range,
                                 bool &value, int &start_index, int &end_index);
  void add_note(T2PInputParagraph *note);
  vector<T2PInputParagraph *> get_notes(size_t text_length_before_fitting,
...
private:
  ...
  void inline_set_value(vector<int> &values, vector<int> &start_indices,
                        vector<int> &end_indices, T2PMarkupType value,
                        bool cleanup_only);
  bool inline_get_value(vector<int> &values, vector<int> &start_indices,
                        vector<int> &end_indices, bool &values_completed,
                        bool paragraph_value, unsigned int index,
                        bool &in_range, bool &value, int &start_index,
                        int &end_index);

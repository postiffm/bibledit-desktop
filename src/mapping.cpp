...
void Mapping::me_to_original(int mychapter, const ustring &myverse,
                             vector<int> &original_chapter,
                             vector<int> &original_verse)
/*
Input:
- a chapter (int).
...
*/
{
  ...
  set<int> encoded;
  // Divide the half-verses by 2 and add them to the containers with values to
  // map.
...
void Mapping::me_to_original(Reference &reference) {
  ...
  me_to_original(reference.chapter_get(), reference.verse_get(),
                 remapped_chapters, remapped_verses);
  ...
  if (remapped_verses.size() > 1) {
    ...
    reference.verse_append(
        convert_to_string(remapped_verses[remapped_verses.size() - 1]));
  }
}
...
void Mapping::original_to_me(vector<int> original_chapter,
                             vector<int> original_verse, vector<int> &mychapter,
                             vector<int> &myverse) {
...
void Mapping::original_to_me(Reference &reference) {
  vector<int> remapped_chapters;
  vector<int> remapped_verses;
  original_to_me(reference.chapter_get(), reference.verse_get(),
                 remapped_chapters, remapped_verses);
  reference.chapter_set(remapped_chapters[0]);
  reference.verse_set(convert_to_string(remapped_verses[0]));
  if (remapped_verses.size() > 1) {
...
void Mapping::get_original(int mychapter, int myverse,
                           vector<int> &originalchapter,
                           vector<int> &originalverse) {
  extern Mappings *mappings;
  mappings->me2original(myname, mybook, mychapter, myverse, originalchapter,
                        originalverse);
...
void Mapping::get_me(int originalchapter, int originalverse,
                     vector<int> &mychapter, vector<int> &myverse) {
  extern Mappings *mappings;
  mappings->original2me(myname, mybook, originalchapter, originalverse,
                        mychapter, myverse);
  if (mychapter.empty()) {

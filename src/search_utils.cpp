...
void search_string_basic(const ustring &project, bool use_book_selection,
...
*/
{
  ...
  for (unsigned int bk = 0; bk < books.size(); bk++) {
    ...
    for (unsigned int ch = 0; ch < chapters.size(); ch++) {
      ...
      ustring chapterfilename =
          project_data_filename_chapter(project, book, chapter, false);
      gchar *quickcontent;
      bool wordfound = false;
      if (g_file_get_contents(chapterfilename.c_str(), &quickcontent, NULL,
                              NULL)) {
        gchar *foundlocation;
        if (casesensitive) {
...
ustring search_in_bibledit_assemble_line(const ustring &input,
                                         AreaType areatype, bool area_id,
...
bool search_in_bibledit_word_boundaries_match(const ustring &text,
                                              const ustring &searchword,
                                              bool matchbeginning,
                                              bool matchending, bool globbing)
  ...
  for (unsigned int bk = 0; bk < availablebooks.size(); bk++) {
    ...
    try {
      ...
      vector<unsigned int> chapters =
          project_get_chapters(project, availablebooks[bk]);
      for (unsigned int ch = 0; ch < chapters.size(); ch++) {
        ...
        vector<ustring> lines =
            project_retrieve_chapter(project, availablebooks[bk], chapters[ch]);
        ...
        for (unsigned int i = 0; i < ccv.verse.size(); i++) {
          ...
          ustring text = search_in_bibledit_assemble_line(
              input, areatype, area_id, area_intro, area_heading, area_chapter,
              area_study, area_notes, area_xref, area_verse);
          ...
          if (!search_in_bibledit_word_boundaries_match(
                  text, localsearchword, search_start_word_match,
                  search_end_word_match, search_globbing))
            continue;
...
void search_load_references(WindowReferences *references_window,
                            vector<Reference> &searchresults)
/*
This function takes the searchresults from a search, and depending on
information
...
*/
{
  ...
  // Deal with how the current search results interact with the ones in the
  // editor.
  ...
  case sstLoad: {
    ...
    references_window->set(searchresults, settings->genconfig.project_get(),
                           NULL);
    break;
  }
  case sstAdd: {

...
ReplaceDialog::ReplaceDialog(int dummy) {
  ...
  gtk_window_set_position(GTK_WINDOW(replacedialog),
  ...
  gtk_table_attach(GTK_TABLE(table1), label7, 0, 1, 0, 1,
                   (GtkAttachOptions)(GTK_FILL), (GtkAttachOptions)(0), 4, 4);
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbuttoncase),
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbuttonbook),
                               settings->session.search_current_book);

  checkbuttonchapter =
      gtk_check_button_new_with_mnemonic(_("Current chapter only"));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(replacedialog), buttonfind,
                               GTK_RESPONSE_OK);
  gtk_widget_set_can_default(GTK_WIDGET(buttonfind), true);

  buttoncancel = gtk_button_new_from_stock("gtk-cancel");
  gtk_widget_show(buttoncancel);
  gtk_dialog_add_action_widget(GTK_DIALOG(replacedialog), buttoncancel,
                               GTK_RESPONSE_CANCEL);
  ...
  g_signal_connect((gpointer)checkbuttonbook, "toggled",
                   G_CALLBACK(on_checkbuttonbook_toggled), gpointer(this));
  g_signal_connect((gpointer)checkbuttonchapter, "toggled",
                   G_CALLBACK(on_checkbuttonchapter_toggled), gpointer(this));
  ...
  g_signal_connect((gpointer)buttonfind, "clicked",
                   G_CALLBACK(replacedialog_on_buttonfind_clicked),
                   gpointer(this));
...
ReplaceDialog::~ReplaceDialog() { gtk_widget_destroy(replacedialog); }

int ReplaceDialog::run() { return gtk_dialog_run(GTK_DIALOG(replacedialog)); }

void ReplaceDialog::on_checkbuttonbook_toggled(GtkToggleButton *togglebutton,
                                               gpointer user_data) {
  ((ReplaceDialog *)user_data)->set_gui();
}

void ReplaceDialog::on_checkbuttonchapter_toggled(GtkToggleButton *togglebutton,
                                                  gpointer user_data) {
  ((ReplaceDialog *)user_data)->set_gui();
}
...
void ReplaceDialog::on_buttonfind_clicked() {
  ...
  settings->session.search_case_sensitive =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(checkbuttoncase));
  settings->session.search_current_book =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(checkbuttonbook));
  settings->session.search_current_chapter =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(checkbuttonchapter));
  ...
  search_string_basic(settings->genconfig.project_get(), true, chapter,
                      results);
  ...
  SessionHighlights sessionhighlights1(
      searchword, settings->session.search_case_sensitive, false, false, false,
      atRaw, false, false, false, false, false, false, false, false);
  settings->session.highlights.push_back(sessionhighlights1);
  SessionHighlights sessionhighlights2(
      replaceword, settings->session.search_case_sensitive, false, false, false,
      atRaw, false, false, false, false, false, false, false, false);
...
void ReplaceDialog::on_word_entry_changed(GtkEditable *editable,
                                          gpointer user_data) {
  ((ReplaceDialog *)user_data)->set_gui();
}
...
void ReplaceDialog::set_gui() {
  ...
  gtk_widget_set_sensitive(selectbutton,
                           !(current_book_active || current_chapter_active));
}

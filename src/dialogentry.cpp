...
EntryDialog::EntryDialog(const ustring &title, const ustring &info,
                         const ustring &value) {
  my_always_ok = false;

  entrydialog = gtk_dialog_new();
  gtk_window_set_title(GTK_WINDOW(entrydialog), title.c_str());
  gtk_window_set_position(GTK_WINDOW(entrydialog),
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(entrydialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(entrydialog), okbutton,
                               GTK_RESPONSE_OK);
  gtk_widget_set_can_default(GTK_WIDGET(okbutton), true);

  gtk_widget_set_sensitive(okbutton, false);

  g_signal_connect((gpointer)entry, "changed", G_CALLBACK(on_entry_changed),
                   gpointer(this));
...
EntryDialog::~EntryDialog() { gtk_widget_destroy(entrydialog); }

int EntryDialog::run() { return gtk_dialog_run(GTK_DIALOG(entrydialog)); }

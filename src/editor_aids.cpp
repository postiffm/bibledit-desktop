...
// TO DO: With static variables in here, this is a candidate to move into the
// Editor2 class.
void marker_get_type_and_subtype(const ustring &project, const ustring &marker,
...
/*
 Returns the property "plain text" of a certain style when displayed in the
 Editor.
...
// Gets the verse marker, normally the "v".
{
  ...
  ustring style = "v";
  for (unsigned int i = 0; i < usfm->styles.size(); i++) {
    if (style_get_starts_verse_number(usfm->styles[i].type,
                                      usfm->styles[i].subtype)) {
...
ustring style_get_default_note_style(const ustring &project,
                                     EditorNoteType type) {
  ustring stylesheet = stylesheet_get_actual();
  extern Styles *styles;
  Usfm *usfm = styles->usfm(stylesheet);
...
// Get the marker that starts a new row in a table.
{
  ...
  ustring style = "tr";
  for (unsigned int i = 0; i < usfm->styles.size(); i++) {
    if (style_get_starts_table_row(usfm->styles[i].type,
                                   usfm->styles[i].subtype)) {
      style = usfm->styles[i].marker;
      break;
    }
...
// Column starts with 1 for the first column.
{
  ...
  for (unsigned int i = 0; i < usfm->styles.size(); i++) {
    if (style_get_starts_table_cell(usfm->styles[i].type,
                                    usfm->styles[i].subtype)) {
...
NoteNumberingType note_numbering_type_get(const ustring &project,
                                          const ustring &marker)
/*
 Gets the numbering type of a note, for example, the numbering could be
 numerical
 or alphabethical.
 */
{
...
ustring note_numbering_user_sequence_get(const ustring &project,
                                         const ustring &marker)
...
gint table_get_n_columns(GtkTable *table) {
  gint n_columns =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(table), "n_columns"));
  return n_columns;
}
...
void usfm_internal_get_text_close_character_style(ustring &text,
...
ustring usfm_get_note_text(GtkTextIter startiter, GtkTextIter enditer,
                           const ustring &project) {
  ...
  while (gtk_text_iter_compare(&iter, &enditer) < 0) {
    ...
    if (new_character_style != previous_character_style) {
      ...
      if (new_character_style.empty()) {
        marker_get_type_and_subtype(project, previous_character_style, type,
                                    subtype);
      ...
      if (paragraph_initialized) {
        if (!new_paragraph_style.empty()) {
          usfm_internal_add_text(
              notetext, usfm_get_full_opening_marker(new_paragraph_style));
        }
      }
    }
    ...
    /*
       ...
       But the fr style was only applied to "v1:", thus giving a space after the
       \ft marker.
       The "\ft " marker itself already has a space.
       We then find that case that one space follows the other.
       Two consecusitive spaces in USFM count as only one space, hence this
       space
       was removed. And that again results in this faulty text:
       "v1:one two three".
       A special routine solves that, putting the space before the "\ft "
       marker.
     */
...
void get_styles_at_iterator(GtkTextIter iter, ustring &paragraph_style,
...
vector<ustring> get_character_styles_between_iterators(GtkTextIter startiter,
                                                       GtkTextIter enditer)
// Gets the character styles between two iterators given.
// To do this properly, it is assumed that the first style encountered will
// always be the paragraph style,
...
bool get_verse_number_at_iterator_internal(GtkTextIter iter,
...
ustring
get_verse_number_at_iterator(GtkTextIter iter, const ustring &verse_marker,
                             const ustring &project, GtkWidget *parent_box)
/*
...
bool get_iterator_at_verse_number(const ustring &verse_number,
                                  const ustring &verse_marker,
                                  GtkWidget *parent_box, GtkTextIter &iter,
                                  GtkWidget *&textview, bool deep_search)
...
// Gives the name of the style that is for markers that are not in the
// stylesheet.
{
  return "unknown";
}
...
// Gives the clipboard text that signifies that no text was copied, but USFM
// code only.
{
  return "<USFM>";
}

void textbuffer_apply_named_tag(GtkTextBuffer *buffer, const ustring &name,
...
void textbuffer_insert_with_named_tags(GtkTextBuffer *buffer, GtkTextIter *iter,
                                       const ustring &text,
                                       ustring first_tag_name,
                                       ustring second_tag_name)
...
// If a tag does not exist, it applies the "unknown" style instead.
{
  ...
  if (second_tag_name.empty()) {
    gtk_text_buffer_insert_with_tags_by_name(buffer, iter, text.c_str(), -1,
                                             first_tag_name.c_str(), NULL);
  } else {
    gtk_text_buffer_insert_with_tags_by_name(buffer, iter, text.c_str(), -1,
                                             first_tag_name.c_str(),
                                             second_tag_name.c_str(), NULL);
  }
}
...
void editor_text_append(GtkTextBuffer *textbuffer, const ustring &text,
                        const ustring &paragraph_style,
                        const ustring &character_style)
...
// It inserts the text at the cursor.
{
  ...
  gtk_text_buffer_get_iter_at_mark(textbuffer, &insertiter,
                                   gtk_text_buffer_get_insert(textbuffer));
  // Insert text together with the style(s).
  textbuffer_insert_with_named_tags(textbuffer, &insertiter, text,
                                    paragraph_style, character_style);
}

gint editor_paragraph_insertion_point_get_offset(
    EditorActionCreateParagraph *paragraph_action) {
  gint offset = 0;
  if (paragraph_action) {
    GtkTextBuffer *textbuffer =
        gtk_text_view_get_buffer(GTK_TEXT_VIEW(paragraph_action->textview));
    GtkTextIter iter;
...
void editor_paragraph_insertion_point_set_offset(
    EditorActionCreateParagraph *paragraph_action, gint offset) {
  if (paragraph_action) {
    GtkTextBuffer *textbuffer =
        gtk_text_view_get_buffer(GTK_TEXT_VIEW(paragraph_action->textview));
    GtkTextIter iter;
...
EditorActionDeleteText *paragraph_delete_last_character_if_space(
    EditorActionCreateParagraph *paragraph_action)
// Creates an action for deleting text for the last character in the text buffer
// if it is a space.
{
  if (paragraph_action) {
    GtkTextBuffer *textbuffer =
...
bool text_starts_paragraph(const ustring &project, ustring &line,
                           const ustring &marker, size_t marker_pos,
...
bool text_starts_verse(const ustring &project, ustring &line,
                       const ustring &marker_text, size_t marker_pos,
                       size_t marker_length, bool is_opener,
                       bool marker_found) {
  if (marker_found) {
    if (marker_pos == 0) {
...
vector<GtkWidget *> editor_get_widgets(GtkWidget *vbox) {
  ...
  gtk_container_foreach(GTK_CONTAINER(vbox), on_editor_get_widgets_callback,
                        gpointer(&widgets));
  return widgets;
}
...
EditorActionDeleteText *paragraph_get_text_and_styles_after_insertion_point(
    EditorActionCreateParagraph *paragraph, vector<ustring> &text,
    vector<ustring> &styles)
// This function accepts a paragraph, and gives a list of text and their
// associated character styles
// from the insertion point to the end of the buffer.
// It returns the EditorAction that would be required to erase that text from
// the paragraph.
// Note that this EditorAction needs to be applied for the effect to be
// obtained.
// If it is not applied, it should then be destroyed.
{
  GtkTextBuffer *textbuffer =
      gtk_text_view_get_buffer(GTK_TEXT_VIEW(paragraph->textview));
  ...
  if (end_offset > start_offset) {
    delete_action = new EditorActionDeleteText(paragraph, start_offset,
                                               end_offset - start_offset);
  }
  return delete_action;
}

void get_text_and_styles_between_iterators(GtkTextIter *startiter,
                                           GtkTextIter *enditer,
                                           vector<ustring> &text,
                                           vector<ustring> &styles)
// This function gives a list of the text and character styles between two
// iterators.
...
void editor_park_widget(GtkWidget *vbox, GtkWidget *widget, gint &offset,
                        GtkWidget *parking)
...
bool move_end_iterator_before_note_caller_and_validate(
    GtkTextIter startiter, GtkTextIter enditer, GtkTextIter &moved_enditer)
// If the iterator is after a note caller, it moves the iterator till it is
// before the note caller.

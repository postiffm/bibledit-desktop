...
void view_parallel_bible_pdf() {
  ...
  if (chapters.empty()) {
    gtkw_dialog_info(NULL, books_id_to_english(settings->genconfig.book_get()) +
                               _("does not exist in this project"));
    return;
  }
  // Progress system.
  ...
  for (unsigned int i = 0; i < project_s_raw.size(); i++) {
    if (project_book_exists(project_s_raw[i], settings->genconfig.book_get())) {
      ...
      messages.push_back(
          _("Project ") + project_s_raw[i] +
          _(" was requested to be included, but it does not contain ") +
  ...
  WithinReferencesRange inrange;
  {
    ...
    select_portion_get_values(
        ...
        portions_chapter_from, portions_verse_from, portions_chapter_to,
        portions_verse_to);
    inrange.add_portion(settings->genconfig.book_get(), portions_chapter_from,
                        portions_verse_from, portions_chapter_to,
                        portions_verse_to);
  ...
  for (unsigned int ch = 0; ch < chapters.size(); ch++) {
    ...
    vector<ustring> verses =
        project_get_verses(settings->genconfig.project_get(),
                           settings->genconfig.book_get(), chapters[ch]);
    for (unsigned int vs = 0; vs < verses.size(); vs++) {
      ...
      // Store the reference.
      Reference reference(settings->genconfig.book_get(), chapters[ch],
                          verses[vs]);
  ...
  view_parallel_references_pdf(
      projectmemory, &project_names, references,
      settings->genconfig.parallel_bible_keep_verses_together_get(), &messages,
      false);

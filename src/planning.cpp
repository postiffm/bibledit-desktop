...
// Edits the planning.
{
  ...
  vector<ustring> tasks =
      settings->projectconfig(project)->planning_tasks_get();
  vector<ustring> addables;
  {
    ...
    for (unsigned int i = 0; i < addable_books.size(); i++) {
      addables.push_back(
          planning_assemble_task(addable_books[i], addable_tasks[i]));
    }
  }
  EditListDialog dialog(&tasks, _("Planned Tasks"),
                        _("of planned tasks: add, remove or reorder them"),
...
void planning_disassemble_task(ustring assembled_task, unsigned int &book,
                               ustring &task) {
  book = 0;
  task.clear();
...
// This produces a planning report.
{
  ...
  // Current date.
  htmlwriter.paragraph(_("Produced on ") +
  ...
  // Date project finishes based on starting date and work completed.
  {
    ...
    ustring paragraphtext = _("The project started on ") +
                            date_time_julian_human_readable(startdate, false) +
                            ", ";
    unsigned int percentage =
        reporting_get_percentage_ready_project(&projectstatus);
    paragraphtext.append(_("and ") + convert_to_string(percentage) +
                         _(" percent of the work has been done. "));
    bool calculation_possible =
        ((currentdate > startdate + 30) && (percentage > 1));
    if (calculation_possible) {
      ...
      date_time_normal_get_year_month_day(enddate, year_complete,
                                          month_complete, day_complete);
      paragraphtext.append(_("If the work is carried on with the same pace it "
                             "will be completed in the year ") +
                           convert_to_string(year_complete) +
                           _(", God willing."));
    } else {
      paragraphtext.append(_("At this stage it is not yet possible to estimate "
                             "when it will finish."));
    }
  ...
  // List of planned tasks, including their dates.
  {
    ...
    for (unsigned int i = 0; i < assembled_tasks.size(); i++) {
      ...
      unsigned int percentage_complete =
          reporting_get_task_percentage_complete(&projectstatus, book, task);
      double duration = 0;
      {
        ...
        vector<unsigned int> chapters = versification_get_chapters(
            projectconfig->versification_get(), book);
      ...
      day += (unsigned int)(round(duration * 7 / 5 *
                                  (100 - percentage_complete) / 100));
    ...
    htmlwriter.table(_("Planned tasks"), column_headers, cell_texts, "",
                     &centers, 0);
  }
}

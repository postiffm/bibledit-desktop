...
// URL transporter.
{
  ...
  // Create a session.
  // Maximum connections is set to 2. One for the listener, and one for posting
  // messages.
  // This is done so that only one messages is posted at a time. If more are
  // posted,
  // the messages get posted out of order, and this may cause confusion in
  // particular
  // when doing the various git commands out of order.
  session = soup_session_async_new_with_options(
      SOUP_SESSION_USER_AGENT, "bibledit/1.0", SOUP_SESSION_MAX_CONNS, "50",
      SOUP_SESSION_MAX_CONNS_PER_HOST, "50", NULL);
  // SoupLogger *logger;
  ...
  soup_session_queue_message(session, server_test_msg,
                             SoupSessionCallback(on_message_ready_callback),
                             gpointer(this));
}
...
void URLTransport::on_message_ready_callback(SoupSession *session,
                                             SoupMessage *msg,
                                             gpointer user_data) {
...
void URLTransport::on_message_in_sequence_ready_callback(SoupSession *session,
                                                         SoupMessage *msg,
                                                         gpointer user_data) {
  ((URLTransport *)user_data)->on_message_in_sequence_ready(session, msg);
}

void URLTransport::on_message_in_sequence_ready(SoupSession *session,
                                                SoupMessage *msg) {
  sequential_messages_in_transit = false;
  send_next_message_in_sequence();
}

void URLTransport::on_message_with_reply_ready_callback(SoupSession *session,
                                                        SoupMessage *msg,
                                                        gpointer user_data) {
  ((URLTransport *)user_data)->on_message_with_reply_ready(session, msg);
}

void URLTransport::on_message_with_reply_ready(SoupSession *session,
                                               SoupMessage *msg) {
  // Trap a reply.
  vector<SoupMessage *>::iterator message_iter =
      messages_awaiting_reply.begin();
  ...
  for (unsigned int i = 0; i < messages_awaiting_reply.size(); i++) {
    if (msg == messages_awaiting_reply[i]) {
      ...
      // (retrieving msg->response_body->data on message failure crashes
      // bibledit).
      if (reply_is_ok)
      ...
      // Remove trap.
      // Note: Do this before signalling the caller, since the caller may modify
      // the trapping system by placing another message in the queue.
...
void URLTransport::send_next_message_in_sequence() {
  ...
  soup_session_queue_message(
      session, msg, SoupSessionCallback(on_message_in_sequence_ready_callback),
      gpointer(this));
}
...
// Sends a message off, and sets up a system for trapping the reply.
{
  ...
  soup_session_queue_message(
      session, msg, SoupSessionCallback(on_message_with_reply_ready_callback),
      gpointer(this));

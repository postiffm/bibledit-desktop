...
GotoReferenceDialog::GotoReferenceDialog(unsigned int currentbook,
  ...
  g_signal_connect((gpointer)dialog, "key_press_event",
                   G_CALLBACK(on_key_press_event), gpointer(this));

  notebook = GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "notebook"));

  // Enter current reference into the free entry, and select it, so user can use
  // that.
  entry_free = GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "entry_free"));
  gtk_widget_grab_focus(entry_free);
  ustring ref = books_id_to_name(language, currentbook) + " " +
                convert_to_string(currentchapter) + ":" + currentverse;
  ...
  // Aided entry widgets.
  treeview_book =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "treeview_book"));
  treeview_chapter =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "treeview_chapter"));
  treeview_verse =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "treeview_verse"));

  // Connect to the signals of the treeviews.
  g_signal_connect_after((gpointer)treeview_book, "move_cursor",
                         G_CALLBACK(on_treeview_book_move_cursor),
                         gpointer(this));
  g_signal_connect_after((gpointer)treeview_book, "row_activated",
                         G_CALLBACK(on_treeview_book_row_activated),
                         gpointer(this));
  g_signal_connect_after((gpointer)treeview_chapter, "move_cursor",
                         G_CALLBACK(on_treeview_chapter_move_cursor),
                         gpointer(this));
  g_signal_connect_after((gpointer)treeview_chapter, "row_activated",
                         G_CALLBACK(on_treeview_chapter_row_activated),
                         gpointer(this));
  g_signal_connect_after((gpointer)treeview_verse, "move_cursor",
                         G_CALLBACK(on_treeview_verse_move_cursor),
                         gpointer(this));
  g_signal_connect_after((gpointer)treeview_verse, "row_activated",
                         G_CALLBACK(on_treeview_verse_row_activated),
                         gpointer(this));
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview_book),
                          GTK_TREE_MODEL(store_book));
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview_chapter),
                          GTK_TREE_MODEL(store_chapter));
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview_verse),
                          GTK_TREE_MODEL(store_verse));
  ...
  // History back.
  radiobutton_back =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "radiobutton_back"));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(radiobutton_back), true);

  // History forward.
  radiobutton_forward =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "radiobutton_forward"));

  // Link history button.
  GSList *history_button_group = NULL;
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_back),
                             history_button_group);
  history_button_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_back));
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_forward),
                             history_button_group);
  history_button_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_forward));

  // Dialog action area.
  InDialogHelp *indialoghelp =
      new InDialogHelp(dialog, gtkbuilder, &shortcuts, NULL);
...
GotoReferenceDialog::~GotoReferenceDialog() { gtk_widget_destroy(dialog); }

int GotoReferenceDialog::run() { return gtk_dialog_run(GTK_DIALOG(dialog)); }
...
/*
  ...
  info.
  For example, when the user types "1", it is interpreted as verse one of the
  current
  chapter of the current book. If he types "21 10" it is interpreted as the
  current
...
*/
{
  ...
  if (page == 0) {
    ...
    reference.book_set(
        books_name_to_id(language, listview_get_active_string(treeview_book)));
    reference.chapter_set(
        convert_to_int(listview_get_active_string(treeview_chapter)));
    ...
    go_forward =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(radiobutton_forward));
  }
}
...
void GotoReferenceDialog::on_okbutton_clicked(GtkButton *button,
                                              gpointer user_data) {
  ((GotoReferenceDialog *)user_data)->on_jump();
}

gboolean GotoReferenceDialog::on_key_press_event(GtkWidget *widget,
                                                 GdkEventKey *event,
                                                 gpointer user_data) {
...
void GotoReferenceDialog::on_entry_free_activate(GtkEntry *entry,
                                                 gpointer user_data) {
...
gboolean GotoReferenceDialog::on_treeview_book_move_cursor(
    GtkTreeView *treeview, GtkMovementStep step, gint count,
    gpointer user_data) {
  ((GotoReferenceDialog *)user_data)->on_move_book();
  return false;
}

void GotoReferenceDialog::on_treeview_book_row_activated(
    GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *column,
    gpointer user_data) {
  ((GotoReferenceDialog *)user_data)->on_activate_book();
}

gboolean GotoReferenceDialog::on_treeview_chapter_move_cursor(
    GtkTreeView *treeview, GtkMovementStep step, gint count,
    gpointer user_data) {
...
void GotoReferenceDialog::on_treeview_chapter_row_activated(
    GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *column,
    gpointer user_data) {
  ((GotoReferenceDialog *)user_data)->on_activate_chapter();
}

gboolean GotoReferenceDialog::on_treeview_verse_move_cursor(
    GtkTreeView *treeview, GtkMovementStep step, gint count,
    gpointer user_data) {
...
void GotoReferenceDialog::on_treeview_verse_row_activated(
    GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *column,
    gpointer user_data) {
...
void GotoReferenceDialog::on_move_book() {
  ...
  unsigned int bookid =
      books_name_to_id(language, listview_get_active_string(treeview_book));
  // Something weird in Gtk: If the listview was not in view, it gives no
  // results.
  // Deal with that here.
  if (bookid == 0)
    return;
...
void GotoReferenceDialog::on_move_chapter() {
  ...
  verses = project_get_verses(settings->genconfig.project_get(), bookid,
                              convert_to_int(chapter));
...
void GotoReferenceDialog::on_activate_verse() { on_activate(); }

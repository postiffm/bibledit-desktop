...
void kjv_home_entry(HtmlWriter2 &htmlwriter) {
  ...
  htmlwriter.hyperlink_add(kjv_html_entry_url(),
                           _("Sword King James Bible with Strong's lemmata"));
  htmlwriter.paragraph_close();
}

ustring kjv_html_entry_url() { return "kjv_entry"; }
...
void kjv_detailed_page(HtmlWriter2 &htmlwriter) {
  ...
  htmlwriter.paragraph_open();
  htmlwriter.text_add(
      _("The King James Bible from the Sword project is useful for its "
        "Strong's numbers tagged in the text. Bibledit-Gtk uses this to find "
        "related verses, among other things. Importing consists of two steps: "
        "exporting it from Sword and creating a database out of it."));
  htmlwriter.paragraph_close();
  htmlwriter.heading_open(4);
  htmlwriter.text_add(_("1. Exporting the text from Sword"));
  htmlwriter.heading_close();
  htmlwriter.paragraph_open();
  htmlwriter.text_add(_("To export the text from the Sword library, it is "
                        "supposed that the KJV Bible has been installed. Open "
  ...
  htmlwriter.paragraph_open();
  htmlwriter.text_add(
      _("This will take a while. It should give no errors. Once it finishes "
        "there will be a file called kjv.txt in the home directory."));
  htmlwriter.paragraph_close();
  htmlwriter.heading_open(4);
  ...
  htmlwriter.paragraph_open();
  htmlwriter.text_add(
      _("Creating the database should take a while. Once done there will be a "
        "file called kjv.sql in the home directory. This file should be added "
        "to the bibledit package, replacing the one it currently has."));
  htmlwriter.paragraph_close();
}

ustring kjv_create_database_url() { return "kjv_create_db"; }

void kjv_action_page(HtmlWriter2 &htmlwriter) {
  htmlwriter.heading_open(3);
  htmlwriter.text_add(_("Database creation from King James Bible exported from "
                        "the Sword library"));
  htmlwriter.heading_close();
  ...
  if (keep_going) {
    messages.push_back(_("Importing data into the database at ") +
                       kjv_sql_file);
  ...
  if (keep_going) {
    htmlwriter.paragraph_open();
    htmlwriter.text_add(_("To use the database, copy the file kjv.sql into the "
                          "Bibledit-Gtk package and re-install."));
    htmlwriter.paragraph_close();
  }
...
void kjv_import_sword(const ustring &textfile, const ustring &database) {
  ...
  sqlite3_exec(db, "create table kjv (book integer, chapter integer, verse "
                   "integer, item integer, fragment text, lemma text);",
               NULL, NULL, NULL);
    ...
    while ((xmlTextReaderRead(reader) == 1)) {
      ...
      case XML_READER_TYPE_ELEMENT: {
        ...
        if (!xmlStrcmp(element_name, BAD_CAST "verse")) {
          ...
          char *attribute;
          attribute =
              (char *)xmlTextReaderGetAttribute(reader, BAD_CAST "osisID");
          if (attribute) {
        ...
        if (!xmlStrcmp(element_name, BAD_CAST "w")) {
          ...
          char *attribute;
          attribute =
              (char *)xmlTextReaderGetAttribute(reader, BAD_CAST "lemma");
          if (attribute) {
            lemmata = attribute;
            free(attribute);
          }
      ...
      case XML_READER_TYPE_END_ELEMENT: {
        ...
        if (!xmlStrcmp(element_name, BAD_CAST "w")) {
          ...
          char *sql;
          sql = g_strdup_printf(
              "insert into kjv values (%d, %d, %d, %d, '%s', '%s');",
              reference.book_get(), reference.chapter_get(),
              convert_to_int(reference.verse_get()), item_number,
...
void kjv_get_strongs_data(const Reference &reference, vector<ustring> &strongs,
...
// This gets the words and their applicable Strong's numbers for a verse.
{
  ...
  try {
    ...
    char *sql;
    sql = g_strdup_printf("select fragment, lemma from kjv where book = %d and "
                          "chapter = %d and verse = %d order by item asc;",
                          reference.book_get(), reference.chapter_get(),
                          convert_to_int(reference.verse_get()));
...
// Passing a Strong's number, this returns all the verses that contain this
// Strong's number.
{
  ...
  try {
    ...
    sqlite3_busy_timeout(db, 1000);
    char *sql;
    // The first like finds any lemma field with 'G68' alone or '...G68' at the
    // end of the lemma.
    // The second like finds any lemma that has G68 at the beginning or middle
    // but not followed by a digit
    // as in 'G689'. The '%%' is the way to get the sql % character in the
    // pattern match. MAP
    sql = g_strdup_printf("select distinct book, chapter, verse from kjv where "
                          "lemma like '%%%s' or lemma like '%%%s[^0-9]%%';",
                          strongs.c_str(), strongs.c_str());
    rc = sqlite3_exec(db, sql, reader.callback, &reader, &error);
    g_free(sql);
    if (rc) {
      throw runtime_error(sqlite3_errmsg(db));
    ...
    for (unsigned int i = 0; i < reader.ustring0.size(); i++) {
      // Get the references, and store it only if it comes from the same
      // Testament we need.
      Reference ref(convert_to_int(reader.ustring0[i]),
                    convert_to_int(reader.ustring1[i]), reader.ustring2[i]);

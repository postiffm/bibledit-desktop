...
void select_portion_get_values(const ustring &project, unsigned int book,
                               const ustring &portion,
...
// Checks whether everything is in the limits as given by "bookmetrics".
{
  ...
  for (unsigned int i = 0; i < portions.size(); i++) {
    ...
    if (portions[i] == CHAPTER_VERSE_SELECTION_ALL) {
      ...
      vector<ustring> from_verses =
          project_get_verses(project, book, chapter_from);
      ...
      myportion.erase(0,
                      verse_from.length() + strlen(CHAPTER_VERSE_SELECTION_TO));
...
ustring select_portion_get_label(const ustring &project, unsigned int book,
                                 ...
                                 const ustring &verse_from,
                                 unsigned int chapter_to,
                                 const ustring &verse_to) {
  ...
  if (chapter_from == lowest_chapter) {
    ...
    if (chapter_to == highest_chapter) {
      ...
      if (verse_from == from_verses[0]) {
        vector<ustring> to_verses =
            project_get_verses(project, book, chapter_to);
...
void WithinReferencesRange::add_portion(unsigned int book_in,
  ...
  for (unsigned int i = 0; i < chapters_from.size(); i++) {
    ...
    if (books[i] == mybook) {
      ...
      if (mychapter < chapters_from[i]) {
        ...
        // At this stage the chapter is within the range of chapters to be
        // printed.

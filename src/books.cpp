...
// When Bibledit got new defined books, there will be books not yet in the
// user's
...
// the existing books.
{
  ...
  vector<unsigned int> books_sorted;
  {
    ...
    for (unsigned int i = 0; i < project_books.size(); i++) {
      if (bookset.find(project_books[i]) != bookset.end()) {
        if (books_sorted_inserted.find(project_books[i]) ==
            books_sorted_inserted.end()) {
  ...
  if (books_sorted.empty()) {
    books.clear();
    quick_sort(books_left_sequences, books_left, 0,
               books_left_sequences.size());
    for (unsigned int i = 0; i < books_left.size(); i++) {
  ...
  for (unsigned int i = 0; i < books_left.size(); i++) {
    ...
    if (insert_location >= books_sorted.size()) {
      ...
      vector<unsigned int>::iterator sequence_iter =
          books_sorted_sequences.begin();
...
unsigned int books_abbreviation_to_id(const ustring &language,
                                      const ustring &abbreviation)
...
unsigned int books_abbreviation_to_id_loose(const ustring &language,
                                            const ustring &abbreviation)
// This returns the id of "abbreviation" in "language".
...
unsigned int books_english_to_id(const ustring &english) {
  ...
  int id = bookmap[s1]; // the unordered_map provides O(1) time to lookup
                        // instead of O(n)
  if (id != 0) {
    return id;
  }
  ...
  for (unsigned int i = 0; i < bookdata_books_count(); i++) {
    ...
    if (s1 == s2) {
      // ... and put it into the bookmap too to save time if we see it again
      // later

...
// Looks whether a file as copied from BibleWorks through the clipboard looks
// okay.
{
#define MYMAX 10
  ReadText rt(filename, true, false);
...
// Gets the id of a book from a line of a BibleWorks database copied through the
// clipboard.
...
// or:
// SCR 1 Corinthians 1:1  Παῦλος κλητὸς ἀπόστολος Ἰησοῦ Χριστοῦ διὰ θελήματος
// Θεοῦ, καὶ Σωσθένης ὁ ἀδελφός,
{
...
void import_check_bibleworks_file(vector<ustring> &filenames,
...
// Checks the file exported from BibleWorks and meant to be imported as a Bible.
{
  ...
  if (messages.empty()) {
    ...
    if (id == 0) {
      messages.push_back(
          "The file cannot be recognized as coming from BibleWorks");
    }
  }
  ...
  if (messages.empty()) {
    ...
    if (!books.empty()) {
      messages.push_back(
          "You try to import data into a Bible that is not empty");
    }
  }
}

void import_bibleworks_text_file(const ustring &file, const ustring &bible,
                                 vector<ustring> &messages)
...
void check_bibleworks_source_language(vector<ustring> &filenames,
                                      vector<ustring> &messages) {
  ...
  for (unsigned int i = 0; i < 2; i++) {
    if (!bibleworks_clipboard_file_okay(filenames[i])) {
      messages.push_back(
          "File " + filenames[i] +
          " cannot be recognized as a textfile coming from BibleWorks");
    }
  }

  // If the second file does not have the "@" character, it is not the
  // morphology, then swap the filenames.
  {
  ...
  // Check that the first file has the text.
  {
    ...
    if (!rt.lines.empty()) {
      if (rt.lines[0].find("@") != string::npos) {
        messages.push_back("File " + filenames[0] +
                           " should have the normal text");
      }
    }
  }
  ...
  // Check that the second file has the morphology.
  {
    ...
    if (!rt.lines.empty()) {
      if (rt.lines[0].find("@") == string::npos) {
        messages.push_back("File " + filenames[1] +
                           " should have the morphology");
      }
    }
  }
}
...
bibleworks_greek_record bibleworks_greek_table_1[] = {
    {"B", "Β"}, {"b", "β"},  {"l", "λ"}, {"o", "ο"}, {"j", "ς"}, {"g", "γ"},
    {"e", "ε"}, {"n", "ν"},  {"s", "σ"}, {"w", "ω"}, {"h", "η"}, {"C", "Χ"},
    {"r", "ρ"}, {"t", "τ"},  {"u", "υ"}, {"D", "Δ"}, {"a", "α"}, {"d", "δ"},
    {"(", ","}, {"m", "μ"},  {"Å", "."}, {" ", " "}, {"i", "ι"}, {"v", "γ"},
    {"k", "κ"}, {"\\", "·"},
...
ustring convert_bibleworks_greek(ustring line) {
  ...
  while (!line.empty()) {
    ...
    for (unsigned int i = 0; i < sizeof(bibleworks_greek_table_2) /
                                     sizeof(*bibleworks_greek_table_2);
         i++) {
      if (!converted) {
    ...
    if (!converted) {
      ...
      for (unsigned int i = 0; i < sizeof(bibleworks_greek_table_1) /
                                       sizeof(*bibleworks_greek_table_1);
           i++) {
        if (!converted) {
    ...
    if (!converted) {
      gw_critical("Output so far: " + outputline + " - unhandled character: " +
                  character + " - input stream: " + line);
...
void bibleworks_define_parsing_indeclinable_form_suffix(ustring &parsing,
                                                        ustring &definition)
...
void bibleworks_define_parsing_conjunction_suffix(ustring &parsing,
                                                  ustring &definition)
...
void bibleworks_define_parsing_adjective_suffix(ustring &parsing,
                                                ustring &definition)
...
void bibleworks_define_parsing_particle_suffix(ustring &parsing,
                                               ustring &definition)
...
void bibleworks_define_parsing_adverb_suffix(ustring &parsing,
                                             ustring &definition)
...
// If so, it adds the description to the "definition" and removes the relevant
...
// The definitions were assembled by trial and error, by searching for a
// parsing, then looking what BibleWorks gave for it.
{
  ...
  if (prefix == "v") {
    definition = "verb";
    // Parsing of the verb may follow different routes depending on the length
    // of the parsing.
...
// The reason this was introduced is so that older and future versions of
// BibleWorks are likely to be recognized.
{
  bool running = false;
...
bool bibleworks_reference_get_decode(ustring response, Reference &reference) {
  // If BibleWorks was not running it might get started at this point, but the
  // value it returns is empty as a result of a timeout.
  if (response.empty())
    return false;

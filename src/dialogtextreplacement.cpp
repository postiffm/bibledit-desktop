...
TextReplacementDialog::TextReplacementDialog(int dummy) {
  ...
  gtk_window_set_title(GTK_WINDOW(textreplacementdialog),
                       _("Text Replacement"));
  gtk_window_set_position(GTK_WINDOW(textreplacementdialog),
                          GTK_WIN_POS_CENTER_ON_PARENT);
  gtk_window_set_modal(GTK_WINDOW(textreplacementdialog), TRUE);
  gtk_window_set_type_hint(GTK_WINDOW(textreplacementdialog),
  ...
  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(textreplacementdialog));
  gtk_widget_show(dialog_action_area1);
  gtk_button_box_set_layout(GTK_BUTTON_BOX(dialog_action_area1),
                            GTK_BUTTONBOX_END);

  new InDialogHelp(textreplacementdialog, NULL, &shortcuts,
                   "menu-preferences/text-replacement");

  cancelbutton1 = gtk_button_new_from_stock("gtk-cancel");
  gtk_widget_show(cancelbutton1);
  gtk_dialog_add_action_widget(GTK_DIALOG(textreplacementdialog), cancelbutton1,
                               GTK_RESPONSE_CANCEL);
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(textreplacementdialog), okbutton1,
                               GTK_RESPONSE_OK);
  ...
  g_signal_connect(renderer1, "edited", G_CALLBACK(cell_text_edited),
                   gpointer(this));

  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(treeview1), -1, "Texts", renderer1, "text",
      COLUMN_ORIGINALS, "editable", COLUMN_EDITABLE1, NULL);

  GtkCellRenderer *renderer2 = gtk_cell_renderer_text_new();
  g_signal_connect(renderer2, "edited", G_CALLBACK(cell_replacement_edited),
                   gpointer(this));

  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(treeview1), -1, "Replacements", renderer2, "text",
      COLUMN_REPLACEMENTS, "editable", COLUMN_EDITABLE2, NULL);

  gtk_tree_selection_set_mode(
      gtk_tree_view_get_selection(GTK_TREE_VIEW(treeview1)),
      GTK_SELECTION_SINGLE);

  // Load texts and replacements - these get initialized to defaults if there
  // were none.
  ...
  for (unsigned int i = 0; i < originals.size(); i++) {
    ...
    gtk_list_store_set(model, &iter, COLUMN_ORIGINALS, originals[i].c_str(),
                       COLUMN_EDITABLE1, 1, COLUMN_REPLACEMENTS,
                       replacements[i].c_str(), COLUMN_EDITABLE2, 1, -1);
  }
...
void TextReplacementDialog::on_okbutton1_clicked(GtkButton *button,
                                                 gpointer user_data) {
...
void TextReplacementDialog::on_okbutton() {
  ...
  settings->genconfig.text_replacement_set(
...
void TextReplacementDialog::on_checkbutton1_toggled(
...
void TextReplacementDialog::cell_text_edited(GtkCellRendererText *cell,
                                             const gchar *path_string,
                                             const gchar *new_text,
                                             gpointer data) {
  ((TextReplacementDialog *)data)
      ->on_cell_edited(cell, path_string, new_text, COLUMN_ORIGINALS);
}

void TextReplacementDialog::cell_replacement_edited(GtkCellRendererText *cell,
                                                    const gchar *path_string,
                                                    const gchar *new_text,
                                                    gpointer data) {
  ((TextReplacementDialog *)data)
      ->on_cell_edited(cell, path_string, new_text, COLUMN_REPLACEMENTS);
}

void TextReplacementDialog::on_cell_edited(GtkCellRendererText *cell,
                                           const gchar *path_string,
  ...
  gtk_list_store_set(GTK_LIST_STORE(model), &iter, column, g_strdup(new_text),
                     -1);
  // Free path.
...
void TextReplacementDialog::gui() {
  ...
  while (valid) {
    if (original_get(GTK_TREE_MODEL(model), &iter) == enter_new_text_here())
      if (replacement_get(GTK_TREE_MODEL(model), &iter) ==
          enter_new_replacement_here())
        newdatafound = true;
    valid = gtk_tree_model_iter_next(model, &iter);
  }
  if (!newdatafound) {
    gtk_list_store_append(GTK_LIST_STORE(model), &iter);
    gtk_list_store_set(GTK_LIST_STORE(model), &iter, COLUMN_ORIGINALS,
                       enter_new_text_here(), COLUMN_EDITABLE1, 1,
                       COLUMN_REPLACEMENTS, enter_new_replacement_here(),
                       COLUMN_EDITABLE2, 1, -1);
  }
}
...
ustring TextReplacementDialog::original_get(GtkTreeModel *model,
                                            GtkTreeIter *iter) {
  gchar *str;
...
ustring TextReplacementDialog::replacement_get(GtkTreeModel *model,
                                               GtkTreeIter *iter) {
  gchar *str;

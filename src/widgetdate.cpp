...
*/
{
  ...
  if (showtime) {
    ...
    spinbutton_minute =
        gtk_spin_button_new(GTK_ADJUSTMENT(spinbutton_minute_adj), 1, 0);
    ...
    spinbutton_second =
        gtk_spin_button_new(GTK_ADJUSTMENT(spinbutton_second_adj), 1, 0);
    ...
    spinbutton_hour =
        gtk_spin_button_new(GTK_ADJUSTMENT(spinbutton_hour_adj), 1, 0);
  ...
  g_signal_connect((gpointer)calendar1, "day_selected",
                   G_CALLBACK(on_calendar_changed), gpointer(this));
  if (showtime) {
    g_signal_connect((gpointer)spinbutton_minute, "changed",
                     G_CALLBACK(on_spinbutton_changed), gpointer(this));
    g_signal_connect((gpointer)spinbutton_minute, "value_changed",
                     G_CALLBACK(on_spinbutton_value_changed), gpointer(this));
    g_signal_connect((gpointer)spinbutton_second, "changed",
                     G_CALLBACK(on_spinbutton_changed), gpointer(this));
    g_signal_connect((gpointer)spinbutton_second, "value_changed",
                     G_CALLBACK(on_spinbutton_value_changed), gpointer(this));
    g_signal_connect((gpointer)spinbutton_hour, "changed",
                     G_CALLBACK(on_spinbutton_changed), gpointer(this));
    g_signal_connect((gpointer)spinbutton_hour, "value_changed",
                     G_CALLBACK(on_spinbutton_value_changed), gpointer(this));
  }
...
void DateWidget::set_date() {
  ...
  date_time_normal_get_year_month_day(
      date_time_seconds_to_julian(*my_seconds_since_epoch), year, month, day);
...
void DateWidget::on_spinbutton_changed(GtkEditable *editable,
                                       gpointer user_data) {
  ((DateWidget *)user_data)->calculate_date_time();
}

void DateWidget::on_spinbutton_value_changed(GtkSpinButton *spinbutton,
                                             gpointer user_data) {
  ((DateWidget *)user_data)->calculate_date_time();
}

void DateWidget::on_calendar_changed(GtkCalendar *calendar,
                                     gpointer user_data) {
  ((DateWidget *)user_data)->calculate_date_time();
}
...
void DateWidget::calculate_date_time() {
  ...
  // Setting the widgets gives a lot of signals, which disturbs the whole
  // process.
  ...
  month++;
  *my_seconds_since_epoch = date_time_julian_to_seconds(
      date_time_julian_day_get_parse(year, month, day));
  if (label_time) {
    unsigned int extra_seconds =
        gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(spinbutton_second));
    extra_seconds += (60 * gtk_spin_button_get_value_as_int(
                               GTK_SPIN_BUTTON(spinbutton_minute)));
    extra_seconds += (3600 * gtk_spin_button_get_value_as_int(
                                 GTK_SPIN_BUTTON(spinbutton_hour)));

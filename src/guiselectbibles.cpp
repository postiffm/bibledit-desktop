...
SelectBiblesGui::SelectBiblesGui(GtkWidget *box, Shortcuts &shortcuts) {
  ...
  g_signal_connect((gpointer)button_remove, "clicked",
                   G_CALLBACK(on_button_remove_clicked), gpointer(this));
  ...
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(treeview), -1, _("Enabled"), renderer0, "active", 0, NULL);
  g_signal_connect(renderer0, "toggled", G_CALLBACK(on_enabled_toggled),
                   gpointer(this));
  GtkCellRenderer *renderer1 = gtk_cell_renderer_text_new();
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(treeview), -1, _("Bible"), renderer1, "text", 1, NULL);
  gtk_tree_selection_set_mode(
      gtk_tree_view_get_selection(GTK_TREE_VIEW(treeview)),
      GTK_SELECTION_MULTIPLE);
}

SelectBiblesGui::~SelectBiblesGui() { gtk_widget_destroy(hbox); }

void SelectBiblesGui::set(const vector<bool> &enabled,
                          const vector<ustring> &bible) {
...
void SelectBiblesGui::on_button_add_clicked(GtkButton *button,
                                            gpointer user_data) {
...
void SelectBiblesGui::on_button_remove_clicked(GtkButton *button,
                                               gpointer user_data) {
...
void SelectBiblesGui::on_button_remove() {
  ...
  gtk_tree_selection_selected_foreach(
      selection, SelectBiblesGui::on_collect_iters, gpointer(&iters));
...
void SelectBiblesGui::on_collect_iters(GtkTreeModel *model, GtkTreePath *path,
                                       GtkTreeIter *iter, gpointer data) {
  ((vector<GtkTreeIter> *)data)->push_back(*iter);
}

void SelectBiblesGui::focus() { gtk_widget_grab_focus(treeview); }

void SelectBiblesGui::on_enabled_toggled(GtkCellRendererToggle *cell,
                                         gchar *path_str, gpointer data) {
  ((SelectBiblesGui *)data)->on_rendering_toggle(cell, path_str);
}

void SelectBiblesGui::on_rendering_toggle(GtkCellRendererToggle *cell,
                                          gchar *path_str) {
  GtkTreeModel *model = (GtkTreeModel *)store;
  GtkTreeIter iter;

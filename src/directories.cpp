...
// Have to have the temp directory to do almost anything...and may need to
// create it
// so we need mkdir, at least a temporary version of it.
#ifdef WIN32
  // mkdir (md) does not play nice. If you run it with /?, it returns code 1,
  // not 0. So
  // I rely on the #ifdef to just "know" that we have compiled on Windows and
  // can assume
...
#ifdef WIN32
  // Set path properly for Windows. We need this because tar -czf, for instance,
  // needs
...
void directories::find_mkdir(void) {
  ...
  //---------------------------------------------
  {
    ...
    GwSpawn spawn("mkdir");
    spawn.arg("--help"); // TODO: Something is messed up here. It creates
                         // Bibledit-x.y.z\--help as a directory!!!
    spawn.run();
    if (spawn.exitstatus == 0) {
      ...
      // Check for mkdir.exe in the rundir (Windows directly through msys2/mingw
      // binary)
  ...
  //---------------------------------------------
  {
    ...
    if (spawn.exitstatus == 0) {
      ...
      if (spawn.exitstatus == 0) {
        ...
        // Check for rm.exe in the rundir (Windows directly through msys2/mingw
        // binary)
...
void directories::find_utilities(void) {
  ...
  //---------------------------------------------
  {
    ...
    if (spawn.exitstatus == 0) {
      ...
      if (spawn.exitstatus == 0) {
        ...
        // Check for cp.exe in the rundir (Windows directly through msys2/mingw
        // binary)
  ...
  //---------------------------------------------
  {
    ...
    if (spawn.exitstatus == 0) {
      ...
      if (spawn.exitstatus == 0) {
        ...
        // Check for mv.exe in the rundir (Windows directly through msys2/mingw
        // binary)
  ...
  //---------------------------------------------
  {
    ...
    if (spawn.exitstatus == 0) {
      ...
      // Check for gzip.exe in the rundir (Windows directly through msys2/mingw
      // binary)
  ...
  //---------------------------------------------
  {
    ...
    if (spawn.exitstatus == 0) {
      ...
      // Check for tar.exe in the rundir (Windows directly through msys2/mingw
      // binary)
  ...
  //---------------------------------------------
  {
    ...
    if (spawn.exitstatus == 0) {
      ...
      // Check for diff.exe in the rundir (Windows directly through msys2/mingw
      // binary)
      GwSpawn spawn(rundir + "\\diff.exe");
      spawn.arg("--version");
      if (spawn.exitstatus == 0) {
        diff = rundir + "\\diff.exe";
      } else {
        gw_message(_("Cannot find a suitable diff utility"));
      }
    }
  }
  ...
  //---------------------------------------------
  {
    ...
    if (spawn.exitstatus == 0) {
      ...
      spawn.arg("--version"); // This is not standard for Windows, but in zip
                              // 3.0 by Info-ZIP that's what works
      if (spawn.exitstatus == 0) {
        zip = "zip.exe";
      } else {
        // Check for zip.exe in the rundir (Windows directly through msys2/mingw
        // binary)
  ...
  //---------------------------------------------
  {
    ...
    GwSpawn spawn("unzip");
    spawn.arg("--version"); // unzip.exe on Windows and Linux (on my systems at
                            // least) returns 10 even though it is present and
                            // runs --version because it doesn't know the
                            // --version flag
    spawn.run();
    if (spawn.exitstatus == 0) {
      ...
      spawn.arg("--version"); // This is not standard for Windows, but in unzip
                              // 6.0 by Info-ZIP that's what works
      if (spawn.exitstatus == 0) {
        unzip = "unzip.exe";
      } else {
        // Check for unzip.exe in the rundir (Windows directly through
        // msys2/mingw binary)
  ...
  //---------------------------------------------
  {
    ...
    if (spawn.exitstatus == 0) {
      ...
      spawn.arg("--version");
      if (spawn.exitstatus == 0) {
        gzip = rundir + "\\gzip.exe";
      } else {
        gw_message(_("Cannot find a suitable gzip utility"));
      }
    }
  }
...
void directories::find_pdfviewer(void) {
  ...
  struct {
    ...
    const char *argument;
      ...
      {"acroread", ""},   {"evince", ""}, {"xpdf", "-paper match"},
      {"ghostview", ""},  {"gpdf", ""},   {"kpdf", ""},
      {"kghostview", ""}, {"open", ""}};

  // This is designed to handle Windows (without bw outpost), as well as Unix,
  // Macintosh
  for (unsigned int i = 0; i < (sizeof(pdf_viewers) / sizeof(*pdf_viewers));
       i++) {
...
void directories::print() {
  ...
  gw_message("Any of these that are blank indicate that we have not worked on "
             "them yet!!!!!");
  gw_message("Git:         \t" + git);
...
void directories::check_structure() {
  ...
  // All these are in the user's home directory (~/.bibledit/... or
  // C:\users\username\.bibledit)

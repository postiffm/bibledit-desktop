...
// This is a wrapper for the recursive cp function on Unix, as Windows uses
// another one.
{
...
// This is a wrapper for the mv function on Unix, which is move on Windows.
{
  ...
  // To do: I don't think I need the force business...I have it in the args by
  // default.
  spawn.arg(from);
  spawn.arg(to);
  spawn.run();
...
// Can't vouch for the portability of this but it seems to work on Linux and
// Windows
void unix_unlink(const ustring &location) { unlink(location.c_str()); }

...
// Upgrades the git system.
{
  ...
  for (unsigned int i = 0; i < projects.size(); i++) {
    ...
    ustring git_directory =
        gw_build_filename(project_data_directory_project(projects[i]), ".git");
    if (projectconfig->git_use_remote_repository_get()) {
      // At times there's a stale index.lock file that prevents any
      // collaboration.
      ...
      command.append("\"");
      maintenance_register_shell_command(datadirectory, command);
      // (Re)initialize the repository. This can be done repeatedly without
      // harm.
      ...
      if (g_file_test(git_directory.c_str(), G_FILE_TEST_IS_DIR)) {
        ...
        ProgressWindow progresswindow(_("Tidying up project ") + projects[i],
                                      false);
...
void git_resolve_conflict_chapter(const ustring &project, unsigned int book,
                                  unsigned int chapter)
// This solves a conflicting chapter.
{
  // Log message
  gw_message(_("Resolving conflict for project ") + project + _(" book ") +
  ...
  /* Read the datafile. If there is a conflict it will look like the example
     below:
  ...
  for (unsigned int i = 0; i < rt.lines.size(); i++) {
    ...
    bool minemarker =
        rt.lines[i].find(git_mine_conflict_marker()) != string::npos;
  ...
  // Next time that a timed pull and push is done, the conflict will show up as
  // resolved.
}
...
void git_resolve_conflicts(const ustring &project,
                           const vector<ustring> &errors)
/*
 This function reads through the "errors" to see if any conflicting merge
 occurred.
 ...
 */
{
  /*
     ...
     The first time that a "git pull" is done, and there is a conflict, it says
     this:
  ...
  // Run a "git status" to find the books and chapters that have a merge
  // conflict.
  ...
  for (unsigned int i = 0; i < conflicted_chapters.size(); i++) {
    git_resolve_conflict_chapter(project, conflicted_chapters[i].book_get(),
                                 conflicted_chapters[i].chapter_get());
  }
...
void git_process_feedback(const ustring &project,
                          const vector<ustring> &feedback,
                          unsigned int watched_book,
                          unsigned int watched_chapter, bool &watch_updated) {
...
Some information is available about the secure git repository at
sourceforge.net.
...
To create a new repository, you need to access the Shell service, then follow
these steps:
   1. Navigate to your repository
         1. cd /home/scm_git/P/PR/PROJECTUNIXNAME
                * PROJECTUNIXNAME is the UNIX name of your project
                * P represents the first letter of that name, and PR the first
two letters of the name.
   2. Create a new directory with the name you want for the repository, eg mkdir
DIRNAME.
   3. Run git --git-dir=DIRNAME init --shared=all --bare (where DIRNAME
represents the name of the repository to be created)
          * This will initialize a new repository at that directory
Notes: Developers should not nest directories / repositories. Directories should
only be created the top level directory of repository.

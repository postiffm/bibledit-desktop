...
public:
  ...
  bool has_content();
  void print();
  void print(unsigned int page_number, bool print_date,
             const ustring &left_running_header,
             const ustring &right_running_header, bool suppress_header,
             unsigned int left_first_chapter, unsigned int left_last_chapter,
             unsigned int right_first_chapter, unsigned int right_last_chapter);
  ...
  void get_referents(map<ustring, ustring> &referents,
                     unsigned int page_number);
  void
  match_and_expand_references_and_referents(map<ustring, ustring> &referents);
...
private:
  ...
  T2PBigBlock
  get_next_big_block_to_be_kept_together(deque<T2PBlock *> &input_blocks,
                                         int column_count);
  int get_column_height(deque<T2PBigBlock> &column, int reference_y);
  void balance_columns(deque<T2PBigBlock> &first_column,
                       deque<T2PBigBlock> &last_column);
  int balance_last_column_higher_than_or_equal_to_first_column(
      deque<T2PBigBlock> &first_column, deque<T2PBigBlock> &last_column);
  int balance_first_column_higher_than_or_equal_to_last_column(
      deque<T2PBigBlock> &first_column, deque<T2PBigBlock> &last_column);
  deque<T2PLayoutContainer *> note_layout_containers;
  PangoRectangle get_next_free_note_rectangle();
  int get_note_height();
  ustring produce_running_header(const ustring &header, bool suppress,
                                 unsigned int first_chapter,
                                 unsigned int last_chapter);
  bool new_page_input_block_encountered(deque<T2PBlock *> &input_blocks,
                                        bool set_flag);
  bool start_new_page;

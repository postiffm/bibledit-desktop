...
Track::Track(unsigned int dummy) { clear(); }
...
bool Track::previous_reference_available() { return (reference_pointer > 0); }
...
void Track::get_next_references(vector<Reference> &references) {
  for (unsigned int i = reference_pointer + 1; i < recorded_references.size();
       i++) {
    references.push_back(recorded_references[i]);
  }
}

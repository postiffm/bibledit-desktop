...
ParallelBibleDialog::ParallelBibleDialog(int dummy) {
  ...
  gtk_builder_add_from_file(
      gtkbuilder, gw_build_filename(Directories->get_package_data(),
                                    "gtkbuilder.parallelbibledialog.xml")
                      .c_str(),
      NULL);
  ...
  label_main_project =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "label_main_project"));
  ...
  gtk_label_set_text(
      GTK_LABEL(label_book),
      books_id_to_english(settings->genconfig.book_get()).c_str());

  label_chapters =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "label_chapters"));
  vector<unsigned int> chapters_from, chapters_to;
  vector<ustring> verses_from, verses_to;
  select_portion_get_values(
      settings->genconfig.project_get(), settings->genconfig.book_get(),
      settings->genconfig.parallel_bible_chapters_verses_get(), chapters_from,
      verses_from, chapters_to, verses_to);
  ustring label;
  for (unsigned int i = 0; i < chapters_from.size(); i++) {
    if (i)
      label.append(" | ");
    label.append(select_portion_get_label(
        settings->genconfig.project_get(), settings->genconfig.book_get(),
  ...
  button_chapters =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "button_chapters"));
  g_signal_connect((gpointer)button_chapters, "clicked",
                   G_CALLBACK(on_button_chapters_clicked), gpointer(this));

  label_button_chapters =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "label_button_chapters"));
  shortcuts.label(label_button_chapters);

  hbox_select_bibles_gui =
      GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "hbox_select_bibles_gui"));
  ...
  checkbutton_keep_together = GTK_WIDGET(
      gtk_builder_get_object(gtkbuilder, "checkbutton_keep_together"));
  shortcuts.button(checkbutton_keep_together);
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_keep_together),
      settings->genconfig.parallel_bible_keep_verses_together_get());

  checkbutton_include_verse0 = GTK_WIDGET(
      gtk_builder_get_object(gtkbuilder, "checkbutton_include_verse0"));
  shortcuts.button(checkbutton_include_verse0);
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_include_verse0),
      settings->genconfig.parallel_bible_include_verse_zero_get());
...
int ParallelBibleDialog::run() { return gtk_dialog_run(GTK_DIALOG(dialog)); }

void ParallelBibleDialog::on_okbutton_clicked(GtkButton *button,
                                              gpointer user_data) {
...
void ParallelBibleDialog::on_okbutton() {
  ...
  settings->genconfig.parallel_bible_keep_verses_together_set(
      gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(checkbutton_keep_together)));
  settings->genconfig.parallel_bible_chapters_verses_set(
...
void ParallelBibleDialog::on_button_chapters_clicked(GtkButton *button,
                                                     gpointer user_data) {
...
void ParallelBibleDialog::on_button_chapters() {
  extern Settings *settings;
  SelectChaptersDialog dialog(settings->genconfig.project_get(),

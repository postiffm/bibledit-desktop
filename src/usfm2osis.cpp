...
Usfm2Osis::~Usfm2Osis() {
  ...
  g_file_set_contents(osisfile.c_str(), (const gchar *)xmlbuffer->content, -1,
                      NULL);
...
// Writes the header.
{
  ...
  xmlTextWriterWriteFormatAttribute(
      xmlwriter, BAD_CAST "xmlns",
      "http://www.bibletechnologies.net/2003/OSIS/namespace");
  xmlTextWriterWriteFormatAttribute(
      xmlwriter, BAD_CAST "xmlns:xsi",
      "http://www.w3.org/2001/XMLSchema-instance");
  xmlTextWriterWriteFormatAttribute(
      xmlwriter, BAD_CAST "xsi:schemaLocation",
      "http://www.bibletechnologies.net/2003/OSIS/namespace "
      "http://www.bibletechnolgologies.net/OSIS/osisCore.2.1.xsd");

  xmlTextWriterStartElement(xmlwriter, BAD_CAST "osisText");
  xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "osisIDWork", "%s",
                                    name.c_str());
  ...
  xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "osisWork", "%s",
                                    name.c_str());
  ...
  date_time_normal_get_year_month_day(date_time_julian_day_get_current(), year,
                                      month, day);
  xmlTextWriterWriteFormatString(
      xmlwriter,
      _("The document is free. It is copyrighted (C) %d by %s. Permission is "
        "granted to copy, distribute and/or modify the documents under the "
        "terms of the GNU Free Documentation License, Version 1.2 or any later "
        "version published by the Free Software Foundation; with no Invariant "
        "Sections, no Front-Cover Texts, and no Back-Cover Texts."),
      year, g_get_real_name());
  xmlTextWriterEndElement(xmlwriter);
...
void Usfm2Osis::open_book(unsigned int id) {
  ...
  xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "osisID", "%s",
                                    book_osis_id.c_str());
}
...
{
  // Variable that holds the usfm code that has not been handled in this
  // routine.
  ...
  while (usfm_code_available(usfm_code, usfmcode, 1000)) {
    marker_found = usfm_search_marker(usfm_code, marker_text, marker_position,
                                      marker_length, marker_is_opener);
    if (marker_found && (marker_position == 0)) {
      ...
      // There are several toc markers in USFM. This converter transforms it
      // into a "x-toc" description.
      ...
      // h2
      // title[@type="runningHead" and placement="inside" and @short='value of
      // h2 marker']
      else if (marker_text == "h2") {
        transform_h_title(usfm_code, marker_length, true, "inside");
      }
      // h3
      // title[@type="runningHead" and placement="outside" and @short='value of
      // h3 marker']
...
// An attempt is made to follow the OSIS manual, Appendix F, USFM to OSIS
// Mapping.
{
  ...
  while (!usfm_code.empty()) {
    ...
    if (marker_found && (marker_position == 0)) {
      ...
      // Markers h( 123) should not occur here, because these have been handled
      // earlier.
      // Markers toc(123) should not occur here, because these have been handled
      // earlier.
      ...
      // @canonical="false"]/title[@type="main"]/title[@level="1"]
      ...
      // imt2
      // div[@type="introduction" and
      // @canonical="false"]/title[@type="main"]/title[@level="2"]
      ...
      // imt3
      // div[@type="introduction" and
      // @canonical="false"]/title[@type="main"]/title[@level="3"]
      ...
      // imt4
      // div[@type="introduction" and
      // @canonical="false"]/title[@type="main"]/title[@level="4"]
      ...
      // is
      // div[@type="introduction" and
      // @canonical="false"]/div[@type="section"]/title
      ...
      // is2
      // div[@type="introduction" and
      // @canonical="false"]/div[@type="section"]/div[@type="subSection"]/title
      ...
      // (This is a format oriented marker --whitespace, but needs to be
      ...
      // iot
      // div[@type="introduction" and
      // @canonical="false"]//div[@type="outline"]/title
      ...
      // io1
      // div[@type="introduction" and
      // @canonical="false"]//div[@type="outline"]/list/item
      ...
      // io2
      // div[@type="introduction" and
      // @canonical="false"]//div[@type="outline"]/list/item/list/item
      ...
      // io3
      // div[@type="introduction" and
      // @canonical="false"]//div[@type="outline"]/list/item/list/item/list/item
      ...
      // io4
      // div[@type="introduction" and
      // @canonical="false"]//div[@type="outline"]/list/item/list/item/list/item/list/item
      ...
      // ior
      // div[@type="introduction" and
      // @canonical="false"]//div[@type="outline"]/list/item/reference
      ...
      // qr
      // l[@type="refrain"] and l[@type="attribution"] (choose qr based on
      // context)
      ...
      // note
        ...
        transform_note(usfm_code, marker_length, marker_is_opener, false,
                       false);
      }
      ...
      // note/reference[@type="source"]
        ...
        transform_character_style(usfm_code, marker_length, marker_is_opener,
      ...
      // note/rdg
        ...
        // but then the resulting OSIS file fails to validate against the
        // schema.
      ...
      // If for presentation purposes this may be encoded within
      // div[@type="publicationData"]
  ...
  // If \b immediately follows a line group, list, or table and is not
  // immediatelly followed by \s, \s#, \p, \ms, \ms#,
  // or \ps, then the \b is simply indicating that the line group, list, or
  // table is closing, OTHERWISE an explicit
  // line break must be encoded

  // \m immediately following a line group, list, or table closes the line
  // group, list, or table and continues the text
  // that follows in the already open container including a div element
  // OTHERWISE an explicit line break must
  // be inserted and the text that follows in the already open container
  // including div (if a \m immediately follows
  // a \b then an explicit line break is inserted)

  // \mi immediately following a line group, list, or table closes the line
  // group, list, or table and continues the text
  // that follows in the parent of the line group, list, or table if an ancestor
  // is q[@type="embeded] OTHERWISE
  // a q[@type='embedded'] is inserted into the parent of of the line group,
  // list, or table, OTHERWISE if a
  // q[type='embedded'] is an ancestor insert an explicit line break. If not
  // q[@type='embedded'] is an ancestor
  // then open one and continue the text that follows with no line break.

  // if the current container or one of its ancestors is a q[type='embeded']
  // then insert a explicit line break and
  // continue within the current container OTHERWISE open a q[@type='embeded']
  // and continue within it

  // \ib, \im, and \imi behave the same as \b, \m, and \mi but are within a
  // div[@type='introduction']

  // In addation and overriding the above, if a line break (
  // ) element would be encoded as the first node (element node or text node)
  // within a div, p, or q element so as
  // to represent a \m or \mi concept, the line break element is unnecessary and
  // should be ommited.

  // In all of the above rules regarding \b, \m, \mi, \imi, and \im a note is
  // ignored when determining immediately
...
// This transformation is called when no other transformation could be done.
{
  ...
  size_t marker_length;
  bool is_opener;
  bool marker_found = usfm_search_marker(usfm_code, marker, marker_position,
...
void Usfm2Osis::transform_usfm_description(ustring &usfm_code,
                                           const ustring &marker_text,
                                           size_t marker_length)
// According to the Osis manual, certain USFM codes translate into "description"
// elements in Osis.
{
  ustring text =
      get_erase_code_till_next_marker(usfm_code, 0, marker_length, true);
  ...
  xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "subType",
  subtype.c_str());
  xmlTextWriterWriteFormatString(xmlwriter, "%s", text.c_str());
  xmlTextWriterEndElement(xmlwriter);
  */
...
void Usfm2Osis::transform_h_title(ustring &usfm_code, size_t marker_length,
                                  bool runningheader, const gchar *placement)
// This transforms the \h into the Osis title element.
{
  ...
  xmlTextWriterStartElement(xmlwriter, BAD_CAST "title");
  if (runningheader) {
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "type",
                                      "runningHead");
  }
  if (placement) {
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "placement", "%s",
                                      placement);
  }
  xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "short", "%s",
                                    title.c_str());
  xmlTextWriterEndElement(xmlwriter);
}
...
// Returns true if the marker would result in an osis division.
{
  ...
  case uftChapter:
    return false;
  case uftChapterLabel:
    return false;
  case uftChapterMarker:
    return false;
  case uftVerse:
    return false;
  case uftVerseMarker:
...
// Opens a new division.
{
  ...
  xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "canonical", "%s",
                                    canonical_text.c_str());
}

void Usfm2Osis::transform_general_title(ustring &usfm_code,
                                        size_t marker_length, const gchar *type,
                                        unsigned int level)
...
void Usfm2Osis::transform_paragraph_start(ustring &usfm_code,
                                          size_t marker_length)
...
void Usfm2Osis::transform_remove_marker(ustring &usfm_code,
                                        size_t marker_length)
...
// Ensures that the chapter milestone is closed.
{
  ...
  if (!chapter_osis_id.empty()) {
    ...
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "eID", "%s",
                                      chapter_osis_id.c_str());
    xmlTextWriterEndElement(xmlwriter);
    chapter_osis_id.clear();
  }
}

void Usfm2Osis::transform_chapter_number(ustring &usfm_code,
                                         size_t marker_length) {
  // Extract the chapter number from the usfm code.
  usfm_code.erase(0, marker_length);
  ustring chapter_number =
      get_erase_code_till_next_marker(usfm_code, 0, 0, true);
  // Store the OSIS id.
  chapter_osis_id = book_osis_id + "." + chapter_number;
  // Write milestone, e.g.: <chapter sID="Titus.2" osisID="Titus.2" n="2"/>
  xmlTextWriterStartElement(xmlwriter, BAD_CAST "chapter");
  xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "sID", "%s",
                                    chapter_osis_id.c_str());
  xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "osisID", "%s",
                                    chapter_osis_id.c_str());
  xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "n", "%s",
                                    chapter_number.c_str());
  xmlTextWriterEndElement(xmlwriter);
}
...
void Usfm2Osis::transform_verse_number(ustring &usfm_code,
                                       size_t marker_length) {
...
void Usfm2Osis::transform_note(ustring &usfm_code, size_t marker_length,
                               bool is_opener, bool endnote, bool xref)
/*
This procedure only does the start of the note, and the end. All markers in
between
...
OSIS: <note osisRef="Gen.3.20" osisID="Gen.3.20!footnote.2"
n="2"><catchWord>Ê-va</catchWord> nghĩa là <q>sự sống.</q></note>
...
*/
{
  ...
  // Write the osisRef.
  xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "osisRef", "%s",
                                    verse_osis_id.c_str());
...
void Usfm2Osis::transform_character_style(ustring &usfm_code,
                                          size_t marker_length, bool is_opener,
                                          const gchar *element) {
  transform_character_style(usfm_code, marker_length, is_opener, element, NULL,
                            NULL);
}
...
void Usfm2Osis::transform_character_style(ustring &usfm_code,
  ...
  transform_character_style(usfm_code, marker_length, is_opener, element,
                            attribute_name, attribute_value, NULL, NULL);
}

void Usfm2Osis::transform_character_style(ustring &usfm_code,
                                          size_t marker_length, bool is_opener,
                                          const gchar *element,
                                          const gchar *attribute1_name,
                                          const gchar *attribute1_value,
                                          const gchar *attribute2_name,
                                          const gchar *attribute2_value) {
  // Remove the marker from the input stream.
  usfm_code.erase(0, marker_length);
  ...
  // If it happened to be a closing marker, we've done all what's needed: bail
  // out.
  if (!is_opener)
    return;
  ...
  if (attribute1_name) {
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST attribute1_name, "%s",
                                      attribute1_value);
  }

  // Optionally write the second attribute.
  if (attribute2_name) {
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST attribute2_name, "%s",
                                      attribute2_value);
  }
...
void Usfm2Osis::transform_figure(ustring &usfm_code, const ustring &marker_text,
                                 size_t marker_length) {
  ...
  ustring cap;
  ustring ref;
  usfm_dissect_figure(usfm_code, marker_text, marker_length, desc, file, size,
                      loc, copy, cap, ref);

  // Transform to osis.
  xmlTextWriterStartElement(xmlwriter, BAD_CAST "figure");
  if (!desc.empty()) {
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "desc", "%s",
                                      desc.c_str());
  }
  if (!file.empty()) {
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "file", "%s",
                                      file.c_str());
  }
  if (!size.empty()) {
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "size", "%s",
                                      size.c_str());
  }
  if (!loc.empty()) {
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "loc", "%s",
                                      loc.c_str());
  }
  if (!copy.empty()) {
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "copy", "%s",
                                      copy.c_str());
  }
  if (!cap.empty()) {
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "cap", "%s",
                                      cap.c_str());
  }
  if (!ref.empty()) {
    xmlTextWriterWriteFormatAttribute(xmlwriter, BAD_CAST "ref", "%s",
                                      ref.c_str());
  }
  xmlTextWriterEndElement(xmlwriter);
}

...
BookLocalization::BookLocalization(const ustring &language_in,
      ...
      while ((xmlTextReaderRead(reader) == 1)) {
        ...
        case XML_READER_TYPE_ELEMENT: {
          ...
          if (opening_element) {
            ...
            // Would like to free(opening_element) but it is used next loop
            // iteration
        ...
        case XML_READER_TYPE_TEXT: {
          ...
          if (opening_element && text) {
            ...
            opening_element =
                NULL; // now we can free it, since we are done with it
          }
          break;
        }
...
// Get all available localizations and store them in the object.
{
  if (available_localizations.empty()) {
    ...
    for (unsigned int i = 0; i < rf1.files.size(); i++) {
      ...
      available_localizations.push_back(
          filename_get_localization(rf1.files[i]));
    }
    ...
    for (unsigned int i = 0; i < rf2.files.size(); i++) {
      ...
      available_localizations.push_back(
          filename_get_localization(rf2.files[i]));
    }
    // Sort everything on name.
    quick_sort(available_localizations, available_filenames, 0,
               available_localizations.size());
  }
...
unsigned int BookLocalizations::name2id(const ustring &language,
                                        const ustring &name) {
  ustring namecf = name.casefold();
  unsigned int pointer = language_pointer_get(language);
  for (unsigned int i = 0;
       i < loaded_localizations[pointer].name_casefold.size(); i++) {
...
unsigned int BookLocalizations::abbrev2id(const ustring &language,
                                          const ustring &abbrev) {
  ustring abbrevcf = abbrev.casefold();
  unsigned int pointer = language_pointer_get(language);
  for (unsigned int i = 0;
       i < loaded_localizations[pointer].abbreviation_casefold.size(); i++) {
...
unsigned int BookLocalizations::abbrev2id_loose(const ustring &language,
                                                const ustring &abbrev) {
  ustring abbrevcf = abbrev.casefold();
  ...
  for (unsigned int i = 0;
       i < loaded_localizations[pointer].abbreviation_casefold.size(); i++) {
    if (loaded_localizations[pointer].abbreviation_casefold[i].find(abbrevcf) ==
        0)
      return loaded_localizations[pointer].id[i];
  }
  return 0;
...
// Returns the name of the file that contains the general adapted book names.
{
  ustring filename;
  filename =
      gw_build_filename(Directories->get_configuration(), "adapted_booknames");
  return filename;
}
...
map<ustring, unsigned int>
general_adapted_booknames_fill_up(map<unsigned int, ustring> &mapping)

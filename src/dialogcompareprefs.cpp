...
ComparePreferencesDialog::ComparePreferencesDialog(int dummy) {
  ...
  gtk_window_set_position(GTK_WINDOW(comparepreferencesdialog),
                          GTK_WIN_POS_CENTER_ON_PARENT);
  gtk_window_set_modal(GTK_WINDOW(comparepreferencesdialog), TRUE);

  dialog_vbox1 =
      gtk_dialog_get_content_area(GTK_DIALOG(comparepreferencesdialog));
  gtk_widget_show(dialog_vbox1);

  checkbutton_disregard_notes =
      gtk_check_button_new_with_mnemonic(_("Disregard changes in notes"));
  gtk_widget_show(checkbutton_disregard_notes);
  gtk_box_pack_start(GTK_BOX(dialog_vbox1), checkbutton_disregard_notes, FALSE,
                     FALSE, 0);

  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(comparepreferencesdialog));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(comparepreferencesdialog),
                               cancelbutton, GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(comparepreferencesdialog), okbutton,
                               GTK_RESPONSE_OK);
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_disregard_notes),
      settings->genconfig.compare_disregard_notes_get());
}
...
void ComparePreferencesDialog::on_okbutton_clicked(GtkButton *button,
                                                   gpointer user_data) {
...
void ComparePreferencesDialog::on_okbutton() {
  extern Settings *settings;
  settings->genconfig.compare_disregard_notes_set(gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(checkbutton_disregard_notes)));
}

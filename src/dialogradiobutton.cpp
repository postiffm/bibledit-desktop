...
RadiobuttonDialog::RadiobuttonDialog(const ustring &title, const ustring &info,
                                     const vector<ustring> &labels,
                                     unsigned int selection, bool autoscale) {
  Shortcuts shortcuts(0);

  radiobuttondialog = gtk_dialog_new();
  gtk_window_set_title(GTK_WINDOW(radiobuttondialog), title.c_str());
  gtk_window_set_position(GTK_WINDOW(radiobuttondialog),
  ...
  for (unsigned int i = 0; i < labels.size(); i++) {
    ...
    g_signal_connect((gpointer)radiobutton, "button_press_event",
                     G_CALLBACK(on_radiobutton_button_press_event),
                     gpointer(this));
  }

  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(radiobuttondialog));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(radiobuttondialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(radiobuttondialog), okbutton,
                               GTK_RESPONSE_OK);
...
gboolean RadiobuttonDialog::on_radiobutton_button_press_event(
...
void RadiobuttonDialog::on_okbutton_clicked(GtkButton *button,
                                            gpointer user_data) {

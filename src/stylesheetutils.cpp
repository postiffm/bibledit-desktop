...
// This returns the name of the database that contains data for recently used
// styles.
{
...
{
  return gw_build_filename(Directories->get_stylesheets(),
                           name + STYLESHEET_XML_SUFFIX);
}
...
// Gets the names of the stylesheets that are there.
{
  ...
  for (unsigned int i = 0; i < rf.files.size(); i++) {
    ...
    name.erase(name.length() - strlen(STYLESHEET_XML_SUFFIX),
               strlen(STYLESHEET_XML_SUFFIX));
    if (!name.empty()) {
...
const char *stylesheet_basic[] = {"id", "h", "mt1", "c",  "s1",
                                  "r",  "p", "v",   "q1", "m"};
const char *stylesheet_paragraph[] = {"rem", "mt2", "imt", "ip",
                                      "ms",  "mr",  "nb"};
const char *stylesheet_word_note[] = {"qt", "nd", "f", "fr", "ft", "fq"};

const char *stylesheet_sil_best_practice[] = {
    "c",       "p",    "r",    "s",    "v",     "f",    "fk",   "fq",
    "fr",      "ft",   "fv",   "h",    "id",    "mt",   "mt2",  "mt3",
    "b",       "d",    "m",    "q",    "q2",    "q3",   "qa",   "qc",
    "qm",      "qm2",  "qm3",  "qr",   "qs",    "qt",   "x",    "xk",
    "xo",      "xq",   "xt",   "ie",   "im",    "imq",  "imt",  "imt2",
    "imt3",    "io",   "io2",  "io3",  "ior",   "ip",   "ipr",  "iq",
    "iq2",     "is",   "is2",  "cov",  "intro", "pref", "pub",  "pubinfo",
    "spine",   "toc",  "toc1", "toc2", "toc3",  "conc", "glo",  "idx",
    "k",       "maps", "w",    "ca",   "cl",    "fig",  "h1",   "h2",
    "h3",      "ide",  "lit",  "mi",   "mr",    "ms",   "nb",   "pc",
    "pi1",     "pi2",  "pm",   "pmc",  "pmo",   "pmr",  "ps",   "rem",
    "restore", "rq",   "s2",   "sp",   "va",    "li",   "li2",  "li3",
    "li4",     "tc1",  "tc2",  "tc3",  "tc4",   "tcr1", "tcr2", "tcr3",
    "tcr4",    "th1",  "th2",  "th3",  "th4",   "thr1", "thr2", "thr3",
    "thr4",    "tr",   "add",  "bk",   "cls",   "nd",   "ord",  "sig",
    "tl"};
...
// It expects a file in the format as it is given in the export function.
{
  ...
  for (unsigned int i = 0;
       i < (sizeof(RECOGNIZED_SUFFIXES) / sizeof(*RECOGNIZED_SUFFIXES)); i++) {
    if (g_str_has_suffix(filename.c_str(), RECOGNIZED_SUFFIXES[i])) {
      name = gw_path_get_basename(filename);
      name.erase(name.length() - strlen(RECOGNIZED_SUFFIXES[i]),
                 strlen(RECOGNIZED_SUFFIXES[i]));
    }
  }
  if (name.empty()) {
...
vector<ustring> stylesheet_get_markers(const ustring &stylesheet,
...
void stylesheet_save_style(
    const ustring &stylesheet, const ustring &marker, const ustring &name,
    const ustring &info, StyleType type, int subtype, double fontsize,
    const ustring &italic, const ustring &bold, const ustring &underline,
    const ustring &smallcaps, bool superscript, const ustring &justification,
    double spacebefore, double spaceafter, double leftmargin,
    double rightmargin, double firstlineindent, bool spancolumns,
    unsigned int color, bool print, bool userbool1, bool userbool2,
    bool userbool3, int userint1, int userint2, int userint3,
...
int stylesheet_style_get_pointer(const vector<Style> &styles,
...
void stylesheet_upgrade_value(vector<ustring> &sheet, const ustring &marker,
                              const ustring &element, const ustring &value)
    ...
    if (within_marker) {
      ...
      if (startpos != string::npos) {
        ...
        if (endpos != string::npos) {
          line.erase(startpos + element_opener.length(),
                     endpos - startpos - element_opener.length());
...
// Upgrade older stylesheets to the currently used format.
{
  // Make the \r marker a section heading instead of a normal paragraph.
  {
    ...
    for (unsigned int i = 0; i < rf.files.size(); i++) {
      ustring filename =
          gw_build_filename(Directories->get_stylesheets(), rf.files[i]);
...
void stylesheet_get_recently_used(const ustring &stylesheet,
                                  vector<ustring> &markers,
...
// Read the recently used data: markers and usage count.
{
  ...
  char *sql;
  sql = g_strdup_printf("select marker, count from '%s';",
                        double_apostrophy(stylesheet).c_str());
...
void stylesheet_set_recently_used(const ustring &stylesheet,
  ...
  sql = g_strdup_printf("delete from '%s';",
                        double_apostrophy(stylesheet).c_str());
  int rc = sqlite3_exec(db, sql, NULL, NULL, NULL);
  g_free(sql);
  // If deleting the markers failed, create the table for the stylesheet.
  if (rc != SQLITE_OK) {
    sql = g_strdup_printf("create table '%s' (marker string, count integer);",
                          double_apostrophy(stylesheet).c_str());
  ...
  for (unsigned int i = 0; i < styles.size(); i++) {
    sql = g_strdup_printf("insert into '%s' values ('%s', %d);",
                          double_apostrophy(stylesheet).c_str(),
                          styles[i].c_str(), counts[i]);
    sqlite3_exec(db, sql, NULL, NULL, NULL);
    g_free(sql);
  }
...
/*
Reads one Style.
stylesheet: which stylesheet to read from. If no stylesheet is given, it reads
from the template.
...
set<ustring> stylesheet_get_styles_of_type(StylesheetType stylesheettype) {
  ...
  case stBasicParagraphWordNote: {
    ...
    for (unsigned int i = 0;
         i < (sizeof(stylesheet_word_note) / sizeof(*stylesheet_word_note));
         i++) {
  ...
  case stBasicParagraph: {
    // Store the markes that are in, and fall through to the next type.
    for (unsigned int i = 0;
         i < (sizeof(stylesheet_paragraph) / sizeof(*stylesheet_paragraph));
         i++) {
  ...
  case stBasic: {
    ...
    for (unsigned int i = 0;
         i < (sizeof(stylesheet_basic) / sizeof(*stylesheet_basic)); i++) {
  ...
  case stSilBestPractice: {
    // Store the markers that are in.
    for (unsigned int i = 0; i < (sizeof(stylesheet_sil_best_practice) /
                                  sizeof(*stylesheet_sil_best_practice));
         i++) {
...
{
  // TO DO: This routine is called A LOT. Why not just cache the result as
  // currStylesheet

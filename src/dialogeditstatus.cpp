...
EditStatusDialog::EditStatusDialog(const ustring &project_in, unsigned int book,
  ...
  gtk_window_set_position(GTK_WINDOW(editstatusdialog),
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolledwindow_books),
                                 GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
  gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(scrolledwindow_books),
                                      GTK_SHADOW_IN);
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview_books),
                          GTK_TREE_MODEL(liststore_books));
  g_object_unref(liststore_books);
  GtkCellRenderer *renderer_books = gtk_cell_renderer_text_new();
  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(treeview_books), -1,
                                              _("Books"), renderer_books,
                                              "text", 0, NULL);
  gtk_tree_selection_set_mode(
      gtk_tree_view_get_selection(GTK_TREE_VIEW(treeview_books)),
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolledwindow_chapters),
                                 GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
  gtk_scrolled_window_set_shadow_type(
      GTK_SCROLLED_WINDOW(scrolledwindow_chapters), GTK_SHADOW_IN);
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview_chapters),
                          GTK_TREE_MODEL(liststore_chapters));
  g_object_unref(liststore_chapters);
  GtkCellRenderer *renderer_chapters = gtk_cell_renderer_text_new();
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(treeview_chapters), -1, _("Chapters"), renderer_chapters,
      "text", 0, NULL);
  gtk_tree_selection_set_mode(
      gtk_tree_view_get_selection(GTK_TREE_VIEW(treeview_chapters)),
      GTK_SELECTION_MULTIPLE);
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolledwindow_verses),
                                 GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
  gtk_scrolled_window_set_shadow_type(
      GTK_SCROLLED_WINDOW(scrolledwindow_verses), GTK_SHADOW_IN);
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview_verses),
                          GTK_TREE_MODEL(liststore_verses));
  g_object_unref(liststore_verses);
  GtkCellRenderer *renderer_verses = gtk_cell_renderer_text_new();
  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(treeview_verses),
                                              -1, _("Verses"), renderer_verses,
                                              "text", 0, NULL);
  gtk_tree_selection_set_mode(
      gtk_tree_view_get_selection(GTK_TREE_VIEW(treeview_verses)),
      GTK_SELECTION_MULTIPLE);
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolledwindow_status),
                                 GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
  gtk_scrolled_window_set_shadow_type(
      GTK_SCROLLED_WINDOW(scrolledwindow_status), GTK_SHADOW_IN);
  ...
  for (unsigned int i = 0; i < alltasks.size(); i++) {
    ...
    checkbutton_status =
        gtk_check_button_new_with_mnemonic(alltasks[i].c_str());
    gtk_widget_show(checkbutton_status);
    gtk_box_pack_start(GTK_BOX(vbox_status), checkbutton_status, FALSE, FALSE,
                       0);

    g_signal_connect((gpointer)checkbutton_status, "toggled",
                     G_CALLBACK(on_checkbutton_status_toggled), gpointer(this));

    statusbuttons.push_back(checkbutton_status);
  }

  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(editstatusdialog));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(editstatusdialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(editstatusdialog), okbutton,
                               GTK_RESPONSE_OK);
...
gboolean EditStatusDialog::on_treeview_books_button_event(GtkWidget *widget,
...
gboolean EditStatusDialog::on_treeview_books_key_event(GtkWidget *widget,
                                                       GdkEventKey *event,
                                                       gpointer user_data) {
  ((EditStatusDialog *)user_data)->on_treeview_books_cursor();
  return FALSE;
...
void EditStatusDialog::load_chapters() {
  ...
  if (currentbooks.size() == 1) {
    ProjectStatusBook *project_status_book =
        projectstatus->books.at(currentbooks[0]);
    for (unsigned int i = 0; i < project_status_book->chapters.size(); i++) {
      ProjectStatusChapter *project_status_chapter =
          project_status_book->chapters.at(i);
...
gboolean EditStatusDialog::on_treeview_chapters_button_event(
    GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
  ((EditStatusDialog *)user_data)->on_treeview_chapters_cursor();
  return FALSE;
}

gboolean EditStatusDialog::on_treeview_chapters_key_event(GtkWidget *widget,
                                                          GdkEventKey *event,
                                                          gpointer user_data) {
  ((EditStatusDialog *)user_data)->on_treeview_chapters_cursor();
  return FALSE;
...
gboolean EditStatusDialog::on_treeview_verses_button_event(
    GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
  ((EditStatusDialog *)user_data)->on_treeview_verses_cursor();
  return FALSE;
}

gboolean EditStatusDialog::on_treeview_verses_key_event(GtkWidget *widget,
                                                        GdkEventKey *event,
                                                        gpointer user_data) {
  ((EditStatusDialog *)user_data)->on_treeview_verses_cursor();
  return FALSE;
...
// Gets the type of status to be edited.
{
  ...
  // Below this one book and one chapter have been selected, and we deal with
  // the verses.
...
// Sets the status.
{
  ...
  case estNone: {
    for (unsigned int i = 0; i < statusbuttons.size(); i++) {
      ...
      gtk_toggle_button_set_inconsistent(GTK_TOGGLE_BUTTON(statusbuttons[i]),
                                         false);
    }
    break;
  }
  ...
  case estBooks: {
    ...
    for (unsigned int i = 0; i < currentbooks.size(); i++) {
      ProjectStatusBook *project_status_book =
          projectstatus->books.at(currentbooks[i]);
      ProjectStatusRecord statusrecord = project_status_book->get();
      reporting_merge_child_status_into_parent(alltasks_size, i, combinedrecord,
                                               statusrecord);
    }
  ...
  case estChapter: {
    ProjectStatusBook *project_status_book =
        projectstatus->books.at(currentbooks[0]);
    ProjectStatusChapter *project_status_chapter =
        project_status_book->chapters.at(currentchapters[0]);
  ...
  case estChapters: {
    ...
    for (unsigned int i = 0; i < currentchapters.size(); i++) {
      ProjectStatusBook *project_status_book =
          projectstatus->books.at(currentbooks[0]);
      ProjectStatusChapter *project_status_chapter =
          project_status_book->chapters.at(currentchapters[i]);
      ProjectStatusRecord statusrecord = project_status_chapter->get();
      reporting_merge_child_status_into_parent(alltasks_size, i, combinedrecord,
                                               statusrecord);
    }
...
void EditStatusDialog::on_checkbutton_status_toggled(
...
void EditStatusDialog::on_okbutton_clicked(GtkButton *button,
                                           gpointer user_data) {

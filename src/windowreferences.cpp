...
WindowReferences::WindowReferences(GtkWidget *parent_layout,
                                   GtkAccelGroup *accelerator_group,
                                   bool startup,
                                   bool reference_management_enabled)
    : FloatingWindow(parent_layout, widReferences, _("References"), startup),
      reference(0, 0, "")
// Window for showing the quick references.
{
  ...
  g_signal_connect((gpointer)webview, "navigation-policy-decision-requested",
                   G_CALLBACK(on_navigation_policy_decision_requested),
                   gpointer(this));
  DEBUG("30")
  // Signal button.
...
void WindowReferences::set(vector<Reference> &refs, const ustring &project_in,
...
// project: project and language for the references.
{
  ...
  std::set<ustring> hidden_references_set(hidden_references.begin(),
                                          hidden_references.end());
...
void WindowReferences::open() {
  ...
  extern Settings *settings;
  // Ask for a file.
  ustring filename = gtkw_file_chooser_open(
      vbox_client, _("Open File"), settings->genconfig.references_file_get());
  if (filename.empty())
    return;
  ...
  Entry3Dialog dialog2(_("Search for"), true,
                       _("Optionally enter _1st searchword"), searchword1,
                       _("Optionally enter _2nd searchword"), searchword2,
                       _("Optionally enter _3rd searchword"), searchword3);
    ...
    if (import_references_searchwords.size() > 0) {
      ...
      for (unsigned int i = 0; i < import_references_searchwords.size(); i++) {
        SessionHighlights sessionhighlights(
            import_references_searchwords[i], false, false, false, false, atRaw,
...
// Loads references from database.
{
  ...
  if (!g_file_test(references_database_filename().c_str(),
  ...
  try {
    ...
    // Read the references.
    {
      ...
      DEBUG("20...sqlitereader.elements=" +
            std::to_string(sqlitereader.ustring0.size()))
      for (unsigned int i = 0; i < sqlitereader.ustring0.size(); i++) {
        Reference reference(convert_to_int(sqlitereader.ustring0[i]),
                            convert_to_int(sqlitereader.ustring1[i]),
                            sqlitereader.ustring2[i]);
    ...
    // Read the searchwords.
    {
      ...
      SqliteReader sqlitereader(0);
      char *sql;
      sql = g_strdup_printf("select word, casesensitive, glob, matchbegin, "
                            "matchend, areatype, areaid, areaintro, "
                            "areaheading, areachapter, areastudy, areanotes, "
                            "areaxref, areaverse from highlights;");
      rc = sqlite3_exec(db, sql, sqlitereader.callback, &sqlitereader, &error);
      g_free(sql);
      if (rc != SQLITE_OK) {
        throw runtime_error(error);
      ...
      for (unsigned int i = 0; i < sqlitereader.ustring0.size(); i++) {
        SessionHighlights sessionhighlights(
            sqlitereader.ustring0[i], convert_to_bool(sqlitereader.ustring1[i]),
...
void WindowReferences::save() {
  ...
  try {
    ...
    char *sql;
    sql = g_strdup_printf("create table refs (book integer, chapter integer, "
    ...
    // Use sqlite prepared statement to really make it faster...less conversions
    // to strings, less sql compile time
    ...
    for (unsigned int i = 0; i < references.size(); i++) {
      ...
      sqlite3_bind_text(stmt, 4, comment.c_str(), comment.size(),
                        SQLITE_STATIC);
      if (sqlite3_step(stmt) != SQLITE_DONE) {
        throw runtime_error(sqlite3_errmsg(db));
      }
      sqlite3_reset(stmt);
      // WAS sql = g_strdup_printf("insert into refs values (%d, %d, '%s',
      // '%s')", references[i].book_get(), references[i].chapter_get(),
      // references[i].verse_get().c_str(),
      // double_apostrophy(comments[i]).c_str());
    ...
    sql = g_strdup_printf("create table highlights (word text, casesensitive "
                          "integer, glob integer, matchbegin integer, matchend "
                          "integer, areatype integer, areaid integer, "
                          "areaintro integer, areaheading integer, areachapter "
                          "integer, areastudy integer, areanotes integer, "
                          "areaxref integer, areaverse integer);");
    rc = sqlite3_exec(db, sql, NULL, NULL, &error);
    g_free(sql);
    if (rc) {
      throw runtime_error(sqlite3_errmsg(db));
    ...
    extern Settings *settings;
    DEBUG("Step 40...highlights.size=" +
          std::to_string(settings->session.highlights.size()))
    for (unsigned int i = 0; i < settings->session.highlights.size(); i++) {
      sql = g_strdup_printf(
          "insert into highlights values ('%s', %d, %d, %d, %d, %d, %d, %d, "
          "%d, %d, %d, %d, %d, %d)",
...
gboolean WindowReferences::on_navigation_policy_decision_requested(
  ...
  ((WindowReferences *)user_data)
      ->navigation_policy_decision_requested(request, navigation_action,
                                             policy_decision);
  return true;
}

void WindowReferences::navigation_policy_decision_requested(
    WebKitNetworkRequest *request, WebKitWebNavigationAction *navigation_action,
...
void WindowReferences::load_webview(const gchar *url) {
  ...
  if (active_url.find("goto ") == 0) {
    ...
    vector<ustring> hidden_references = references_hidden_ones_load(project);
    EditListDialog dialog(&hidden_references, _("Hidden references"),
                          _("of references and comments that will never be "
                            "shown in the reference area."),
...
void WindowReferences::html_write_references(HtmlWriter2 &htmlwriter) {
  ...
  for (unsigned int i = 0; i < settings->session.highlights.size(); i++) {
    ...
    wholewords.push_back(settings->session.highlights[i].matchbegin &&
  ...
  for (unsigned int i = lower_boundary; i < upper_boundary; i++) {
    ...
    if (settings->genconfig.reference_window_show_verse_text_get()) {
      ...
      searchwords_find_fast(text, searchwords, wholewords, casesensitives,
                            startpositions, lengths);
      size_t processposition = 0;
      if (settings->genconfig.reference_window_show_relevant_bits_get()) {
        // Show a few words before the search word, and a few after, not the
        // whole text.
        ...
        for (unsigned int i = 0; i < startpositions.size(); i++) {
          ...
          unsigned int min = 0;
          if (parsewords1.words.size() > 2)
            min = parsewords1.words.size() - 2;
          ...
          ParseWords parsewords2(
              text.substr(startpositions[i] + lengths[i], 1000));
        ...
        for (unsigned int i = 0; i < startpositions.size(); i++) {
          htmlwriter.text_add(
              text.substr(0, startpositions[i] - processposition));
          htmlwriter.bold_open();
          htmlwriter.text_add(
              text.substr(startpositions[i] - processposition, lengths[i]));
          htmlwriter.bold_close();
          text.erase(0, startpositions[i] - processposition + lengths[i]);
          processposition = startpositions[i] + lengths[i];
        }
        // Add whatever is left over. This could be the full text in case it
        // wasn't processed.
...
void WindowReferences::html_write_action_bar(HtmlWriter2 &htmlwriter,
                                             bool topbar) {
  // If there are no references, don't write an action bar at the bottom, only
  // at the top.
  if (!topbar) {
    if (references.empty()) {
      return;
  ...
  if (!references.empty()) {
    ...
    htmlwriter.text_add(
        "Items " + convert_to_string((unsigned int)(lower_boundary + 1)) +
        " - " + convert_to_string((unsigned int)upper_boundary) + " of " +
        convert_to_string((unsigned int)references.size()));
    if (upper_boundary < references.size()) {
      htmlwriter.text_add(" ");
...
void WindowReferences::html_write_action_page(HtmlWriter2 &htmlwriter) {
  ...
  if (active_entry >= 0) {
    ...
    htmlwriter.hyperlink_add(
        "dismiss cursor",
        "Dismiss " + references[active_entry].human_readable(language));
    htmlwriter.paragraph_close();
  }
  ...
  if (upper_boundary > lower_boundary) {
    htmlwriter.paragraph_open();
    htmlwriter.hyperlink_add(
        "dismiss page", "Dismiss the whole page of " +
                            convert_to_string(upper_boundary - lower_boundary) +
                            " references");
    htmlwriter.paragraph_close();
  }
  // If there are any references at all, offer the option to dismiss the whole
  // lot.
  if (!references.empty()) {
    htmlwriter.paragraph_open();
    htmlwriter.hyperlink_add(
        "dismiss all", "Dismiss the whole lot of " +
                           convert_to_string((unsigned int)references.size()) +
                           " references");
    htmlwriter.paragraph_close();
  }
  ...
  if ((active_entry >= 0) && references_management_on) {
    htmlwriter.paragraph_open();
    htmlwriter.hyperlink_add("hide", "Hide \"" + hide_string(active_entry) +
                                         "\" from now on");
    htmlwriter.paragraph_close();
  }
  // Manage the hidden references.
...
void WindowReferences::goto_next_previous_internal(bool next) {
  ...
  // Switch pages back till the reference to be selected is within the visible
  // bounds.
  while (selection < (int)lower_boundary) {
    load_webview("prev");
  }

  // Switch pages forward till the references to be selected is within the
  // visible bounds.

...
NotesDialog::NotesDialog(int dummy) {
  ...
  gtk_window_set_position(GTK_WINDOW(notesdialog),
  ...
  label10 = gtk_label_new(
      _("While editing a project note, there are some categories that can be "
        "assigned to this note.\nThe list of categories can be edited here."));
  ...
  new InDialogHelp(notesdialog, NULL, NULL,
                   (gchar *)"menu-preferences/project-notes");

  cancelbutton1 = gtk_button_new_from_stock("gtk-cancel");
  gtk_widget_show(cancelbutton1);
  gtk_dialog_add_action_widget(GTK_DIALOG(notesdialog), cancelbutton1,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton1), true);

  okbutton1 = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton1);
  gtk_dialog_add_action_widget(GTK_DIALOG(notesdialog), okbutton1,
                               GTK_RESPONSE_OK);
  ...
  g_signal_connect((gpointer)buttondelete, "clicked",
                   G_CALLBACK(on_buttondelete_clicked), gpointer(this));
  g_signal_connect((gpointer)buttonadd, "clicked",
                   G_CALLBACK(on_buttonadd_clicked), gpointer(this));
...
NotesDialog::~NotesDialog() { gtk_widget_destroy(notesdialog); }

int NotesDialog::run() { return gtk_dialog_run(GTK_DIALOG(notesdialog)); }
...
void NotesDialog::on_buttondelete_clicked(GtkButton *button,
                                          gpointer user_data) {
...
void NotesDialog::on_delete_category() {
  ...
  set<ustring> categories_in_db(categories_in_database.begin(),
                                categories_in_database.end());
  ...
  if (found == categories_in_db.end()) {
    ...
    ustring message = _("This category could not be deleted, because there are "
...
void NotesDialog::set_gui(const vector<ustring> &categories,
                          const ustring &display_value) {

...
// void pdfviewer_run(const gchar * command, const gchar * argument, const
// ustring & filename)
void pdfviewer_run(const ustring &command, const ustring &argument,
...
void pdfviewer_view(const ustring &filename) {
  ...
  if (!settings->genconfig.pdf_viewer_automatic_get()) {
    if (gw_find_program_in_path(
            settings->genconfig.pdf_viewer_path_get().c_str())) {
      // pdfviewer_run((gchar *)
      // settings->genconfig.pdf_viewer_path_get().c_str(), (gchar *)
      // settings->genconfig.pdf_viewer_arguments_get().c_str(), filename);
      pdfviewer_run(settings->genconfig.pdf_viewer_path_get(),
                    settings->genconfig.pdf_viewer_arguments_get(), filename);
      return;
    }
  }
  // Use directories container that has figured out a PDF viewer in the path
  if (Directories->get_pdfviewer() != "") {
    pdfviewer_run(Directories->get_pdfviewer(),
                  Directories->get_pdfviewer_args(), filename);
    return;
  }
#ifdef WIN32
  else {
    ...
    ustring message = _("The file cannot be displayed, because there is no "
                        "suitable pdf viewer installed.\n");
    message.append(_("Install a supported pdf viewer, or set up your own under "

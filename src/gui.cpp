...
// Sets image and text in the GUI whether things are okay or not.
{
  if (ok) {
    ...
    gtk_image_set_from_stock(GTK_IMAGE(image), "gtk-dialog-warning",
                             GTK_ICON_SIZE_BUTTON);
    gtk_label_set_text(GTK_LABEL(label), _("To do:"));
  }
}

bool gui_double_question(GtkWidget *parent, ustring question1,
                         ustring question2)
...
*/
{
  ...
  /*
     ...
     But it was preferrable that e.g. "Old Testament" would be given, not only
     when
     the whole OT was there, but also when all OT books that are currently in
     the
     project were there. E.g. if the project contains Genesis and Exodus, and
     both

...
void MainWindow::on_view_bibles ()
{
  if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(view_bibles))) {
    ...
    if (!window_bibles) { window_bibles = new TabbedWindow("Bibles", layout, accelerator_group, windows_startup_pointer != G_MAXINT); }
...
void MainWindow::on_view_concordance ()
{
  if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(view_concordance))) {
	// This tabbed window may already be created, so it need not be created again
    if (!window_info) { window_info = new TabbedWindow("Information", layout, accelerator_group, windows_startup_pointer != G_MAXINT); }

...
SelectChaptersDialog::SelectChaptersDialog(const ustring &project,
                                           unsigned int book,
                                           const ustring &currentselection) {
  ...
  gtk_window_set_position(GTK_WINDOW(selectchaptersdialog),
  ...
  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(selectchaptersdialog));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(selectchaptersdialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(selectchaptersdialog), okbutton,
                               GTK_RESPONSE_OK);
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeviewchapterfrom),
                          GTK_TREE_MODEL(liststorechapterfrom));
  g_object_unref(liststorechapterfrom);
  GtkCellRenderer *rendererchapterfrom = gtk_cell_renderer_text_new();
  columnchapterfrom = gtk_tree_view_column_new_with_attributes(
      "", rendererchapterfrom, "text", 0, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeviewchapterfrom),
                              columnchapterfrom);
  selectchapterfrom =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(treeviewchapterfrom));
  gtk_tree_selection_set_mode(selectchapterfrom, GTK_SELECTION_SINGLE);

  liststoreversefrom = gtk_list_store_new(1, G_TYPE_STRING);
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeviewversefrom),
                          GTK_TREE_MODEL(liststoreversefrom));
  g_object_unref(liststoreversefrom);
  GtkCellRenderer *rendererversefrom = gtk_cell_renderer_text_new();
  columnversefrom = gtk_tree_view_column_new_with_attributes(
      "", rendererversefrom, "text", 0, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeviewversefrom),
                              columnversefrom);
  selectversefrom =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(treeviewversefrom));
  gtk_tree_selection_set_mode(selectversefrom, GTK_SELECTION_SINGLE);

  liststorechapterto = gtk_list_store_new(1, G_TYPE_STRING);
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeviewchapterto),
                          GTK_TREE_MODEL(liststorechapterto));
  g_object_unref(liststorechapterto);
  GtkCellRenderer *rendererchapterto = gtk_cell_renderer_text_new();
  columnchapterto = gtk_tree_view_column_new_with_attributes(
      "", rendererchapterto, "text", 0, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeviewchapterto),
                              columnchapterto);
  selectchapterto =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(treeviewchapterto));
  gtk_tree_selection_set_mode(selectchapterto, GTK_SELECTION_SINGLE);

  liststoreverseto = gtk_list_store_new(1, G_TYPE_STRING);
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeviewverseto),
                          GTK_TREE_MODEL(liststoreverseto));
  g_object_unref(liststoreverseto);
  GtkCellRenderer *rendererverseto = gtk_cell_renderer_text_new();
  columnverseto = gtk_tree_view_column_new_with_attributes("", rendererverseto,
                                                           "text", 0, NULL);
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeviewportions),
                          GTK_TREE_MODEL(liststoreportions));
  g_object_unref(liststoreportions);
  GtkCellRenderer *rendererportions = gtk_cell_renderer_text_new();
  columnportions = gtk_tree_view_column_new_with_attributes(
      "", rendererportions, "text", 0, NULL);
  ...
  g_signal_connect((gpointer)buttonall, "clicked",
                   G_CALLBACK(on_buttonall_clicked), gpointer(this));
  g_signal_connect((gpointer)treeviewportions, "cursor_changed",
                   G_CALLBACK(on_treeview1_cursor_changed), gpointer(this));
  g_signal_connect((gpointer)treeviewportions, "move_cursor",
                   G_CALLBACK(on_treeview1_move_cursor), gpointer(this));
...
void SelectChaptersDialog::on_okbutton_clicked(GtkButton *button,
                                               gpointer user_data) {
...
void SelectChaptersDialog::on_okbutton() {
  new_selection =
      select_portion_get_portion(listview_get_strings(treeviewportions));
...
void SelectChaptersDialog::on_fromchapter() {
  ...
  // If from_chapter > to_chapter, push to_chapter up, as this is probably what
  // the user wants.
  unsigned int to =
...
void SelectChaptersDialog::on_fromverse() { verses_from_to(true); }
...
void SelectChaptersDialog::on_toverse() { verses_from_to(false); }

void SelectChaptersDialog::on_buttonall_clicked(GtkButton *button,
                                                gpointer user_data) {
...
// Sets the portion in the gui.
{
  ...
  ustring verse_from, verse_to;
  {
    ...
    select_portion_get_values(myproject, mybook, selection, chapters_from,
                              verses_from, chapters_to, verses_to);
  ...
  listview_set_strings(treeviewversefrom, liststoreversefrom,
                       project_get_verses(myproject, mybook, chapter_from));
  listview_set_strings(treeviewverseto, liststoreverseto,
                       project_get_verses(myproject, mybook, chapter_to));
...
void SelectChaptersDialog::verses_from_to(bool called_by_from) {
  ...
  unsigned int chapter =
      convert_to_int(listview_get_active_string(treeviewchapterfrom));
  if (convert_to_int(listview_get_active_string(treeviewchapterto)) ==
      chapter) {
...
void SelectChaptersDialog::on_treeviewchapterfrom_row_activated(
...
gboolean SelectChaptersDialog::on_treeviewchapterfrom_button_release_event(
    GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
  ((SelectChaptersDialog *)user_data)->on_fromchapter();
  return false;
}

gboolean SelectChaptersDialog::on_treeviewchapterfrom_key_release_event(
    GtkWidget *widget, GdkEventKey *event, gpointer user_data) {
  ((SelectChaptersDialog *)user_data)->on_fromchapter();
  return false;
}

void SelectChaptersDialog::on_treeviewversefrom_row_activated(
    GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *column,
    gpointer user_data) {
...
gboolean SelectChaptersDialog::on_treeviewversefrom_button_release_event(
    GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
  ((SelectChaptersDialog *)user_data)->on_fromverse();
  return false;
}

gboolean SelectChaptersDialog::on_treeviewversefrom_key_release_event(
    GtkWidget *widget, GdkEventKey *event, gpointer user_data) {
  ((SelectChaptersDialog *)user_data)->on_fromverse();
  return false;
}

void SelectChaptersDialog::on_treeviewchapterto_row_activated(
    GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *column,
    gpointer user_data) {
...
gboolean SelectChaptersDialog::on_treeviewchapterto_button_release_event(
    GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
  ((SelectChaptersDialog *)user_data)->on_tochapter();
  return false;
}

gboolean SelectChaptersDialog::on_treeviewchapterto_key_release_event(
    GtkWidget *widget, GdkEventKey *event, gpointer user_data) {
  ((SelectChaptersDialog *)user_data)->on_tochapter();
  return false;
}

void SelectChaptersDialog::on_treeviewverseto_row_activated(
    GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *column,
    gpointer user_data) {
...
gboolean SelectChaptersDialog::on_treeviewverseto_button_release_event(
    GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
  ((SelectChaptersDialog *)user_data)->on_toverse();
  return false;
}

gboolean SelectChaptersDialog::on_treeviewverseto_key_release_event(
    GtkWidget *widget, GdkEventKey *event, gpointer user_data) {
  ((SelectChaptersDialog *)user_data)->on_toverse();
  return false;
}

void SelectChaptersDialog::on_treeview1_cursor_changed(GtkTreeView *treeview,
                                                       gpointer user_data) {
...
gboolean SelectChaptersDialog::on_treeview1_move_cursor(GtkTreeView *treeview,
...
void SelectChaptersDialog::on_button_add_clicked(GtkButton *button,
                                                 gpointer user_data) {
...
void SelectChaptersDialog::on_button_delete_clicked(GtkButton *button,
                                                    gpointer user_data) {
...
{
  return select_portion_get_label(
      myproject, mybook,
      convert_to_int(listview_get_active_string(treeviewchapterfrom)),

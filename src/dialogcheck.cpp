...
CheckDialog::CheckDialog(CheckDialogType checkdialogtype) {
  ...
  gtk_window_set_position(GTK_WINDOW(checkdialog),
  ...
  case cdtChaptersVerses:
    information_setup(
        _("Checks the chapter and verse markers, to see whether they are "
          "correct according to the versification of the project."));
    book_selection_setup();
    break;
  case cdtMarkersCount:
    information_setup(_("Counts the number of times a marker is used."));
    sorting_setup(_("_No sorting"), _("Sort on _marker"), _("_Sort on count"),
                  NULL);
    book_selection_setup();
    break;
  ...
  case cdtMarkersCompare:
    information_setup(_("Compares the markers of the current project with the "
                        "ones in another project. This other project is taken "
                        "as the standard to compare against."));
    markers_comparison_setup();
    book_selection_setup();
    break;
  case cdtMarkersSpacing:
    information_setup(_("Checks whether the text that belongs to selected "
                        "markers has one space after it."));
    markers_spacing_setup();
    book_selection_setup();
    break;
  case cdtCharactersCount:
    information_setup(_("Counts the number of characters."));
    sorting_setup(_("_No sorting"), _("_Sort on character"),
                  _("Sort on co_unt"), NULL);
    book_selection_setup();
    break;
  case cdtWordsCapitalization:
    information_setup(
        _("Check capitalization of the text. Looks for missing capitals after "
          "selected punctuation marks, and for suspicious use of capitals such "
          "as \"hOuse\" or \"HOuse\"."));
  ...
  case cdtMatchingPairs:
    information_setup(
        _("Check matching pairs of punctuation. Checks that opening and "
          "closing punctuation match, e.g. a [ needs a ]. Also checks that "
          "punctuation is closed in the reverse order that it was opened."));
  ...
  case cdtWordsCount:
    ...
    sorting_setup(_("_No sorting"), _("Sort on _word"), _("_Sort on count"),
                  NULL);
    word_count_setup();
    book_selection_setup();
    break;
  case cdtReferencesInventory:
    information_setup(
        _("Makes an inventory of the references contained in the text."));
  ...
  case cdtNTQuotationsFromOT:
    information_setup(_(
        "Shows the places where the New Testament quotes the Old Testament."));
    include_verse_text_setup();
    output_in_ot_order_setup();
    book_selection_setup();
    break;
  case cdtSynopticParallelsNT:
    information_setup(_("Shows the synoptic parallel passages of the ") +
                      project_name + _(" New Testament."));
    include_verse_text_setup();
    parallel_project_setup();
    book_selection_setup();
    break;
  case cdtParallelsOT:
    information_setup(_("Shows the parallel passages of the ") + project_name +
                      _(" Old Testament."));
    include_verse_text_setup();
    parallel_project_setup();
    book_selection_setup();
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(checkdialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(checkdialog), okbutton,
                               GTK_RESPONSE_OK);
...
int CheckDialog::run() { return gtk_dialog_run(GTK_DIALOG(checkdialog)); }
...
void CheckDialog::sorting_setup(const gchar *sort0, const gchar *sort1,
                                const gchar *sort2, const gchar *sort3) {
  ...
  if (sort0) {
    ...
    gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_sort0),
                               radiobutton_sort_group);
    radiobutton_sort_group =
        gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_sort0));
  }
  ...
  if (sort1) {
    ...
    gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_sort1),
                               radiobutton_sort_group);
    radiobutton_sort_group =
        gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_sort1));
  }
  ...
  if (sort2) {
    ...
    gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_sort2),
                               radiobutton_sort_group);
    radiobutton_sort_group =
        gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_sort2));
  }
  ...
  if (sort3) {
    ...
    gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_sort3),
                               radiobutton_sort_group);
    radiobutton_sort_group =
        gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_sort3));
  }
...
void CheckDialog::book_selection_setup() {
  ...
  g_signal_connect((gpointer)button_books, "clicked",
                   G_CALLBACK(on_button_books_clicked), gpointer(this));

  extern Settings *settings;
  vector<unsigned int> scripture_books =
      project_get_books(settings->genconfig.project_get());
...
void CheckDialog::on_button_books_clicked(GtkButton *button,
                                          gpointer user_data) {
...
void CheckDialog::set_gui_books() {
  extern Settings *settings;
  ustring message =
      gui_book_selection_information(settings->genconfig.project_get());
...
void CheckDialog::area_selection_setup() {
  ...
  g_signal_connect((gpointer)button_area, "clicked",
                   G_CALLBACK(on_button_area_clicked), gpointer(this));

  gtk_label_set_text(GTK_LABEL(label_area), area_information().c_str());
}

void CheckDialog::on_button_area_clicked(GtkButton *button,
                                         gpointer user_data) {
...
void CheckDialog::markers_comparison_setup() {
  ...
  selectprojectgui->build(
      vbox3, _("Compare _with: "),
  ...
  checkbutton_compare_all =
      gtk_check_button_new_with_mnemonic(_("Compare _all markers"));
  gtk_widget_show(checkbutton_compare_all);
  gtk_box_pack_start(GTK_BOX(vbox3), checkbutton_compare_all, FALSE, FALSE, 0);

  // Set whether active.
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_compare_all),
      settings->genconfig.check_markers_compare_all_markers_get());

  label17 = gtk_label_new(
      _("Below provide a list of markers to be included in the comparison, "
        "separated with spaces and without the backslashes, e.g.: id c v"));
  ...
  gtk_box_pack_start(GTK_BOX(vbox3), entry_compare_include_only, FALSE, FALSE,
                     0);
  gtk_entry_set_activates_default(GTK_ENTRY(entry_compare_include_only), TRUE);

  // Set the markers.
  gtk_entry_set_text(
      GTK_ENTRY(entry_compare_include_only),
      settings->genconfig.check_markers_compare_include_only_get().c_str());
  ...
  gtk_entry_set_text(
      ...
      settings->genconfig.check_markers_compare_ignore_get().c_str());

  hseparator4 = gtk_hseparator_new();
  gtk_widget_show(hseparator4);
  gtk_box_pack_start(GTK_BOX(vbox3), hseparator4, TRUE, TRUE, 0);

  checkbutton_compare_ignore_verse_zero = gtk_check_button_new_with_mnemonic(
      _("Ignore anything before the _first verse"));
  gtk_widget_show(checkbutton_compare_ignore_verse_zero);
  gtk_box_pack_start(GTK_BOX(vbox3), checkbutton_compare_ignore_verse_zero,
                     FALSE, FALSE, 0);

  // Set the state.
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_compare_ignore_verse_zero),
      settings->genconfig.check_markers_compare_ignore_verse_zero_get());

  hseparator5 = gtk_hseparator_new();
  gtk_widget_show(hseparator5);
  gtk_box_pack_start(GTK_BOX(vbox3), hseparator5, TRUE, TRUE, 0);

  g_signal_connect_after((gpointer)checkbutton_compare_all, "toggled",
                         G_CALLBACK(on_checkbutton_compare_all_toggled),
                         gpointer(this));
...
void CheckDialog::markers_spacing_setup() {
  ...
  usfm_spacing_label2 = gtk_label_new_with_mnemonic(
      _("_Provide a list of markers to be checked, separated with spaces, and "
        "without the backslashes, e.g.: xo xt"));
  ...
  gtk_label_set_mnemonic_widget(GTK_LABEL(usfm_spacing_label2),
                                usfm_spacing_entry1);

  // Set the markers.
  gtk_entry_set_text(
      GTK_ENTRY(usfm_spacing_entry1),
      settings->genconfig.check_markers_spacing_include_get().c_str());
}

void CheckDialog::on_checkbutton_compare_all_toggled(
...
void CheckDialog::on_checkbutton_compare_all() {
  bool all;
  all =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(checkbutton_compare_all));
...
void CheckDialog::characters_unwanted_patterns_setup() {
  ...
  label46 = gtk_label_new(_("Enter a list of sequences of characters that are "
  ...
  gtk_box_pack_start(GTK_BOX(hbox19), button_unwanted_patterns, FALSE, FALSE,
                     0);
  ...
  g_signal_connect((gpointer)button_unwanted_patterns, "clicked",
                   G_CALLBACK(on_button_unwanted_patterns_clicked),
                   gpointer(this));
}

void CheckDialog::on_button_unwanted_patterns_clicked(GtkButton *button,
                                                      gpointer user_data) {
...
void CheckDialog::on_button_unwanted_patterns() {
  extern Settings *settings;
  ReadText rt(
      checks_unwanted_patterns_get_filename(settings->genconfig.project_get()),
      true, false);
  EditListDialog dialog(&rt.lines, _("Unwanted patterns"),
                        _("of patterns that are not wanted"), true, true, true,
                        true, true, true, false, NULL);
  if (dialog.run() == GTK_RESPONSE_OK) {
    write_lines(checks_unwanted_patterns_get_filename(
...
void CheckDialog::words_capitalization_setup() {
  ...
  label23 = gtk_label_new_with_mnemonic(
      _("Enter a list of p_unctuation characters, which must be followed by a "
        "capitalized word."));
  ...
  gtk_box_pack_start(GTK_BOX(hbox5), entry_capitalization_punctuation, TRUE,
                     TRUE, 0);
  gtk_entry_set_activates_default(GTK_ENTRY(entry_capitalization_punctuation),
                                  TRUE);

  // Enter value.
  gtk_entry_set_text(
      GTK_ENTRY(entry_capitalization_punctuation),
      settings->genconfig.check_capitalization_punctuation_get().c_str());

  button_capitalization_punctuation = gtk_button_new();
  gtk_widget_show(button_capitalization_punctuation);
  gtk_box_pack_start(GTK_BOX(hbox5), button_capitalization_punctuation, FALSE,
                     FALSE, 0);

  alignment3 = gtk_alignment_new(0.5, 0.5, 0, 0);
  gtk_widget_show(alignment3);
  gtk_container_add(GTK_CONTAINER(button_capitalization_punctuation),
                    alignment3);
  ...
  label25 = gtk_label_new_with_mnemonic(
      _("Enter a list of characters, after which a _lower case letter is "
        "acceptable.\nExample: The '>' in the following makes the lowercase "
  ...
  gtk_box_pack_start(GTK_BOX(hbox7), entry_capitalization_lowercase, TRUE, TRUE,
                     0);
  gtk_entry_set_activates_default(GTK_ENTRY(entry_capitalization_lowercase),
                                  TRUE);

  // Enter value.
  gtk_entry_set_text(
      GTK_ENTRY(entry_capitalization_lowercase),
      settings->genconfig.check_capitalization_ignore_get().c_str());

  button_capitalization_lowercase = gtk_button_new();
  gtk_widget_show(button_capitalization_lowercase);
  gtk_box_pack_start(GTK_BOX(hbox7), button_capitalization_lowercase, FALSE,
                     FALSE, 0);
  ...
  label27 = gtk_label_new(
      _("Enter a list of abbreviations that prevent errors saying that a "
        "period is not followed by an upper case letter."));
  ...
  gtk_box_pack_start(GTK_BOX(vbox6), button_capitalization_abbreviations, FALSE,
                     FALSE, 0);

  alignment5 = gtk_alignment_new(0.5, 0.5, 0, 0);
  gtk_widget_show(alignment5);
  gtk_container_add(GTK_CONTAINER(button_capitalization_abbreviations),
                    alignment5);
  ...
  label29 = gtk_label_new(
      _("Enter a list of prefixes which are allowed to occur uncapitalized "
        "before the first capital letter in a word."));
  gtk_widget_show(label29);
  gtk_box_pack_start(GTK_BOX(vbox9), label29, TRUE, TRUE, 0);
  gtk_label_set_line_wrap(GTK_LABEL(label29), TRUE);
  gtk_misc_set_alignment(GTK_MISC(label29), 0, 0.5);

  checkbutton_capitalization_prefixes =
      gtk_check_button_new_with_mnemonic(_("Allow an_y prefixes"));
  gtk_widget_show(checkbutton_capitalization_prefixes);
  gtk_box_pack_start(GTK_BOX(vbox9), checkbutton_capitalization_prefixes, FALSE,
                     FALSE, 0);

  // Set value
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_capitalization_prefixes),
  ...
  gtk_box_pack_start(GTK_BOX(vbox7), button_capitalization_prefixes, FALSE,
                     FALSE, 0);
  ...
  label31 = gtk_label_new(
      _("Enter a list of suffixes which are allowed to occur capitalized at "
        "the end of a word.\nExample: If \"seaCan\" is a valid word because "
        "the \"Can\" suffix must be capitalized, enter \"Can\" in this list."));
  ...
  gtk_box_pack_start(GTK_BOX(vbox8), button_capitalization_suffixes, FALSE,
                     FALSE, 0);
  ...
  g_signal_connect((gpointer)button_capitalization_punctuation, "clicked",
                   G_CALLBACK(on_button_capitalization_punctuation_clicked),
                   gpointer(this));
  g_signal_connect((gpointer)button_capitalization_lowercase, "clicked",
                   G_CALLBACK(on_button_capitalization_lowercase_clicked),
                   gpointer(this));
  g_signal_connect((gpointer)button_capitalization_abbreviations, "clicked",
                   G_CALLBACK(on_button_capitalization_abbreviations_clicked),
                   gpointer(this));
  g_signal_connect((gpointer)checkbutton_capitalization_prefixes, "toggled",
                   G_CALLBACK(on_checkbutton_capitalization_prefixes_toggled),
                   gpointer(this));
  g_signal_connect((gpointer)button_capitalization_prefixes, "clicked",
                   G_CALLBACK(on_button_capitalization_prefixes_clicked),
                   gpointer(this));
  g_signal_connect((gpointer)button_capitalization_suffixes, "clicked",
                   G_CALLBACK(on_button_capitalization_suffixes_clicked),
                   gpointer(this));

  on_checkbutton_prefixes();

  gtk_label_set_mnemonic_widget(GTK_LABEL(label23),
                                entry_capitalization_punctuation);
  gtk_label_set_mnemonic_widget(GTK_LABEL(label25),
                                entry_capitalization_lowercase);
}

void CheckDialog::on_button_capitalization_punctuation_clicked(
    GtkButton *button, gpointer user_data) {
  ((CheckDialog *)user_data)->capitalization_punctuation_default();
}

void CheckDialog::capitalization_punctuation_default() {
  gtk_entry_set_text(GTK_ENTRY(entry_capitalization_punctuation),
                     CAPITALIZATION_PUNCTUATION);
}

void CheckDialog::on_button_capitalization_lowercase_clicked(
    GtkButton *button, gpointer user_data) {
  ((CheckDialog *)user_data)->capitalization_lowercase_default();
}

void CheckDialog::capitalization_lowercase_default() {
  gtk_entry_set_text(GTK_ENTRY(entry_capitalization_lowercase),
                     CAPITALIZATION_LOWERCASE);
}

void CheckDialog::on_button_capitalization_abbreviations_clicked(
    GtkButton *button, gpointer user_data) {
...
void CheckDialog::capitalization_abbreviations_edit() {
  ...
  ReadText rt(
      ...
      true);
  EditListDialog dialog(&rt.lines, _("Abbreviations"), _("of abbreviations"),
...
void CheckDialog::on_checkbutton_capitalization_prefixes_toggled(
    GtkToggleButton *togglebutton, gpointer user_data) {
...
void CheckDialog::on_checkbutton_prefixes() {
  ...
  gtk_widget_set_sensitive(button_capitalization_prefixes,
                           !gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
                               checkbutton_capitalization_prefixes)));
}

void CheckDialog::on_button_capitalization_prefixes_clicked(
    GtkButton *button, gpointer user_data) {
...
void CheckDialog::button_capitalization_prefixes_edit() {
  extern Settings *settings;
  ReadText rt(checks_uncapitalized_prefixes_get_filename(
                  settings->genconfig.project_get()),
              true);
  EditListDialog dialog(&rt.lines, _("Uncapitalized prefixes"),
                        _("of uncapitalized prefixes"), true, true, true, true,
                        true, true, false, NULL);
  if (dialog.run() == GTK_RESPONSE_OK) {
    write_lines(checks_uncapitalized_prefixes_get_filename(
                    settings->genconfig.project_get()),
                rt.lines);
  }
}

void CheckDialog::on_button_capitalization_suffixes_clicked(
    GtkButton *button, gpointer user_data) {
...
void CheckDialog::capitalization_suffixes_edit() {
  extern Settings *settings;
  ReadText rt(checks_capitalized_suffixes_get_filename(
                  settings->genconfig.project_get()),
              true);
  EditListDialog dialog(&rt.lines, _("Capitalized suffixes"),
                        _("of capitalized suffixes"), true, true, true, true,
                        true, true, false, NULL);
  if (dialog.run() == GTK_RESPONSE_OK) {
    write_lines(checks_capitalized_suffixes_get_filename(
                    settings->genconfig.project_get()),
                rt.lines);
  }
}
...
void CheckDialog::word_repetition_setup() {
  ...
  checkbutton_repetition_case =
      gtk_check_button_new_with_mnemonic(_("C_ase insensitive"));
  gtk_widget_show(checkbutton_repetition_case);
  gtk_box_pack_start(GTK_BOX(vbox10), checkbutton_repetition_case, FALSE, FALSE,
                     0);

  // Set value.
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_repetition_case),
  ...
  checkbutton_repetition_show = gtk_check_button_new_with_mnemonic(
      _("Only _show repeating words in this list ->"));
  ...
  // Set state.
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_repetition_show),
      settings->genconfig.check_repetition_show_only_these_get());

  checkbutton_repetition_ignore = gtk_check_button_new_with_mnemonic(
      _("_Ignore repeating words given in this list ->\nor repeating word "
        "pairs that have texts in between them from this list ->"));
  gtk_widget_show(checkbutton_repetition_ignore);
  gtk_table_attach(GTK_TABLE(table1), checkbutton_repetition_ignore, 0, 1, 1, 2,
                   (GtkAttachOptions)(GTK_FILL), (GtkAttachOptions)(0), 0, 0);
  // Set state
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_repetition_ignore),
      settings->genconfig.check_repetition_ignore_these_get());
  ...
  g_signal_connect((gpointer)button_repetition_show, "clicked",
                   G_CALLBACK(on_button_repetition_show_clicked),
                   gpointer(this));
  g_signal_connect((gpointer)checkbutton_repetition_show, "toggled",
                   G_CALLBACK(on_checkbutton_repetition_show_toggled),
                   gpointer(this));
  g_signal_connect((gpointer)checkbutton_repetition_ignore, "toggled",
                   G_CALLBACK(on_checkbutton_repetition_ignore_toggled),
                   gpointer(this));
  g_signal_connect((gpointer)button_repetition_ignore, "clicked",
                   G_CALLBACK(on_button_repetition_ignore_clicked),
                   gpointer(this));
}

void CheckDialog::on_checkbutton_repetition_show_toggled(
    GtkToggleButton *togglebutton, gpointer user_data) {
  ((CheckDialog *)user_data)->on_checkbutton_repetition_show();
}

void CheckDialog::on_checkbutton_repetition_show() { set_gui(); }

void CheckDialog::on_button_repetition_show_clicked(GtkButton *button,
                                                    gpointer user_data) {
...
void CheckDialog::on_button_repetition_show() {
  extern Settings *settings;
  ReadText rt(checks_repetition_show_only_get_filename(
                  settings->genconfig.project_get()),
              true);
  EditListDialog dialog(
      &rt.lines, _("Repeating words"),
      _("of contiguous repeating words to show only in the report"), true, true,
      true, true, true, true, false, NULL);
  if (dialog.run() == GTK_RESPONSE_OK) {
    write_lines(checks_repetition_show_only_get_filename(
                    settings->genconfig.project_get()),
                rt.lines);
  }
}

void CheckDialog::on_checkbutton_repetition_ignore_toggled(
    GtkToggleButton *togglebutton, gpointer user_data) {
  ((CheckDialog *)user_data)->on_checkbutton_repetition_ignore();
}

void CheckDialog::on_checkbutton_repetition_ignore() { set_gui(); }

void CheckDialog::on_button_repetition_ignore_clicked(GtkButton *button,
                                                      gpointer user_data) {
...
void CheckDialog::on_button_repetition_ignore() {
  extern Settings *settings;
  ReadText rt(
      checks_repetition_ignore_get_filename(settings->genconfig.project_get()),
      true);
  EditListDialog dialog(&rt.lines, _("Repeating words"),
                        _("of contiguous repeating words to ignore or text "
                          "between these pairs of words"),
                        true, true, true, true, true, true, false, NULL);
  if (dialog.run() == GTK_RESPONSE_OK) {
    write_lines(checks_repetition_ignore_get_filename(
                    settings->genconfig.project_get()),
                rt.lines);
  }
}
...
void CheckDialog::unwanted_words_setup() {
  ...
  label50 = gtk_label_new(_("Enter a list of whole words that are unwanted in "
                            "the text.\nOne word per line."));
  ...
  g_signal_connect((gpointer)button_unwanted_words, "clicked",
                   G_CALLBACK(on_button_unwanted_words_clicked),
                   gpointer(this));
}

void CheckDialog::on_button_unwanted_words_clicked(GtkButton *button,
                                                   gpointer user_data) {
...
void CheckDialog::on_button_unwanted_words() {
  extern Settings *settings;
  ReadText rt(
      checks_unwanted_words_get_filename(settings->genconfig.project_get()),
      true);
  EditListDialog dialog(&rt.lines, _("Unwanted words"),
                        _("of whole words that are not wanted"), true, true,
                        true, true, true, true, false, NULL);
  if (dialog.run() == GTK_RESPONSE_OK) {
    write_lines(
        checks_unwanted_words_get_filename(settings->genconfig.project_get()),
...
void CheckDialog::punctuation_matching_pairs_setup() {
  ...
  label43 = gtk_label_new_with_mnemonic(
      _("Enter a list of punctuation characters to be _ignored.\nEnter them "
        "without spaces.\nOnly one of a pair is needed, the matching one is "
        "added automatically."));
  ...
  gtk_box_pack_start(GTK_BOX(vbox11), entry_matching_pairs_ignore, FALSE, FALSE,
                     0);
  gtk_entry_set_activates_default(GTK_ENTRY(entry_matching_pairs_ignore), TRUE);

  // Set value.
  gtk_entry_set_text(
      GTK_ENTRY(entry_matching_pairs_ignore),
      settings->genconfig.check_matching_pairs_ignore_get().c_str());

  gtk_label_set_mnemonic_widget(GTK_LABEL(label43),
...
// Update the GUI.
{
  ...
  if (checkbutton_repetition_case) {
    gtk_widget_set_sensitive(button_repetition_show,
                             gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
                                 checkbutton_repetition_show)));
    gtk_widget_set_sensitive(button_repetition_ignore,
                             gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
                                 checkbutton_repetition_ignore)));
  }
}
...
void CheckDialog::word_count_setup() {
  ...
  wordcount_checkbutton1 = gtk_check_button_new_with_mnemonic(
      _("Do not _include words with a count of"));
  gtk_widget_show(wordcount_checkbutton1);
  gtk_box_pack_start(GTK_BOX(wordcount_hbox1), wordcount_checkbutton1, FALSE,
                     FALSE, 0);

  wordcount_spinbutton1_adj = gtk_adjustment_new(0, 0, 1000000, 1, 10, 0);
  wordcount_spinbutton1 =
      gtk_spin_button_new(GTK_ADJUSTMENT(wordcount_spinbutton1_adj), 1, 0);
  gtk_widget_show(wordcount_spinbutton1);
  gtk_box_pack_start(GTK_BOX(wordcount_hbox1), wordcount_spinbutton1, FALSE,
                     FALSE, 0);

  wordcount_label2 = gtk_label_new(_("and up"));
  gtk_widget_show(wordcount_label2);
  gtk_box_pack_start(GTK_BOX(wordcount_hbox1), wordcount_label2, FALSE, FALSE,
                     0);

  wordcount_label1 = gtk_label_new_with_mnemonic(
      _("Bibledit-Gtk normally knows the right word boundaries, but if this is "
        "not done satisfactorily, you can enter _extra characters that should "
        "be part of a word"));
  ...
  g_signal_connect_after((gpointer)wordcount_checkbutton1, "clicked",
                         G_CALLBACK(on_wordcount_checkbutton1_activate),
                         gpointer(this));
  g_signal_connect((gpointer)wordcount_entry1, "changed",
                   G_CALLBACK(on_wordcount_entry1_changed), gpointer(this));
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(wordcount_checkbutton1),
                               count > 0);
  gtk_entry_set_text(
...
void CheckDialog::on_wordcount_checkbutton1_activate(GtkButton *button,
                                                     gpointer user_data) {
...
void CheckDialog::on_wordcount_checkbutton1() {
  bool active =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(wordcount_checkbutton1));
  gtk_widget_set_sensitive(wordcount_spinbutton1, active);
  if (active)
    gtk_widget_grab_focus(wordcount_spinbutton1);
}

void CheckDialog::on_wordcount_entry1_changed(GtkEditable *editable,
                                              gpointer user_data) {
...
void CheckDialog::on_wordcount_entry1() {
  ...
  if (chars != non_alphas) {
    ...
    gtkw_dialog_info(
        checkdialog,
        _("Bibledit-Gtk already recognizes this character as part of a "
...
void CheckDialog::parallel_project_setup() {
  ...
  checkbutton_include_second_project =
      gtk_check_button_new_with_mnemonic(_("_Compare with another project:"));
  gtk_widget_show(checkbutton_include_second_project);
  gtk_box_pack_start(GTK_BOX(vbox1), checkbutton_include_second_project, TRUE,
                     TRUE, 0);

  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_include_second_project),
      settings->session.check_include_second_project);

  g_signal_connect_after(
      (gpointer)checkbutton_include_second_project, "clicked",
      G_CALLBACK(on_include_second_project_activate), gpointer(this));

  selectparallelprojectgui = new SelectProjectGui(0);
  selectparallelprojectgui->build(
      vbox1, "", settings->genconfig.check_markers_compare_project_get());
}

void CheckDialog::on_include_second_project_activate(GtkButton *button,
                                                     gpointer user_data) {
...
void CheckDialog::include_verse_text_setup() {
  checkbutton_include_verse_text =
      gtk_check_button_new_with_mnemonic(_("_Include verse text"));
  gtk_widget_show(checkbutton_include_verse_text);
  gtk_box_pack_start(GTK_BOX(vbox1), checkbutton_include_verse_text, TRUE, TRUE,
                     0);

  extern Settings *settings;
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_include_verse_text),
      settings->session.check_include_verse_text);
}

void CheckDialog::output_in_ot_order_setup() {
  checkbutton_output_in_ot_order =
      gtk_check_button_new_with_mnemonic(_("O_utput in Old Testament order"));
  gtk_widget_show(checkbutton_output_in_ot_order);
  gtk_box_pack_start(GTK_BOX(vbox1), checkbutton_output_in_ot_order, TRUE, TRUE,
                     0);

  extern Settings *settings;
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(checkbutton_output_in_ot_order),
      settings->session.check_output_in_ot_order);
}
...
void CheckDialog::on_okbutton() {
  ...
  if (selectprojectgui) {
    settings->genconfig.check_markers_compare_project_set(
        selectprojectgui->project);
    settings->genconfig.check_markers_compare_all_markers_set(
        gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(checkbutton_compare_all)));
    settings->genconfig.check_markers_compare_include_only_set(
        gtk_entry_get_text(GTK_ENTRY(entry_compare_include_only)));
    settings->genconfig.check_markers_compare_ignore_set(
        gtk_entry_get_text(GTK_ENTRY(entry_compare_ignore)));
    settings->genconfig.check_markers_compare_ignore_verse_zero_set(
        gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(checkbutton_compare_ignore_verse_zero)));
  }
  ...
  if (entry_capitalization_punctuation) {
    settings->genconfig.check_capitalization_punctuation_set(remove_spaces(
        gtk_entry_get_text(GTK_ENTRY(entry_capitalization_punctuation))));
    settings->genconfig.check_capitalization_ignore_set(remove_spaces(
        gtk_entry_get_text(GTK_ENTRY(entry_capitalization_lowercase))));
    settings->genconfig.check_capitalization_allow_any_prefixes_set(
        gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(checkbutton_capitalization_prefixes)));
  }
  // Words - repetition
  if (checkbutton_repetition_case) {
    settings->genconfig.check_repetition_ignore_case_set(
        gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(checkbutton_repetition_case)));
    settings->genconfig.check_repetition_show_only_these_set(
        gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(checkbutton_repetition_show)));
    settings->genconfig.check_repetition_ignore_these_set(
        gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(checkbutton_repetition_ignore)));
  }
  // Punctuation - matching pairs.
  if (entry_matching_pairs_ignore) {
    settings->genconfig.check_matching_pairs_ignore_set(
        gtk_entry_get_text(GTK_ENTRY(entry_matching_pairs_ignore)));
  }
  // Words - Inventory / Count.
  if (wordcount_checkbutton1) {
    int count = gtk_spin_button_get_value_as_int(
        GTK_SPIN_BUTTON(wordcount_spinbutton1));
    if (!gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(wordcount_checkbutton1)))
      count = 0;
    settings->genconfig.check_words_inventory_not_include_words_count_set(
        count);
    settings->genconfig.check_words_inventory_word_forming_characters_set(
        gtk_entry_get_text(GTK_ENTRY(wordcount_entry1)));
  }
  // Markers - Spacing.
  if (usfm_spacing_entry1) {
    settings->genconfig.check_markers_spacing_include_set(
        gtk_entry_get_text(GTK_ENTRY(usfm_spacing_entry1)));
  }
  // Include verse text.
  if (checkbutton_include_verse_text) {
    settings->session.check_include_verse_text = gtk_toggle_button_get_active(
        GTK_TOGGLE_BUTTON(checkbutton_include_verse_text));
  }
  ...
  if (selectparallelprojectgui) {
    settings->genconfig.check_markers_compare_project_set(
        selectparallelprojectgui->project);
  }
  ...
  if (checkbutton_output_in_ot_order) {
    settings->session.check_output_in_ot_order = gtk_toggle_button_get_active(
        GTK_TOGGLE_BUTTON(checkbutton_output_in_ot_order));
  }
}
...
void CheckDialog::set_relevant_books() {
  ...
  // If the books in the current selection are all within the relevant list,
  // bail out.
  extern Settings *settings;
  vector<unsigned int> current_selection(
      settings->session.selected_books.begin(),
      settings->session.selected_books.end());

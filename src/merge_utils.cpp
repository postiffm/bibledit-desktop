...
const gchar *merge_verse_indicator() { return _("___verse___indicator___"); }
...
void merge_editor_and_file(vector<ustring> merge_base,
                           vector<ustring> editor_lines, const ustring &project,
...
// and stores the result to file.
{
  ...
  // If PHP is not available, skip running the filter to avoid complete data
  // loss.
  if (!gw_find_program_in_path("php")) {
    gw_message(_("Cannot find PHP."));
    gw_message(
        _("Bibledit-Gtk needs PHP for improved collaboration and merging."));
    gw_message(_("Some changes were lost."));
    gw_message(_("Install package php5-cli."));
    return;
  }
  ustring basefile =
      gw_build_filename(Directories->get_temp(), "mergebase.txt");
  write_lines(basefile, merge_base);
  ustring userfile =
      gw_build_filename(Directories->get_temp(), "mergeuser.txt");
  write_lines(userfile, editor_lines);
  vector<ustring> server_lines =
      project_retrieve_chapter(project, book, chapter);
  ustring serverfile =
      gw_build_filename(Directories->get_temp(), "mergeserver.txt");
  write_lines(serverfile, server_lines);
  ustring outputfile =
      gw_build_filename(Directories->get_temp(), "mergeoutput.txt");
  GwSpawn spawn("php");

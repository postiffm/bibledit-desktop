...
WindowShowRelatedVerses::WindowShowRelatedVerses(
    ...
    : FloatingWindow(parent_layout, widShowRelatedVerses, _("Related verses"),
                     startup),
      myreference(0)
...
void WindowShowRelatedVerses::go_to(const ustring &project,
  ...
  if (!myreference.equals(reference)) {
    ...
    //   gw_destroy_source (event_id);
    // }
    // event_id = g_timeout_add_full(G_PRIORITY_DEFAULT, 1000,
...
gboolean WindowShowRelatedVerses::on_navigation_policy_decision_requested(
  ...
  ((WindowShowRelatedVerses *)user_data)
      ->navigation_policy_decision_requested(request, navigation_action,
                                             policy_decision);
  return true;
}

void WindowShowRelatedVerses::navigation_policy_decision_requested(
    WebKitNetworkRequest *request, WebKitWebNavigationAction *navigation_action,
...
void WindowShowRelatedVerses::load_webview(const gchar *url) {
  ...
  if (active_url.find("keyterm ") == 0) {
    ...
    for (unsigned int i = 0; i < keyterms.size(); i++) {
      ...
      keyterms_retrieve_renderings(myproject, term, category, renderings,
    ...
    parallel_passages_retrieve(myreference, parallel_references,
                               parallel_comments);
    if (!parallel_references.empty()) {
      ...
      for (unsigned int i = 0; i < parallel_references.size(); i++) {
        htmlwriter.paragraph_open();
        htmlwriter.text_add(parallel_references[i].human_readable(language) +
                            " " + parallel_comments[i]);
        htmlwriter.paragraph_close();
      }
      htmlwriter.paragraph_open();

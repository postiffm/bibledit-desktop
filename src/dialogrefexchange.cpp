...
ReferenceExchangeDialog::ReferenceExchangeDialog(int dummy) {
  ...
  gtk_builder_add_from_file(
      gtkbuilder, gw_build_filename(Directories->get_package_data(),
                                    "gtkbuilder.referenceexchangedialog.xml")
                      .c_str(),
      NULL);
  ...
  g_signal_connect((gpointer)button_url, "clicked",
                   G_CALLBACK(on_url_test_clicked), gpointer(this));
...
void ReferenceExchangeDialog::on_okbutton_clicked(GtkButton *button,
                                                  gpointer user_data) {
...
void ReferenceExchangeDialog::on_okbutton() {
  extern Settings *settings;
  settings->genconfig.bibledit_web_url_set(
      gtk_entry_get_text(GTK_ENTRY(entry_url)));
  settings->genconfig.bibledit_web_user_set(
      gtk_entry_get_text(GTK_ENTRY(entry_user)));
}

void ReferenceExchangeDialog::on_url_test_clicked(GtkButton *button,
                                                  gpointer user_data) {

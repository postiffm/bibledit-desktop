...
RemoteRepositoryAssistant::RemoteRepositoryAssistant(int dummy)
    : AssistantBase(_("Remote repository setup"),
                    _("menu-preferences/dialog-remote-repository"))
// Assistant for managing the remote repository.
{
  ...
  introduction(_("Remote repository management for Bible ") + bible +
               _(" or the project notes"));
  ...
  page_number_bible_notes_selector = gtk_assistant_append_page(
      GTK_ASSISTANT(assistant), vbox_bible_notes_selector);

  GSList *radiobutton_bible_notes_selector_group = NULL;

  radiobutton_bible_notes_selector_bible =
      gtk_radio_button_new_with_mnemonic(NULL, _("B_ible repository"));
  gtk_widget_show(radiobutton_bible_notes_selector_bible);
  gtk_box_pack_start(GTK_BOX(vbox_bible_notes_selector),
                     radiobutton_bible_notes_selector_bible, FALSE, FALSE, 0);
  gtk_radio_button_set_group(
      GTK_RADIO_BUTTON(radiobutton_bible_notes_selector_bible),
      radiobutton_bible_notes_selector_group);
  radiobutton_bible_notes_selector_group = gtk_radio_button_get_group(
      GTK_RADIO_BUTTON(radiobutton_bible_notes_selector_bible));

  radiobutton_bible_notes_selector_notes =
      gtk_radio_button_new_with_mnemonic(NULL, _("Project _notes repository"));
  gtk_widget_show(radiobutton_bible_notes_selector_notes);
  gtk_box_pack_start(GTK_BOX(vbox_bible_notes_selector),
                     radiobutton_bible_notes_selector_notes, FALSE, FALSE, 0);
  gtk_radio_button_set_group(
      GTK_RADIO_BUTTON(radiobutton_bible_notes_selector_notes),
      radiobutton_bible_notes_selector_group);
  radiobutton_bible_notes_selector_group = gtk_radio_button_get_group(
      GTK_RADIO_BUTTON(radiobutton_bible_notes_selector_notes));
  ...
  gtk_assistant_set_page_title(
      GTK_ASSISTANT(assistant), vbox_bible_notes_selector,
      _("What type of repository would you like to set?"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant),
                              vbox_bible_notes_selector,
                              GTK_ASSISTANT_PAGE_CONTENT);
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant),
                                  vbox_bible_notes_selector, true);

  // Build the GUI for the setting whether to use a remote repository.
  checkbutton_use_repository =
      gtk_check_button_new_with_mnemonic(_("_Use remote repository"));
  gtk_widget_show(checkbutton_use_repository);
  page_number_use_repository = gtk_assistant_append_page(
      GTK_ASSISTANT(assistant), checkbutton_use_repository);

  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant),
                               checkbutton_use_repository,
                               _("Would you like to use a remote repository?"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant),
                              checkbutton_use_repository,
                              GTK_ASSISTANT_PAGE_CONTENT);
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant),
                                  checkbutton_use_repository, true);

  // System for trying out git.
  git_tried_and_okay = false;

  label_try_git = gtk_label_new(
      _("The local content tracker has been tested and works fine"));
  gtk_widget_show(label_try_git);
  page_number_try_git =
      gtk_assistant_append_page(GTK_ASSISTANT(assistant), label_try_git);

  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), label_try_git,
                               _("Trying out the content tracker"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant), label_try_git,
                              GTK_ASSISTANT_PAGE_CONTENT);
  ...
  page_number_repository =
      gtk_assistant_append_page(GTK_ASSISTANT(assistant), vbox_repository);
  ...
  gtk_box_pack_start(GTK_BOX(hbox_repository), label_repository, FALSE, FALSE,
                     0);
  ...
  gtk_box_pack_start(GTK_BOX(vbox_repository), label_repository_accessible,
                     FALSE, FALSE, 0);
  ...
  gtk_assistant_set_page_title(
      GTK_ASSISTANT(assistant), vbox_repository,
      _("Enter the location of the remote repository"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant), vbox_repository,
                              GTK_ASSISTANT_PAGE_CONTENT);
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant), vbox_repository,
                                  false);

  g_signal_connect((gpointer)entry_repository, "changed",
                   G_CALLBACK(on_entry_repository_changed), gpointer(this));

  // GUI for cloning the repository.
  vbox_clone = gtk_vbox_new(FALSE, 0);
  gtk_widget_show(vbox_clone);
  page_number_clone =
      gtk_assistant_append_page(GTK_ASSISTANT(assistant), vbox_clone);
  ...
  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), vbox_clone,
                               _("Cloning data"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant), vbox_clone,
                              GTK_ASSISTANT_PAGE_CONTENT);
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant), vbox_clone, false);

  g_signal_connect((gpointer)button_clone, "clicked",
                   G_CALLBACK(on_button_clone_clicked), gpointer(this));

  // Write tester.
  label_write_test = gtk_label_new("");
  gtk_widget_show(label_write_test);
  page_number_write_test =
      gtk_assistant_append_page(GTK_ASSISTANT(assistant), label_write_test);

  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), label_write_test,
                               _("Remote repository write test"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant), label_write_test,
                              GTK_ASSISTANT_PAGE_CONTENT);
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant), label_write_test,
                                  false);
  ...
  page_number_push =
      gtk_assistant_append_page(GTK_ASSISTANT(assistant), vbox_push);
  ...
  label1 =
      gtk_label_new_with_mnemonic(_("_Push my data to the remote repository"));
  gtk_widget_show(label1);
  gtk_box_pack_start(GTK_BOX(hbox1), label1, FALSE, FALSE, 0);

  gtk_assistant_set_page_title(
      GTK_ASSISTANT(assistant), vbox_push,
      _("Would you like to push your data to the repository?"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant), vbox_push,
                              GTK_ASSISTANT_PAGE_CONTENT);
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant), vbox_push, true);

  g_signal_connect((gpointer)button_push, "clicked",
                   G_CALLBACK(on_button_push_clicked), gpointer(this));
  ...
  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), label_confirm,
                               _("Settings are ready to be applied"));
...
void RemoteRepositoryAssistant::on_assistant_prepare_signal(
    GtkAssistant *assistant, GtkWidget *page, gpointer user_data) {
...
void RemoteRepositoryAssistant::on_assistant_prepare(GtkWidget *page) {
  ...
  if (page == checkbutton_use_repository) {
    // Set all values in the GUI, according to the project configuration if it
    // is a Bible,
    ...
    if (bible_notes_selector_bible()) {
      ...
      use_remote_repository =
          settings->genconfig
              .consultation_notes_git_use_remote_repository_get();
    }
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbutton_use_repository),
                                 use_remote_repository);
    ...
    if (bible_notes_selector_bible()) {
      ...
      repository_url = settings->genconfig
                           .consultation_notes_git_remote_repository_url_get();
    }
  ...
  if (page == vbox_clone) {
    ...
    gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant), vbox_clone,
                                    repository_was_cloned());
...
void RemoteRepositoryAssistant::on_assistant_apply_signal(
    GtkAssistant *assistant, gpointer user_data) {
...
void RemoteRepositoryAssistant::on_assistant_apply() {
  ...
  // Whether to use the remote repository.
  bool use_remote_repository = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(checkbutton_use_repository));
  if (bible_notes_selector_bible())
    projectconfig->git_use_remote_repository_set(use_remote_repository);
  else
    settings->genconfig.consultation_notes_git_use_remote_repository_set(
        use_remote_repository);
  ...
  if (bible_notes_selector_bible())
    ...
    settings->genconfig.consultation_notes_git_remote_repository_url_set(
        repository_url_get());
  ...
  if (repository_was_cloned()) {
    ...
    // This is necessary for the consultation notes, since git has been seen to
    // cause spurious renames.
    GwSpawn spawn("git");
...
gint RemoteRepositoryAssistant::assistant_forward_function(gint current_page,
                                                           gpointer user_data) {
  return ((RemoteRepositoryAssistant *)user_data)
      ->assistant_forward(current_page);
}
...
// Tries git and returns true if everything's fine.
{
  ...
  if (okay) {
    ...
    gw_message(
        _("Fetch data from the first local repository into the remote one"));
...
// Checks the git version.
{
  ...
  if (!spawn.result) {
    gtk_label_set_text(GTK_LABEL(label_try_git),
                       _("Program git failed to run. Has git been installed?"));
  }
  return okay;
}
...
{
  ustring directory =
      gw_build_filename(Directories->get_temp(), "git-test", name);
  return directory;
}
...
bool RemoteRepositoryAssistant::try_git_create_repository(const ustring &name,
                                                          bool remote) {
  ...
  if (!okay) {
    gtk_label_set_text(GTK_LABEL(label_try_git),
                       _("git init fails to create a repository"));
  }
  return okay;
}

bool RemoteRepositoryAssistant::try_git_store_data_in_repository(
    const ustring &repository, const gchar *data)
// Put data into the repository.
{
  ...
  if (!okay) {
    gtk_label_set_text(GTK_LABEL(label_try_git),
                       _("git add fails to add data to the repository"));
  }
  ...
  if (!okay) {
    gtk_label_set_text(GTK_LABEL(label_try_git),
                       _("git commit fails to commit data to the repository"));
  }

  return okay;
}

bool RemoteRepositoryAssistant::try_git_check_data_in_repository(
    const ustring &repository, const gchar *data)
// Checks the data into the repository.
{
  ...
  if (!okay) {
    gtk_label_set_text(GTK_LABEL(label_try_git),
                       _("cannot read data in repository"));
  }
  ...
  if (okay) {
    if (repository_data != standard_data) {
      gtk_label_set_text(GTK_LABEL(label_try_git),
                         _("unexpected data in repository"));
    }
  }
  return okay;
}

bool RemoteRepositoryAssistant::try_git_fetch_repository(const ustring &remote,
                                                         const ustring &local)
// Fetch the local into the remote repository.
{
  ...
  if (!okay) {
    gtk_label_set_text(
        GTK_LABEL(label_try_git),
        _("git --bare fetch fails to fetch data into remote repository"));
  }
  return okay;
}

bool RemoteRepositoryAssistant::try_git_checkout_repository(
    const ustring &local, const ustring &remote)
// Checks the remote repository out into the local one.
{
  ...
  if (!okay) {
    gtk_label_set_text(GTK_LABEL(label_try_git),
                       _("git clone fails to clone the repository"));
  }
...
// Pull repository.
{
  ...
  if (!okay) {
    gtk_label_set_text(GTK_LABEL(label_try_git),
                       _("git pull from remote repository failed"));
  }
  return okay;
}
...
// Push repository.
{
  ...
  if (!okay) {
    gtk_label_set_text(GTK_LABEL(label_try_git),
                       _("git push to remote repository failed"));
  }
  return okay;
}

void RemoteRepositoryAssistant::on_entry_repository_changed(
...
void RemoteRepositoryAssistant::on_entry_repository() {
  if (ignore_entry_repository_changed)
    return;
  gw_destroy_source(event_id_entry_repository);
  event_id_entry_repository = g_timeout_add_full(
      G_PRIORITY_DEFAULT, 1000, GSourceFunc(on_entry_changed_timeout),
      gpointer(this), NULL);
}
...
void RemoteRepositoryAssistant::on_button_clone_clicked(GtkButton *button,
                                                        gpointer user_data) {
...
void RemoteRepositoryAssistant::on_button_clone() {
  ...
  if (repository_was_cloned()) {
    gtk_label_set_text(GTK_LABEL(label_clone),
                       _("The data has been cloned, you can go forward"));
    previously_cloned_url = repository_url_get();
  } else {
    gtk_label_set_text(GTK_LABEL(label_clone),
                       _("Cloning the data failed, please try again"));
    repository_unclone();
  }
}
...
// Checks whether there is write access from the local clone to the remote
// repository.
{
  // GUI update.
  // If a wrong host is entered as the git repository, the testing for the write
  // access
  // may "hang" for a long time. For that reason it can be cancelled by the
  // user.
  ProgressWindow progresswindow(_("Testing write access"), true);
  gtk_label_set_text(GTK_LABEL(label_write_test),
                     _("Testing write access to the remote repository"));

  // Temporal file for trying write access.
  ustring filename = "test_repository_writable";
  g_file_set_contents(
      gw_build_filename(persistent_clone_directory, filename).c_str(), "", 0,
      NULL);
  ...
  // Notice the --all switch to be used when pushing to an empty remote
  // repository.
  ...
  if (write_access_granted) {
    gtk_label_set_text(GTK_LABEL(label_write_test),
                       _("Write access was granted, you can go forward"));
  } else {
    gtk_label_set_text(
        GTK_LABEL(label_write_test),
        _("No write access. Please check the system log for more information"));
  }
}

void RemoteRepositoryAssistant::on_button_push_clicked(GtkButton *button,
                                                       gpointer user_data) {
...
/*
It copies the existing data, without the .git directory, into the persistent
clone,
replaces any data that was there, and then pushes this data to the remote
repository.
...
*/
{
  ...
  // In rare cases a .git directory could have been copied along with our data.
  // Remove that.
  unix_rmdir(gw_build_filename(temporal_data_directory, ".git"));

  // Remove all directories and all files from the persistent clone directory,
  // but leave the .git directory
  {
    ReadDirectories rd(persistent_clone_directory, "", "");
    for (unsigned int i = 0; i < rd.directories.size(); i++) {
      if (rd.directories[i] != ".git") {
        unix_rmdir(
            gw_build_filename(persistent_clone_directory, rd.directories[i]));
      }
    }
    ReadFiles rf(persistent_clone_directory, "", "");
    for (unsigned int i = 0; i < rf.files.size(); i++) {
      unix_unlink(
          gw_build_filename(persistent_clone_directory, rf.files[i]).c_str());
    }
  }

  // Move our data, from its temporal location, into the persistent clone
  // directory.
  progresswindow.set_fraction(0.4);
  {
    ReadDirectories rd(temporal_data_directory, "", "");
    for (unsigned int i = 0; i < rd.directories.size(); i++) {
      unix_mv(gw_build_filename(temporal_data_directory, rd.directories[i]),
              persistent_clone_directory);
    }
    ReadFiles rf(temporal_data_directory, "", "");
    for (unsigned int i = 0; i < rf.files.size(); i++) {
      unix_mv(gw_build_filename(temporal_data_directory, rf.files[i]),
              persistent_clone_directory);
    }
  }
  ...
  if (spawn.exitstatus == 0) {
    // Clone okay.
    gtk_label_set_text(GTK_LABEL(label_push),
                       _("Your data has been pushed to the remote repository"));
  } else {
    // Clone failed.
    gtk_label_set_text(GTK_LABEL(label_push),
                       _("Your data could not be pushed to the remote "
...
bool RemoteRepositoryAssistant::bible_notes_selector_bible() {
  return gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(radiobutton_bible_notes_selector_bible));
}

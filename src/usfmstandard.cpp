...
USFMStandard::USFMStandard(int dummy) { load(); }
...
void USFMStandard::load() {
  ...
  ustring xmlfilename =
      gw_build_filename(Directories->get_package_data(), "usfm.xml");
    ...
    while ((xmlTextReaderRead(reader) == 1)) {
      ...
      case XML_READER_TYPE_ELEMENT: {
        ...
        if (!strcmp(opening_element, "entry")) {
          ...
          attribute = (char *)xmlTextReaderGetAttribute(reader, BAD_CAST
                                                        "startswithbackslash");
          if (attribute) {
          ...
          variants.clear();
          attribute =
              (char *)xmlTextReaderGetAttribute(reader, BAD_CAST "variants");
          if (attribute) {
            variants = attribute;
            free(attribute);
          }
          ...
          startsline_v = true;
          attribute =
              (char *)xmlTextReaderGetAttribute(reader, BAD_CAST "startsline");
          if (attribute) {
            if (!strcmp(attribute, "no")) {
              startsline_v = false;
            }
          ...
          attribute =
              (char *)xmlTextReaderGetAttribute(reader, BAD_CAST "function");
          if (attribute) {

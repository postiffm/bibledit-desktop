...
TaskDurationDialog::TaskDurationDialog(vector<ustring> *tasks,
  ...
  gtk_window_set_title(GTK_WINDOW(taskdurationdialog),
                       _("Task Duration Setup"));
  gtk_window_set_position(GTK_WINDOW(taskdurationdialog),
                          GTK_WIN_POS_CENTER_ON_PARENT);
  gtk_window_set_type_hint(GTK_WINDOW(taskdurationdialog),
  ...
  label4 = gtk_label_new(
      _("Set the number of days a task takes to complete one chapter"));
  ...
  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(taskdurationdialog));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(taskdurationdialog), cancelbutton1,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton1), true);

  okbutton1 = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton1);
  gtk_dialog_add_action_widget(GTK_DIALOG(taskdurationdialog), okbutton1,
                               GTK_RESPONSE_OK);
...
void TaskDurationDialog::on_okbutton1_clicked(GtkButton *button,
                                              gpointer user_data) {
...
void TaskDurationDialog::on_okbutton1() {
  ...
  for (unsigned int i = 0; i < spinbuttons.size(); i++) {
    mydurations->push_back(
        gtk_spin_button_get_value(GTK_SPIN_BUTTON(spinbuttons[i])));
  }
}

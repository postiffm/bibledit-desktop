...
OutpostDialog::OutpostDialog(int dummy) {
  ...
  gtk_window_set_position(GTK_WINDOW(outpostdialog),
  ...
  checkbutton_use =
      gtk_check_button_new_with_mnemonic(_("_Use Windows Outpost"));
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbutton_use),
                               settings->genconfig.use_outpost_get());
  ...
  new InDialogHelp(outpostdialog, NULL, NULL,
                   "menu-preferences/windows-outpost");

  cancelbutton = gtk_button_new_from_stock("gtk-cancel");
  gtk_widget_show(cancelbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(outpostdialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(outpostdialog), okbutton,
                               GTK_RESPONSE_OK);
...
OutpostDialog::~OutpostDialog() { gtk_widget_destroy(outpostdialog); }

int OutpostDialog::run() { return gtk_dialog_run(GTK_DIALOG(outpostdialog)); }
...
void OutpostDialog::on_ok() {
  ...
  extern Settings *settings;
  bool use_outpost =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(checkbutton_use));

...
ImportBookRead::ImportBookRead(const ustring &filename,
...
gchar *unicode_convert(gchar *data, const ustring &encoding) {
  ...
  GError *error = NULL;
  output =
      g_convert(data, length, "UTF-8", encoding.c_str(), NULL, NULL, &error);
  if (output == NULL) {
    cerr << error->message << endl;
    g_error_free(error);
  }
...
void import_check_usfm_files(vector<ustring> &filenames,
                             vector<unsigned int> &bookids,
                             const ustring &bible, vector<ustring> &messages) {
  // Check whether all the USFM files have proper \id data.
  if (messages.empty()) {
    vector<ustring> files_names_temp;
    for (unsigned int i = 0; i < filenames.size(); i++) {
      unsigned int book_id =
          books_paratext_to_id(get_usfm_id_from_file(filenames[i]));
      if (book_id) {
  ...
  if (messages.empty()) {
    ...
    set<unsigned int> books_in_project_set(books_in_project.begin(),
                                           books_in_project.end());
    for (unsigned int i = 0; i < bookids.size(); i++) {
      if (books_in_project_set.find(bookids[i]) != books_in_project_set.end()) {
        messages.push_back(_("File ") + filenames[i] + _(" has book ") +
                           books_id_to_english(bookids[i]) +
                           _(", but this one is already in the project"));
      }
    }
  }
}

void import_usfm_file(const ustring &file, unsigned int book,
                      const ustring &project, vector<ustring> &messages) {
  // Read the file.
  gchar *contents;
  gsize length;
  GError *error = NULL;

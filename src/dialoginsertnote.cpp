  ...
  for (unsigned int i = 0; i < styles.size(); i++) {
    ...
    case indtEndnote: {
      if (styles[i].type == stFootEndNote) {
        if ((styles[i].subtype == fentStandardContent) || (styles[i].subtype == fentContent) || (styles[i].subtype == fentContentWithEndmarker) || (styles[i].subtype == fentParagraph)) {
    ...
    case indtCrossreference: {
      if (styles[i].type == stCrossreference) {
        if ((styles[i].subtype == ctStandardContent) || (styles[i].subtype == ctContent) || (styles[i].subtype == ctContentWithEndmarker)) {
          content_markers.push_back(styles[i].marker);
...
// Also set the label accordingly.
{
  ...
  if (gtk_widget_get_sensitive(entry_reference_separator)) {
    result = settings->genconfig.chapter_get() + gtk_entry_get_text(GTK_ENTRY(entry_reference_separator)) + settings->genconfig.verse_get();

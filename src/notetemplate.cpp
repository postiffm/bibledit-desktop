...
NoteTemplate::NoteTemplate(const ustring &filename, NoteType notetype,
                           bool write) {
  // Save variables.
  myfilename = filename;
  mywrite = write;
...
NoteTemplate::~NoteTemplate() {
  if (mywrite) {
    ...
    output.push_back(
        xml_text_embed_in_tags(0, NUMBERING, convert_to_string(numbering)));
    output.push_back(xml_text_embed_in_tags(0, ANCHOR, anchor));
    output.push_back(xml_text_embed_in_tags(
        0, AUTOMATIC_REFERENCE, convert_to_string(automatic_reference)));
    output.push_back(xml_text_embed_in_tags(
        0, CHAPTER, convert_to_string(reference_chapter)));
    output.push_back(xml_text_embed_in_tags(0, DIVISION, reference_division));
    output.push_back(
        xml_text_embed_in_tags(0, VERSE, convert_to_string(reference_verse)));
    ...
    for (unsigned int i = 0; i < content_markers.size(); i++) {
      ...
      output.push_back(xml_text_embed_in_tags(
          1, REMEMBER, convert_to_string(content_remembers[i])));
      output.push_back(
          xml_text_embed_in_tags(1, SIZE, convert_to_string(content_sizes[i])));
      output.push_back(xml_text_embed_in_tags(
          1, TIDY, convert_to_string(content_tidies[i])));
...
void NoteTemplate::text_handler(GMarkupParseContext *context, const gchar *text,
...
void NoteTemplate::error_handler(GMarkupParseContext *context, GError *error,
                                 gpointer user_data) {
  cerr << error->message << endl;
}
...
*/
{
  ...
  if (element_name == CONTENT) {
    // Empty content markers are not loaded, because these cause unpredictable
    // behaviour.

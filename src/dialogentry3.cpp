...
Entry3Dialog::Entry3Dialog(const string &title, bool allow_empty_values,
  ...
  gtk_window_set_position(GTK_WINDOW(entry3dialog),
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(entry3dialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(entry3dialog), okbutton,
                               GTK_RESPONSE_OK);
  ...
  g_signal_connect((gpointer)entry1, "changed", G_CALLBACK(on_entry_changed),
                   gpointer(this));
  g_signal_connect((gpointer)entry2, "changed", G_CALLBACK(on_entry_changed),
                   gpointer(this));
  g_signal_connect((gpointer)entry3, "changed", G_CALLBACK(on_entry_changed),
                   gpointer(this));
...
Entry3Dialog::~Entry3Dialog() { gtk_widget_destroy(entry3dialog); }

int Entry3Dialog::run() { return gtk_dialog_run(GTK_DIALOG(entry3dialog)); }
...
// Activates the ok button only when we've valuesin the entries.
{
  ...
  bool sensitive;
  sensitive = !(entered_value1.empty() || entered_value2.empty() ||
                entered_value3.empty());

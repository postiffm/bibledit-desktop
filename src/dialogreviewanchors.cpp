...
ReviewAnchorsDialog::ReviewAnchorsDialog(vector<unsigned int> *books,
...
// Reviews anchors that would be placed in a file for the Resource Viewer.
{
  ...
  gtk_window_set_position(GTK_WINDOW(reviewanchorsdialog),
  ...
  label1 =
      gtk_label_new(_("Review the anchors that will be placed.\nTo remove some "
  ...
  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(reviewanchorsdialog));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(reviewanchorsdialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(reviewanchorsdialog), okbutton,
                               GTK_RESPONSE_OK);
  ...
  store = gtk_list_store_new(5, G_TYPE_STRING, G_TYPE_INT, G_TYPE_INT,
                             G_TYPE_INT, G_TYPE_INT);
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview_anchors),
                          GTK_TREE_MODEL(store));
  g_object_unref(store);
  GtkCellRenderer *renderer = gtk_cell_renderer_text_new();
  column0 = gtk_tree_view_column_new_with_attributes(_("Book"), renderer,
                                                     "text", 0, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview_anchors), column0);
  column1 = gtk_tree_view_column_new_with_attributes(_("Chapter"), renderer,
                                                     "text", 1, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview_anchors), column1);
  column2 = gtk_tree_view_column_new_with_attributes(_("Verse"), renderer,
                                                     "text", 2, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview_anchors), column2);
  column3 = gtk_tree_view_column_new_with_attributes(_("Line"), renderer,
                                                     "text", 3, NULL);
  ...
  for (unsigned int i = 0; i < books->size(); i++) {
    ...
    gtk_list_store_set(store, &iter, 0,
                       books_id_to_english(books->at(i)).c_str(), 1,
                       chapters->at(i), 2, verses->at(i), 3, linenumbers->at(i),
                       4, books->at(i), -1);
  }

  g_signal_connect((gpointer)treeview_anchors, "key_press_event",
                   G_CALLBACK(on_treeview_key_press_event), gpointer(this));
  g_signal_connect((gpointer)button_spurious_anchors, "clicked",
                   G_CALLBACK(on_button_spurious_anchors_clicked),
                   gpointer(this));
  g_signal_connect((gpointer)okbutton, "clicked",
                   G_CALLBACK(static_on_okbutton_clicked), gpointer(this));
...
gboolean ReviewAnchorsDialog::on_treeview_key_press_event(GtkWidget *widget,
                                                          GdkEventKey *event,
                                                          gpointer user_data) {
...
// Handles pressing keys in the treeview.
{
  ...
  if (event->keyval == GDK_KEY_Delete || event->keyval == GDK_KEY_KP_Delete) {
    ...
    gtk_tree_selection_selected_foreach(
        select, ReviewAnchorsDialog::on_collect_iters, gpointer(&iters));
...
void ReviewAnchorsDialog::on_collect_iters(GtkTreeModel *model,
                                           GtkTreePath *path, GtkTreeIter *iter,
                                           gpointer data) {
  ((vector<GtkTreeIter> *)data)->push_back(*iter);
}

void ReviewAnchorsDialog::on_button_spurious_anchors_clicked(
...
*/
{
  ...
  GtkTreeModel *model =
      gtk_tree_view_get_model(GTK_TREE_VIEW(treeview_anchors));
  GtkTreeIter iter;
  gboolean valid;
...
void ReviewAnchorsDialog::static_on_okbutton_clicked(GtkButton *button,
                                                     gpointer user_data) {
...
void ReviewAnchorsDialog::on_okbutton_clicked() {
  ...
  while (valid) {
    ...
    gtk_tree_model_get(model, &iter, 1, &chapter, 2, &verse, 3, &linenumber, 4,
                       &book, -1);
    // Store.

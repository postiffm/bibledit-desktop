...
// Converts XeTeX code to a pdf file.
{
  ...
  // surface = cairo_pdf_surface_create(pdffile.c_str(),
  // pango_units_to_points(page_width_pango_units),
  // pango_units_to_points(page_height_pango_units));
...
void XeTeX2Pdf::page_size_set(double width_centimeters,
...
void XeTeX2Pdf::page_margins_set(double inside_margin_centimeters,
                                 double right_margin_centimeters,
                                 double top_margin_centimeters,
                                 double bottom_margin_centimeters)
// Set the margins of the page, in centimeters.
{
  inside_margin_pango_units =
      centimeters_to_pango_units(inside_margin_centimeters);
  outside_margin_pango_units =
      centimeters_to_pango_units(right_margin_centimeters);
  top_margin_pango_units = centimeters_to_pango_units(top_margin_centimeters);
  bottom_margin_pango_units =
      centimeters_to_pango_units(bottom_margin_centimeters);
}
...
// Text is normally laid out in two columns, but there are elements that span
// the columns.
{
...
// Sets the engine to go to a new page. If odd is true, it goes to a new odd
// page.
{
  // Close any open paragraph.
  close_paragraph();

  // The paragraph before the new page should not have its 'keep-with-next'
  // property set, else the engine chokes on that.
  /*
  if (!input_data.empty()) {
...
// Runs the converter.
{
  ...
  ustring filename_tex =
      gw_build_filename(Directories->get_temp(), "document.tex");
  ...
  /*
    ...
    for (unsigned int i = 0; i < input.size(); i++) {
      ...
      case t2pitOpenKeepTogether:
        {
          // Paragraphs from here till the next t2pitCloseKeepTogether should be
    kept together
      ...
      case t2pitCloseKeepTogether:
        {
          // Clear the flag.
          keep_data_together = false;
          // To properly work with the layout engine's model, add an empty
    block.
...
// Lays out one paragraph.
{
  ...
  if (intrusion) {
    ...
    layoutcontainer->layout_drop_caps(intrusion,
  //input_paragraph->font_size_points);
  }
  ...
  if (//input_paragraph->space_before_mm != 0) {
    ...
    rectangle.x = 0;
    rectangle.y = 0;
    rectangle.width = page_width_pango_units - inside_margin_pango_units -
  outside_margin_pango_units;
    rectangle.height =
  millimeters_to_pango_units(//input_paragraph->space_before_mm);
    int column_count =
  get_column_count_rectangle_width(//input_paragraph->column_count,
  rectangle.width);
  ...
  while (!//input_paragraph->text.empty() && line_number < 1000) {
    ...
    layoutcontainer->layout_text(input_paragraph, line_number,
  ...
  outside_margin_pango_units;
    rectangle.height =
  millimeters_to_pango_units(//input_paragraph->space_after_mm);
    int column_count =
  get_column_count_rectangle_width(//input_paragraph->column_count,
  rectangle.width);
    T2PBlock *block = new T2PBlock(rectangle, column_count);
...
// Gets the next layout container to be used.
{
  /*
    ...
    rectangle.x = 0;
    rectangle.y = 0;
    // Only the width of the block is set, because the other sizes depend on
    factors not yet known at this stage.
    rectangle.width = page_width_pango_units - inside_margin_pango_units -
    outside_margin_pango_units;
    rectangle.height = 0;
    ...
    if (!input_blocks.empty()) {
      ...
      // Look 2 blocks back, because by default the intrusion, that's the
    drop-caps, is two lines high.
...
/*
 Widows are the first lines of a paragraph, while the rest is in another column
 or on another page.
...
{
  /*
  // Widows and orphans control is is implemented throught the "keep_with_next"
  property of the T2PBlock object.
  // The "keep with next" mechanism will then take over when the blocks are
  fitted in.
...
int XeTeX2Pdf::get_column_count_rectangle_width(int column_count_in,
                                                int &width) {
...
// Produces the next page and related objects.
{
  ...
  page = new T2PPage(page_number, page_width_pango_units,
  page_height_pango_units, inside_margin_pango_units,
  outside_margin_pango_units, top_margin_pango_units, bottom_margin_pango_units,
...
// Add text to whatever container is on top of the stack.
{
  ...
  * The usual alpha-numerical characters and punctuation marks can be used as is
  and behave as expected.
  * However, some symbols have a special meaning to TeX, and to use these
  characters, you have to (usually) "escape" them
  * by putting a backslash in front of the character.
  * For example, "$10" should be typed as "\$10", and 10% as 10\%.
  * (Note: TeX ignores blanks following an escaped character; if you want a
  blank (as in "10% of ..."),
  * you have to "escape" the blank as well ("10\%\ of ...").
  Braces and parentheses:
  * Brackets and round parentheses can be used as is and shouldn't be escaped,
  * but curly braces ("{","}") are used for grouping in TeX, and don't get
  printed.
  * To get curly braces in the output, you must use the escaped versions, "\{",
  and "\}".
  Quotation marks:
  * The usual double quotation marks don't come out correctly in TeX.
  * To get correct double quotes, surround the quoted phrase by a left and a
  right pair of single quotes (``TeX'').
  */
  // replace_text (text, "$", "\\$");
  // replace_text (text, "$ ", "$\\ ");
  // replace_text (text, "%", "\\%");
  ...
  // working here, reading gentle.pdf page 10. But later the ptx2pdf macros were
  // used instead.
  ...
  */
  ...
  // Store the text. this is not correct, it should be stored not as a line,
  // else it gets spaces where no spaces are intended.
  xetex_lines.push_back(text);
  // Add the text to the paragraph.
  ensure_open_paragraph();
  // input_paragraph->add_text(text);
  // Header suppression is done here, because only input paragraphs that have
  // text are considered in the suppression system.
  if (suppress_header_on_this_page) {
    // input_paragraph->suppress_header = true;
  }
  suppress_header_on_this_page = false;
  // References and referents are stored here, because only input paragraphs
  // with text are considered in the system.
...
/*
 ...
 Each paragraph will use the font that was current at the time that the input
 paragraph object was created.
 */
{
  font = font_in;
}
...
void XeTeX2Pdf::set_running_chapter_number(unsigned int left,
                                           unsigned int right)
...
// Used for creating a table of contents with page numbers.
{
  ...
  if (reference.empty())
    return;
  // Bail out if there's another paragraph, because the reference is only dealt
  with if it is found in the main paragraph.
  if (stacked_input_paragraph)
    return;
...
// Used for creating a table of contents with page numbers.
{
  ...
  if (referent.empty())
    return;
  // Bail out if there's another paragraph, because the referent is only dealt
  with if it is found in the main paragraph.
  if (stacked_input_paragraph)
    return;

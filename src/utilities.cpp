...
void quick_sort(vector<T1> &one, vector<T2> &two, unsigned int beg, unsigned int end) {
  if (end > beg + 1) {
    T1 piv = one[beg];
...
void quick_sort(vector<T1> &one, vector<bool> &two, unsigned int beg, unsigned int end) {
  if (end > beg + 1) {
    T1 piv = one[beg];
    unsigned int l = beg + 1;
    unsigned int r = end;

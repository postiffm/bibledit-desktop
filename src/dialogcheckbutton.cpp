...
CheckbuttonDialog::CheckbuttonDialog(const ustring &title, const ustring &info,
  ...
  gtk_window_set_position(GTK_WINDOW(checkbuttondialog),
  ...
  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(checkbuttondialog));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(checkbuttondialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(checkbuttondialog), okbutton,
                               GTK_RESPONSE_OK);
...
void CheckbuttonDialog::on_okbutton_clicked(GtkButton *button,
                                            gpointer user_data) {
...
void CheckbuttonDialog::on_okbutton() {
  ...
  for (unsigned int i = 0; i < checkbuttons.size(); i++) {
    bitpattern_add(bitpattern, gtk_toggle_button_get_active(
                                   GTK_TOGGLE_BUTTON(checkbuttons[i])));
  }
}

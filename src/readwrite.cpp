...
First parameter: -r|-w
This can be either -r or -w which determines whether the remaining arguments are
going to do a "read" operation
from the specified Bibledit-Gtk Bible / project, or do a "write" operation to
that Bible / project.

Second parameter: project
This gives the name of the Bibledit-Gtk Bible / project.
All we have to do is ensure that the project name we want to access is a
valid/existing one.

Third parameter: book
This is simply the 3-letter book code for the Bible book that is being
read/written to.
I.e., MAT for Matthew, GEN for Genesis, etc.

Fourth parameter: chapter|0
This can be either a chapter number or 0 (zero) for reading/writing either an
individual chapter
...
For a read (-r) operation this fileName argument is the name of the file that
will be created by bibledit-rdwrt
containing a copy of the whole book (corresponding to the 3-letter code),
or that contains the individual chapter contents (of a designated chapter) of an
existing Bibledit-Gtk book file
in the Bible / project. It should be prefixed with a path us.
Since bibledit-rdwrt is a console operation, after AdaptIt calls it using
::wxExecute,
it would need to read the resulting temporary file to grab the contents for its
use.
For a write (-w) operation this fileName argument is the name of the temporary
file that bibledit-rdwrt reads
...
single chapter
for the book specified by the book 3-letter code and the chapter (number)
argument.
...

{
  ...
  if (!project_book_exists(project, book)) {
    gw_critical(_("Book ") + string(argv[3]) +
                _(" does not exist in project ") + project);
    return;
  }

  // Fourth parameter: chapter | 0.
  // Bibledit-Gtk supports chapter 0.
  // But here it is different. Chapter 0 means to read/write the whole book, not
  // chapter 0.
  ...
  if (chapter > 0) {
    ...
    if (chapterset.find(chapter) == chapterset.end()) {
      gw_critical(_("Chapter ") + convert_to_string(chapter) +
                  _(" does not exist in project ") + project + _(" book ") +
                  string(argv[3]));
      return;
    }
  }

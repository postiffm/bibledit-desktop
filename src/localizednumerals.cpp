...
// Returns the filename that belongs to the localization of "language".
{
  ...
  if (full_filename.empty()) {
    full_filename =
        gw_build_filename(Directories->get_templates_user(), base_name);
    if (!g_file_test(full_filename.c_str(), G_FILE_TEST_IS_REGULAR)) {
      full_filename.clear();
    }
  }
...
bool NumeralLocalization::available() { return !localizations.empty(); }
...
// Also keeps the \c <latin numeral> and the \v <latin numeral>.
{
  ...
  while (!number.empty()) {
    ...
    // Look back for a space. If it is found then the numeral does not belong to
    // a USFM code, and the conversion can go ahead.
    // If no space is found, but a backslash instead, then the conversion cannot
    // go ahead since the numeral is part of a USFM code.
    if (convert) {

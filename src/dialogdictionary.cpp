...
DictionaryDialog::DictionaryDialog(const ustring &project) {
  ...
  gtk_window_set_position(GTK_WINDOW(dictionarydialog),
  ...
  label_info = gtk_label_new_with_mnemonic(
      _("To change the order in which a dictionary is consulted, drag it to "
        "another location."));
  ...
  gtk_box_pack_start(GTK_BOX(dialog_vbox1), treeview_dictionaries, TRUE, TRUE,
                     0);
  gtk_widget_set_size_request(treeview_dictionaries, -1, 400);
  gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(treeview_dictionaries),
                                    FALSE);
  ...
  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(dictionarydialog));
  gtk_widget_show(dialog_action_area1);
  gtk_button_box_set_layout(GTK_BUTTON_BOX(dialog_action_area1),
                            GTK_BUTTONBOX_END);

  new InDialogHelp(dictionarydialog, NULL, NULL,
                   "file/project/project-properties/dictionaries");

  cancelbutton1 = gtk_button_new_from_stock("gtk-cancel");
  gtk_widget_show(cancelbutton1);
  gtk_dialog_add_action_widget(GTK_DIALOG(dictionarydialog), cancelbutton1,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton1), true);

  okbutton1 = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton1);
  gtk_dialog_add_action_widget(GTK_DIALOG(dictionarydialog), okbutton1,
                               GTK_RESPONSE_OK);
  ...
  g_signal_connect((gpointer)button_edit, "clicked",
                   G_CALLBACK(on_button_edit_clicked), gpointer(this));
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview_dictionaries),
                          GTK_TREE_MODEL(model));
  g_object_unref(model);
  GtkCellRenderer *renderer = gtk_cell_renderer_text_new();
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(treeview_dictionaries), -1, "", renderer, "text",
      COLUMN_BOOK, NULL);
  gtk_tree_selection_set_mode(
      gtk_tree_view_get_selection(GTK_TREE_VIEW(treeview_dictionaries)),
...
DictionaryDialog::~DictionaryDialog() { gtk_widget_destroy(dictionarydialog); }

int DictionaryDialog::run() {
  return gtk_dialog_run(GTK_DIALOG(dictionarydialog));
}

void DictionaryDialog::on_okbutton1_clicked(GtkButton *button,
                                            gpointer user_data) {
...
void DictionaryDialog::on_button_add_clicked(GtkButton *button,
                                             gpointer user_data) {
...
void DictionaryDialog::on_button_add() {
  ...
  {
    vector<ustring> loaded_dictionaries =
        listview_get_strings(treeview_dictionaries);
    set<ustring> dictionary_set(loaded_dictionaries.begin(),
                                loaded_dictionaries.end());
...
void DictionaryDialog::on_button_delete_clicked(GtkButton *button,
                                                gpointer user_data) {
...
void DictionaryDialog::on_treeview_dictionaries_cursor_changed(
...
void DictionaryDialog::on_button_edit_clicked(GtkButton *button,
                                              gpointer user_data) {
...
void DictionaryDialog::on_button_edit() {
  // Edit dictionary.
  EditDictionaryDialog dialog(
      listview_get_active_string(treeview_dictionaries));
  dialog.run();
}

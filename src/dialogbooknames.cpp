...
BooknamesDialog::BooknamesDialog(const map <unsigned int, ustring>& books, const gchar * info, const gchar * heading2, GtkWindow *transient_parent)
{
  ...
  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(treeview1), -1, _("Book in Bibledit-Desktop"), renderer1, "text", COLUMN_BIBLEDIT, NULL);
  ...
  for (unsigned int i = 0; i < ids.size(); i++) {
    ...
    gtk_list_store_set(model, &iter, COLUMN_BIBLEDIT, books_id_to_localname(ids[i]).c_str(), COLUMN_EDITABLE, 1, COLUMN_URL, text.c_str(), -1);
  }

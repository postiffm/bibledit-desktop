...
BooknamesDialog::BooknamesDialog(const map<unsigned int, ustring> &books,
                                 const gchar *info, const gchar *heading2) {
  // Shortcuts.
  Shortcuts shortcuts(0);

  resourcebooksdialog = gtk_dialog_new();
  gtk_window_set_title(GTK_WINDOW(resourcebooksdialog), _("Books"));
  gtk_window_set_position(GTK_WINDOW(resourcebooksdialog),
                          GTK_WIN_POS_CENTER_ON_PARENT);
  gtk_window_set_modal(GTK_WINDOW(resourcebooksdialog), TRUE);
  gtk_window_set_type_hint(GTK_WINDOW(resourcebooksdialog),
  ...
  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(resourcebooksdialog));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(resourcebooksdialog), cancelbutton1,
                               GTK_RESPONSE_CANCEL);
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(resourcebooksdialog), okbutton1,
                               GTK_RESPONSE_OK);
  ...
  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(treeview1), -1,
                                              "Book in Bibledit-Gtk", renderer1,
                                              "text", COLUMN_BIBLEDIT, NULL);
  GtkCellRenderer *renderer2 = gtk_cell_renderer_text_new();
  g_signal_connect(renderer2, "edited", G_CALLBACK(cell_edited),
                   gpointer(this));
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(treeview1), -1, heading2, renderer2, "text", COLUMN_URL,
  ...
  for (unsigned int i = 0; i < ids.size(); i++) {
    ...
    gtk_list_store_set(model, &iter, COLUMN_BIBLEDIT,
                       books_id_to_english(ids[i]).c_str(), COLUMN_EDITABLE, 1,
                       COLUMN_URL, text.c_str(), -1);
  }

  new DialogAutoScaler(resourcebooksdialog, G_MAXINT);
}

BooknamesDialog::~BooknamesDialog() { gtk_widget_destroy(resourcebooksdialog); }

int BooknamesDialog::run() {
  return gtk_dialog_run(GTK_DIALOG(resourcebooksdialog));
}

void BooknamesDialog::on_okbutton1_clicked(GtkButton *button,
                                           gpointer user_data) {
...
void BooknamesDialog::on_okbutton() {
  ...
  while (valid) {
    ...
    gchar *str_abbrev;
    gchar *str_text;
    gtk_tree_model_get(GTK_TREE_MODEL(model), &iter, COLUMN_BIBLEDIT,
                       &str_abbrev, COLUMN_URL, &str_text, -1);
...
void BooknamesDialog::cell_edited(GtkCellRendererText *cell,
                                  const gchar *path_string,
                                  const gchar *new_text, gpointer data) {
  ((BooknamesDialog *)data)->on_cell_edited(cell, path_string, new_text);
}

void BooknamesDialog::on_cell_edited(GtkCellRendererText *cell,
                                     const gchar *path_string,
  ...
  gtk_list_store_set(GTK_LIST_STORE(model), &iter, COLUMN_URL,
                     g_strdup(new_text), -1);
  // Free path.
  gtk_tree_path_free(path);
}

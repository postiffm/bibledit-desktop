...
EditNoteDialog::EditNoteDialog(Editor2 *editor) {
  ...
  gtk_window_set_position(GTK_WINDOW(editnotedialog),
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(editnotedialog), cancelbutton1,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton1), true);

  okbutton1 = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton1);
  gtk_dialog_add_action_widget(GTK_DIALOG(editnotedialog), okbutton1,
                               GTK_RESPONSE_OK);
  ...
  note_paragraph = static_cast<EditorActionCreateNoteParagraph *>(
      myeditor->focused_paragraph);
  ...
  if (note_paragraph) {
    ...
    ustring character = "*";
    biblenotes_get_note_numbering(note_paragraph->caller_usfm, numberingtype,
                                  character);
...
EditNoteDialog::~EditNoteDialog() { gtk_widget_destroy(editnotedialog); }

int EditNoteDialog::run() { return gtk_dialog_run(GTK_DIALOG(editnotedialog)); }

void EditNoteDialog::on_radiobutton_numbering_toggled(
...
void EditNoteDialog::on_okbutton1_clicked(GtkButton *button,
                                          gpointer user_data) {
...
void EditNoteDialog::on_okbutton() {
  ...
  EditorNoteType oldnotetype = note_type_get(
      myeditor->project_get(), note_paragraph->opening_closing_marker);

  // Get and store the new note marker and type.
  note_paragraph->opening_closing_marker =
      get_marker(combobox_get_active_string(combobox1));
  EditorNoteType newnotetype = note_type_get(
      myeditor->project_get(), note_paragraph->opening_closing_marker);
  ...
  note_paragraph->caller_usfm =
      biblenotes_get_note_caller(numberingtype, character);
    ...
    do {
      ...
      if (!paragraph_style.empty()) {
        gtk_text_buffer_remove_tag_by_name(
            note_paragraph->textbuffer, paragraph_style.c_str(), &iter, &iter2);
      }
      if (!character_style.empty()) {
        gtk_text_buffer_remove_tag_by_name(
            note_paragraph->textbuffer, character_style.c_str(), &iter, &iter2);
      }
      if (!paragraph_style.empty()) {
        textbuffer_apply_named_tag(
            note_paragraph->textbuffer,
            switch_marker(paragraph_style, switch_to_xref), &iter, &iter2);
      }
      if (!character_style.empty()) {
        textbuffer_apply_named_tag(
            note_paragraph->textbuffer,
            switch_marker(character_style, switch_to_xref), &iter, &iter2);
      }
...
ustring EditNoteDialog::switch_marker(const ustring &marker,
                                      bool switch_to_xref)
/*

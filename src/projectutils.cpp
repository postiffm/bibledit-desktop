...
void project_store_chapter_internal(const ustring &project, unsigned int book,
  ...
  // Store data.
  try {
    ustring chapterdirectory =
        project_data_directory_chapter(project, book, chapter);
...
{
  // Ensure that we create separate routines for each Bible, as this routine
  // will need to be called upon restore as well.
}
...
{
  ustring directory = gw_build_filename(project_data_directory_project(project),
                                        books_id_to_english(book));
  return directory;
}

ustring project_data_directory_chapter(const ustring &project,
                                       unsigned int book, unsigned int chapter)
// Returns the data directory for the chapter, e.g.:
// ~/.bibledit/projects/testproject/data/Genesis/1
{
  ustring directory = gw_build_filename(
      project_data_directory_book(project, book), convert_to_string(chapter));
  return directory;
}

ustring project_data_filename_chapter(const ustring &project, unsigned int book,
                                      unsigned int chapter, bool checkexists)
...
// ~/.bibledit/projects/testproject/data/Genesis/1/data
{
  ustring filename = gw_build_filename(
      project_data_directory_chapter(project, book, chapter), "data");
  if (checkexists) {
    if (!g_file_test(filename.c_str(), G_FILE_TEST_IS_REGULAR))
      throw runtime_error(books_id_to_english(book) + " " +
                          convert_to_string(chapter) + _(" does not exist"));
  }
  return filename;
}
...
bool project_exists(const ustring &project) {
  return g_file_test(
      gw_build_filename(Directories->get_projects(), project).c_str(),
      G_FILE_TEST_IS_DIR);
}

void project_create_restore(const ustring &project,
                            const ustring &restore_from) {
...
// Copies "project" to "newproject.
{
  ustring olddirectory =
      gw_build_filename(Directories->get_projects(), project);
  ustring newdirectory =
      gw_build_filename(Directories->get_projects(), newproject);
  ...
  // Use of a remote repository is switched off so as not to mess with our
  // server.
...
void project_store_book(const ustring &project, unsigned int book,
                        CategorizeChapterVerse &ccv) {
  ...
  for (unsigned int i = 0; i < chapters.size(); i++) {
    project_store_chapter_internal(project, book, chapters[i], ccv, firsts[i],
                                   lasts[i], timestamp);
  }
}
...
void project_store_chapter(const ustring &project, unsigned int book,
                           CategorizeChapterVerse &ccv) {
  ...
  unsigned int timestamp = time(0);
  project_store_chapter_internal(project, book, chapter, ccv, 0,
                                 ccv.chapter.size(), timestamp);
}

void project_remove_chapter(const ustring &project, unsigned int book,
                            unsigned int chapter) {
  // Statistics.
...
void project_store_verse(const ustring &project, unsigned int book,
  ...
  // Retrieve current data, categorize it, store new verse at right place, store
  // it.
...
ustring project_retrieve_verse_extended(const ustring &project,
                                        unsigned int book, unsigned int chapter,
                                        const ustring &verse)
/*
...
*/
{
  ...
  for (unsigned int a = 0; a < available_verses_sets.size(); a++) {
    for (unsigned int r = 0; r < requested_verses.size(); r++) {
      if (available_verses_sets[a].find(requested_verses[r]) !=
          available_verses_sets[a].end()) {
        if (retrieved_verses.find(all_verses[a]) == retrieved_verses.end()) {
          retrieved_verses.insert(all_verses[a]);
          if (!line.empty())
            line.append("\n");
          line.append(
              project_retrieve_verse(project, book, chapter, all_verses[a]));
        }
      }
    }
  }
...
ustring project_retrieve_verse(const ustring &project, unsigned int book,
                               unsigned int chapter, const ustring &verse) {
  // Retrieve the verse.
  ustring line;
  vector<ustring> lines = project_retrieve_chapter(project, book, chapter);
...
vector<ustring> project_retrieve_chapter(const ustring &project,
                                         unsigned int book,
                                         unsigned int chapter)
// Get the chapter from disk.
{
  ...
  // retrieve the information from that project in case that project was
  // updated.
  // If the file stamp of the source project is newer, then that means it was
  // updated.
  ...
  if (projectconfig->depending_on_switch_get()) {
    ...
    if (!source_project.empty()) {
      ...
      ustring sourcefilename =
          project_data_filename_chapter(source_project, book, chapter, false);
      unsigned int sourcefiledate = file_get_modification_time(sourcefilename);
      if (sourcefiledate >= myfiledate) {
        script_filter(script, script == scripts_straight_through(),
                      sourcefilename, script_temporal_output_file());
...
vector<unsigned int> project_get_chapters(const ustring &project,
                                          unsigned int book)
...
vector<ustring> project_get_verses(const ustring &project, unsigned int book,
                                   unsigned int chapter)
...
// Returns true if a project was chosen.
{
  ...
  ListviewDialog dialog(_("Open project"), editable_projects, focusedproject,
                        true, NULL);
  dialog.two_pages(_("Editable"), _("Non-editable"), non_editable_projects,
                   true);
  dialog.tab_switcher(GDK_KEY_O, GDK_CONTROL_MASK, _("Ctrl-O"),
                      _("to display the editable projects"),
                      _("to display the non-editable projects"));

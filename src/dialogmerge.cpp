...
MergeDialog::MergeDialog(const ustring &text) {
  ...
  gtk_window_set_position(GTK_WINDOW(mergedialog),
                          GTK_WIN_POS_CENTER_ON_PARENT);
  gtk_window_set_modal(GTK_WINDOW(mergedialog), TRUE);
  gtk_window_set_default_size(
      GTK_WINDOW(mergedialog),
      (int)(settings->genconfig.screen_width_get() * 0.8),
      (int)(settings->genconfig.screen_height_get() * 0.9));

  dialog_vbox1 = gtk_dialog_get_content_area(GTK_DIALOG(mergedialog));
  gtk_widget_show(dialog_vbox1);

  label = gtk_label_new(
      _("The merge operation needs manual intervention. Each place that needs "
        "intervention shows two buttons with the two alternatives. Please "
        "press the buttons with the correct text."));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(mergedialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(mergedialog), okbutton,
                               GTK_RESPONSE_OK);
  ...
  g_signal_connect_after((gpointer)textbuffer, "changed",
...
MergeDialog::~MergeDialog() { gtk_widget_destroy(mergedialog); }

int MergeDialog::run() { return gtk_dialog_run(GTK_DIALOG(mergedialog)); }
...
void MergeDialog::load_text(ustring text) {
  ...
  while (pos != string::npos) {
    ...
    gtk_text_buffer_get_end_iter(textbuffer, &iter);
    GtkTextChildAnchor *childanchor1 =
        gtk_text_buffer_create_child_anchor(textbuffer, &iter);
    GtkWidget *button1 = gtk_button_new_with_label(alternative1.c_str());
    gtk_text_view_add_child_at_anchor(GTK_TEXT_VIEW(textview), button1,
                                      childanchor1);
    gtk_widget_show_all(button1);
    g_signal_connect((gpointer)button1, "clicked",
                     G_CALLBACK(on_mergebutton_clicked), gpointer(this));
    ...
    gtk_text_buffer_get_end_iter(textbuffer, &iter);
    GtkTextChildAnchor *childanchor2 =
        gtk_text_buffer_create_child_anchor(textbuffer, &iter);
    GtkWidget *button2 = gtk_button_new_with_label(alternative2.c_str());
    gtk_text_view_add_child_at_anchor(GTK_TEXT_VIEW(textview), button2,
                                      childanchor2);
...
void MergeDialog::on_textbuffer_changed(GtkTextBuffer *textbuffer,
                                        gpointer user_data) {
...
void MergeDialog::on_textbuffer() {
  ...
  event_textbuffer = g_timeout_add_full(G_PRIORITY_DEFAULT, 200,
                                        GSourceFunc(on_textbuffer_delayed),
                                        gpointer(this), NULL);
}
...
void MergeDialog::on_mergebutton_clicked(GtkButton *button,
                                         gpointer user_data) {
  ...
  for (unsigned int i = 0; i < buttonpairs.size(); i++) {
    ...
    if (match) {
      ...
      gtk_text_buffer_get_iter_at_child_anchor(
          textbuffer, &iter, buttonpairs[i].button1.childanchor);
      gtk_text_buffer_place_cursor(textbuffer, &iter);
      iter2 = iter;
      gtk_text_iter_forward_char(&iter2);
      gtk_text_buffer_delete(textbuffer, &iter, &iter2);
      gtk_text_buffer_get_iter_at_child_anchor(
          textbuffer, &iter, buttonpairs[i].button2.childanchor);
      iter2 = iter;
      gtk_text_iter_forward_char(&iter2);
      gtk_text_buffer_delete(textbuffer, &iter, &iter2);
      if (!text.empty())
...
__conflict__marker__before__ servant of Jesus Christ and
__conflict__marker__middle__ __conflict__marker__after__

This function handles this case so that it does not lead to confusion.
*/
{
  replace_text(text, merge_conflict_markup(1) + " " + merge_conflict_markup(2),
               merge_conflict_markup(1) + "  " + merge_conflict_markup(2));
  replace_text(text, merge_conflict_markup(2) + " " + merge_conflict_markup(3),
               merge_conflict_markup(2) + "  " + merge_conflict_markup(3));
}

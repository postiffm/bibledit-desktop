...
unsigned int color_gdk_to_decimal(GdkColor *color) {
  unsigned int result;
  result = (65536 * (color->red / 256)) + (256 * (color->green / 256)) +
           (color->blue / 256);
  return result;
}
...
void color_widget_set(GtkWidget *widget, unsigned int normal_text,
                      unsigned int background, unsigned int selected_text,

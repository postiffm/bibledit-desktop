...
ListviewDialog::ListviewDialog(const ustring &title, vector<ustring> &list,
                               const ustring &focus, bool sortlist, gchar *help)
...
// If the user selects one out of that, it returns it in "focus".
{
  ...
  gtk_window_set_position(GTK_WINDOW(listviewdialog),
  ...
  label1 = gtk_label_new(_("Type the first couple of characters of the item, "
  ...
  gtk_notebook_set_tab_label(
      ...
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(notebook1), 0), label_tab_1);
  ...
  column2 =
      gtk_tree_view_column_new_with_attributes("", renderer2, "text", 0, NULL);
  ...
  gtk_notebook_set_tab_label(
      GTK_NOTEBOOK(notebook1),
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(notebook1), 1), label_tab_2);
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(listviewdialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(listviewdialog), okbutton,
                               GTK_RESPONSE_OK);
  gtk_widget_set_can_default(GTK_WIDGET(okbutton), true);

  g_signal_connect_after((gpointer)notebook1, "switch_page",
                         G_CALLBACK(on_notebook1_switch_page), gpointer(this));
  g_signal_connect((gpointer)treeview1, "row_activated",
                   G_CALLBACK(on_treeview1_row_activated), gpointer(this));
...
ListviewDialog::~ListviewDialog() { gtk_widget_destroy(listviewdialog); }

void ListviewDialog::two_pages(const gchar *tab1, const gchar *tab2,
                               vector<ustring> &list, bool sortlist) {
  // Modify GUI.
...
void ListviewDialog::tab_switcher(guint accel_key, GdkModifierType accel_mods,
  ...
  g_signal_connect((gpointer)listviewdialog, "activate_focus",
                   G_CALLBACK(on_activate_focus), gpointer(this));
...
int ListviewDialog::run() { return gtk_dialog_run(GTK_DIALOG(listviewdialog)); }

void ListviewDialog::on_treeview1_row_activated(GtkTreeView *treeview,
                                                GtkTreePath *path,
...
void ListviewDialog::static_on_okbutton_clicked(GtkButton *button,
                                                gpointer user_data) {
...
void ListviewDialog::on_notebook1_switch_page(GtkNotebook *notebook,

...
ImportAssistant::ImportAssistant(WindowReferences *references_window,
                                 ...
                                 WindowsOutpost *windows_outpost)
...
// Import assistant.
{
  ...
  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), vbox_select_type,
                               _("What would you like to import?"));
  ...
  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), vbox_bible_name,
                               _("Is this the right Bible to import into?"));
  ...
  gtk_assistant_set_page_title(
      GTK_ASSISTANT(assistant), vbox_bible_type,
      _("What type of data would you like to import?"));
  ...
  radiobutton_bible_bibleworks = gtk_radio_button_new_with_mnemonic(
      NULL, _("BibleWorks Exported Database"));
  ...
  radiobutton_bible_online_bible =
      gtk_radio_button_new_with_mnemonic(NULL, _("Online Bible Text"));
  gtk_widget_show(radiobutton_bible_online_bible);
  gtk_box_pack_start(GTK_BOX(vbox_bible_type), radiobutton_bible_online_bible,
                     FALSE, FALSE, 0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_bible_online_bible),
                             radiobutton_bible_type_group);
  radiobutton_bible_type_group = gtk_radio_button_get_group(
      GTK_RADIO_BUTTON(radiobutton_bible_online_bible));

  radiobutton_bible_raw_text =
      gtk_radio_button_new_with_mnemonic(NULL, _("Raw Text"));
  gtk_widget_show(radiobutton_bible_raw_text);
  gtk_box_pack_start(GTK_BOX(vbox_bible_type), radiobutton_bible_raw_text,
                     FALSE, FALSE, 0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_bible_raw_text),
                             radiobutton_bible_type_group);
  radiobutton_bible_type_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_bible_raw_text));
  ...
  page_number_online_bible_running = gtk_assistant_append_page(
      GTK_ASSISTANT(assistant), label_online_bible_running);

  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant),
                               label_online_bible_running,
                               _("Connected to the Online Bible?"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant),
                              label_online_bible_running,
                              GTK_ASSISTANT_PAGE_CONTENT);
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant),
                                  label_online_bible_running, false);

  // In case there's no data in the project, the Online Bible won't get
  // connected.
  ...
  page_number_online_bible_bible = gtk_assistant_append_page(
      GTK_ASSISTANT(assistant), vbox_online_bible_bible);

  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant),
                               vbox_online_bible_bible,
                               _("Which Bible would you like to import?"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant), vbox_online_bible_bible,
                              GTK_ASSISTANT_PAGE_CONTENT);
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant),
                                  vbox_online_bible_bible, false);

  combobox_online_bible_bible = gtk_combo_box_new_text();
  gtk_widget_show(combobox_online_bible_bible);
  gtk_box_pack_start(GTK_BOX(vbox_online_bible_bible),
                     combobox_online_bible_bible, false, false, 0);

  g_signal_connect((gpointer)combobox_online_bible_bible, "changed",
                   G_CALLBACK(on_combobox_online_bible_bible_changed),
                   gpointer(this));
  ...
  page_number_files =
      gtk_assistant_append_page(GTK_ASSISTANT(assistant), vbox_files);
  gtk_container_set_border_width(GTK_CONTAINER(vbox_files), 10);

  gtk_assistant_set_page_title(
      GTK_ASSISTANT(assistant), vbox_files,
      _("Which file or files would you like to import?"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant), vbox_files,
                              GTK_ASSISTANT_PAGE_CONTENT);
  ...
  g_signal_connect((gpointer)button_files, "clicked",
                   G_CALLBACK(on_button_files_clicked), gpointer(this));
  ...
  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), label_confirm,
                               _("The import is about to be done"));
...
void ImportAssistant::on_assistant_prepare_signal(GtkAssistant *assistant,
                                                  GtkWidget *page,
                                                  gpointer user_data) {
...
void ImportAssistant::on_assistant_prepare(GtkWidget *page) {
  ...
  if (page == label_online_bible_running) {
    if (my_windows_outpost->online_bible_server_connected) {
      gtk_label_set_text(GTK_LABEL(label_online_bible_running),
                         _("Yes, connected to the Online Bible"));
    } else {
      gtk_label_set_text(GTK_LABEL(label_online_bible_running),
                         _("No, could not connect to the Online Bible\nIs the "
                           "Online Bible running?"));
    }
    gtk_assistant_set_page_complete(
        GTK_ASSISTANT(assistant), label_online_bible_running,
        my_windows_outpost->online_bible_server_connected);
  }
  ...
  if (page == vbox_online_bible_bible) {
    ...
    if (bibles.empty()) {
      ustring response =
          my_windows_outpost->OnlineBibleDirectCommandResponseGet(
              "GetVersionList");
  ...
  if (page == vbox_files) {
    ...
    gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant), vbox_files,
                                    files_messages.empty());
  }
  ...
  if (page == label_summary) {
    ...
    gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant), label_summary,
                                    summary_messages.empty());
  }
}

void ImportAssistant::on_assistant_apply_signal(GtkAssistant *assistant,
                                                gpointer user_data) {
...
void ImportAssistant::on_assistant_apply() {
  ...
  case itBible: {
    ...
    case ibtUsfm: {
      ...
      for (unsigned int i = 0; i < files_names.size(); i++) {
        ...
        import_usfm_file(files_names[i], files_book_ids[i], bible_name,
                         summary_messages);
      }
      break;
    }
    ...
    case ibtOnlineBible: {
      import_online_bible(my_windows_outpost, combobox_get_active_string(
                                                  combobox_online_bible_bible),
                          bible_name, summary_messages);
      break;
    }
    case ibtRawText: {
      summary_messages.push_back(
          _("Importing raw text is a manual process. The online help provides "
  ...
  case itReferences: {
    summary_messages.push_back(
        _("Importing references is done in the references window."));
    summary_messages.push_back(_("In that window, click on [actions], then "
                                 "click on \"Import a list of references\"."));
    break;
  }
  case itStylesheet: {
    if (my_styles_window) {
  ...
  case itNotes: {
    summary_messages.push_back(_("After this window closes an opportunity will "
                                 "be offered to import notes."));
    import_notes = true;
    break;
  }
  case itKeyterms: {
    summary_messages.push_back(_("After this window closes an opportunity will "
                                 "be offered to import keyterms."));
...
gint ImportAssistant::assistant_forward_function(gint current_page,
                                                 gpointer user_data) {
...
void ImportAssistant::on_button_bible_name_clicked(GtkButton *button,
                                                   gpointer user_data) {
...
void ImportAssistant::on_button_files_clicked(GtkButton *button,
                                              gpointer user_data) {
...
// Selection and processing of the files to import.
{
  ...
  // Select files.
  {
    vector<ustring> files =
        gtkw_file_chooser_open_multiple(assistant, "", directory);
    if (!files.empty()) {
      files_names = files;
    }
  }
  ...
  if (!compressed_files.empty()) {
    if (compressed_files.size() > 1) {
      files_messages.push_back(
          _("You have selected more than one compressed file"));
    }
    if (compressed_files.size() == 1) {
      if (files_names.size() != 1) {
        files_messages.push_back(
            _("You have selected a mixture of normal and compressed files"));
      }
    }
  }

  // Optionally uncompress the archive and let the user select files from within
  // it.
  if (files_messages.empty()) {
    if (compressed_files.size() == 1) {
      ustring unpack_directory =
          gw_build_filename(Directories->get_temp(), "uncompress");
      unix_rmdir(unpack_directory);
      uncompress(compressed_files[0], unpack_directory);
      gtkw_dialog_info(assistant, _("You will now be asked to select files "
                                    "from within the compressed archive"));
      files_names =
          gtkw_file_chooser_open_multiple(assistant, "", unpack_directory);
    }
  }
  ...
  if (files_messages.empty()) {
    ...
    if (!non_unicode_files.empty()) {
      files_messages.push_back(
          _("The following files are not in the right Unicode format and are "
            "therefore not fit for import:"));
      for (unsigned int i = 0; i < non_unicode_files.size(); i++) {
        files_messages.push_back(non_unicode_files[i]);
      }
      files_messages.push_back(_("The online help provides more information "
                                 "about how to make these fit for use."));
    }
  }
  ...
  if (files_messages.empty()) {
    ...
    case itBible: {
      switch (get_bible_type()) {
      case ibtUsfm: {
        import_check_usfm_files(files_names, files_book_ids, bible_name,
                                files_messages);
        break;
      }
      case ibtBibleWorks: {
        import_check_bibleworks_file(files_names, files_book_ids, bible_name,
                                     files_messages);
        break;
      }
      case ibtOnlineBible: {
...
ImportBibleType ImportAssistant::get_bible_type() {
  ...
  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(radiobutton_bible_online_bible))) {
    return ibtOnlineBible;
  }
  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(radiobutton_bible_raw_text))) {
    return ibtRawText;
  }
  return ibtUsfm;
}

void ImportAssistant::on_combobox_online_bible_bible_changed(

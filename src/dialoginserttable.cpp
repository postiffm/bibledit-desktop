...
InsertTableDialog::InsertTableDialog(const ustring &project) {
  ...
  spinbutton_rows =
      gtk_spin_button_new(GTK_ADJUSTMENT(spinbutton_rows_adj), 1, 0);
  ...
  spinbutton_columns =
      gtk_spin_button_new(GTK_ADJUSTMENT(spinbutton_columns_adj), 1, 0);
  ...
  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(inserttabledialog));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(inserttabledialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(inserttabledialog), okbutton,
                               GTK_RESPONSE_OK);
...
void InsertTableDialog::on_okbutton_clicked(GtkButton *button,
                                            gpointer user_data) {
...
void InsertTableDialog::on_okbutton() {
  unsigned int rows =
      gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(spinbutton_rows));
  unsigned int columns =
      gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(spinbutton_columns));

  for (unsigned int row = 0; row < rows; row++) {
    rawtext.append(
        usfm_get_full_opening_marker(style_get_table_row_marker(myproject)));
    for (unsigned int column = 1; column <= columns; column++) {
      rawtext.append(usfm_get_full_opening_marker(
          style_get_table_cell_marker(myproject, column)));
    }
  }
}

...
T2PPage::T2PPage(int page_number, int page_width, int page_height,
                 int inside_margin, int outside_margin, int top_margin,
                 int bottom_margin, int header_height, int footer_height,
                 cairo_t *cairo_in, bool print_date_in, bool right_to_left_in)
 ...
 */
{
  ...
  // Create the reference area for the header.
  {
    ...
    header_reference_area =
        new T2PReferenceArea(rectangle, cairo, right_to_left);
  }
  ...
  // Create the reference area for the text.
  {
    ...
    rectangle.width = page_width - inside_margin - outside_margin;
    rectangle.height = page_height - top_margin - header_height -
...
// Prints the page.
{
  ...
  if (text_reference_area->has_content()) {
    header_reference_area->print(
        number, print_date, text_reference_area->left_running_header(),
        text_reference_area->right_running_header(),
        text_reference_area->suppress_headers(),
        text_reference_area->left_running_first_chapter(),
        text_reference_area->left_running_last_chapter(),
        text_reference_area->right_running_first_chapter(),
        text_reference_area->right_running_last_chapter());
  }

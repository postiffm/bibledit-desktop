...
enum EditorActionApplication { eaaInitial, eaaUndo, eaaRedo };
...
public:
  ...
  void apply(GtkTextTagTable *texttagtable, bool editable,
...
public:
  EditorActionChangeParagraphStyle(const ustring &style,
                                   EditorActionCreateParagraph *parent_action);
...
public:
  EditorActionInsertText(EditorActionCreateParagraph *parent_action,
                         gint offset_in, const ustring &text_in);
...
public:
  EditorActionDeleteText(EditorActionCreateParagraph *parent_action,
...
public:
  EditorActionChangeCharacterStyle(EditorActionCreateParagraph *parent_action,
                                   const ustring &style_in, gint offset_in,
                                   gint length_in);
...
private:
  ...
  void change_styles(const vector<ustring> &old_ones,
                     const vector<ustring> &new_ones);
};
...
public:
  EditorActionCreateNoteParagraph(GtkWidget *vbox, const ustring &marker_in,
                                  const ustring &caller_usfm_in,
                                  const ustring &caller_text_in,
                                  const ustring &identifier_in);
...
private:
  ustring caller_text;
  GtkWidget *label;
  static gboolean on_caller_enter_notify_event(GtkWidget *widget,
                                               GdkEventCrossing *event,
                                               gpointer user_data);
  gboolean on_caller_enter_notify(GdkEventCrossing *event);
  static gboolean on_caller_leave_notify_event(GtkWidget *widget,
                                               GdkEventCrossing *event,
                                               gpointer user_data);

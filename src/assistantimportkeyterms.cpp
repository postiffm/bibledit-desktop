...
// Assistant for adding keyterms.
{
  ...
  introduction(_("Text files that contain keyterms can be imported into the "
                 "keyterms database."));
  ...
  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), vbox_select_file,
                               _("Which file would you like to import?"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant), vbox_select_file,
                              GTK_ASSISTANT_PAGE_CONTENT);

  label5 = gtk_label_new(
      _("The file that will be imported should satisfy certain conditions. The "
  ...
  g_signal_connect((gpointer)button_open, "selection-changed",
                   G_CALLBACK(on_file_chooser_open_file_activated),
                   gpointer(this));
  ...
  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), vbox_type,
                               _("What type of file are you importing?"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant), vbox_type,
                              GTK_ASSISTANT_PAGE_CONTENT);
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant), vbox_type, true);

  GSList *radiobutton_type_standard_group = NULL;

  radiobutton_type_standard =
      gtk_radio_button_new_with_mnemonic(NULL, _("Standard textfile"));
  gtk_widget_show(radiobutton_type_standard);
  gtk_box_pack_start(GTK_BOX(vbox_type), radiobutton_type_standard, FALSE,
                     FALSE, 0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_type_standard),
                             radiobutton_type_standard_group);
  radiobutton_type_standard_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_type_standard));

  radiobutton_type_otkey_db =
      gtk_radio_button_new_with_mnemonic(NULL, "OTKEY.DB");
  gtk_widget_show(radiobutton_type_otkey_db);
  gtk_box_pack_start(GTK_BOX(vbox_type), radiobutton_type_otkey_db, FALSE,
                     FALSE, 0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_type_otkey_db),
                             radiobutton_type_standard_group);
  radiobutton_type_standard_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_type_otkey_db));

  radiobutton_type_ktref_db =
      gtk_radio_button_new_with_mnemonic(NULL, "KTREF.DB");
  gtk_widget_show(radiobutton_type_ktref_db);
  gtk_box_pack_start(GTK_BOX(vbox_type), radiobutton_type_ktref_db, FALSE,
                     FALSE, 0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_type_ktref_db),
                             radiobutton_type_standard_group);
  radiobutton_type_standard_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_type_ktref_db));

  radiobutton_type_ktbh = gtk_radio_button_new_with_mnemonic(
      NULL, _("Key Terms in Biblical Hebrew Project"));
  gtk_widget_show(radiobutton_type_ktbh);
  gtk_box_pack_start(GTK_BOX(vbox_type), radiobutton_type_ktbh, FALSE, FALSE,
                     0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_type_ktbh),
                             radiobutton_type_standard_group);
  radiobutton_type_standard_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_type_ktbh));
  ...
  gtk_assistant_set_page_title(GTK_ASSISTANT(assistant), vbox_category,
                               _("Into which collection will you import?"));
  gtk_assistant_set_page_type(GTK_ASSISTANT(assistant), vbox_category,
                              GTK_ASSISTANT_PAGE_CONTENT);
  ...
  g_signal_connect((gpointer)entry_category, "changed",
                   G_CALLBACK(on_entry_category_changed), gpointer(this));
...
void ImportKeytermsAssistant::on_assistant_apply_signal(GtkAssistant *assistant,
                                                        gpointer user_data) {
...
void ImportKeytermsAssistant::on_assistant_apply() {
  ustring filename =
      gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(button_open));
  ustring category = gtk_entry_get_text(GTK_ENTRY(entry_category));
  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(radiobutton_type_standard))) {
    keyterms_import_textfile(filename, category);
  }
  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(radiobutton_type_otkey_db))) {
    keyterms_import_otkey_db(filename, category);
  }
  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(radiobutton_type_ktref_db))) {
...
gint ImportKeytermsAssistant::assistant_forward_function(gint current_page,
                                                         gpointer user_data) {
  return ((ImportKeytermsAssistant *)user_data)
...
void ImportKeytermsAssistant::on_file_chooser_open_file_activated(
    GtkFileChooser *chooser, gpointer user_data) {
...
void ImportKeytermsAssistant::on_file_chooser_open() {
  ustring filename =
      gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(button_open));
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant), vbox_select_file,
                                  !filename.empty());
}

void ImportKeytermsAssistant::on_entry_category_changed(GtkEditable *editable,
                                                        gpointer user_data) {
...
// Give appropriate information about entering the category.
{
  ...
  gtk_assistant_set_page_complete(GTK_ASSISTANT(assistant), vbox_category,
                                  (!category.empty()) && (!exists));
}

...
OpenDocument::OpenDocument(const ustring &project, const ustring &filename,
...
// Note: To create the template use zip -r template.odt *
  ...
  command.append(
      gw_build_filename(Directories->get_package_data(), "template.odt"));
  command.append(" -d ");
...
// General formatter for USFM lines given.
{
  ...
  for (unsigned int ln = 0; ln < lines.size(); ln++) {
    ...
    usfm_handle_inline_text(line, usfm_inline_markers, NULL, imOpenDocument,
                            NULL);
    // Signal new line.
    if (odttextparagraph)
    ...
    if (usfm->is_identifier(marker)) {
      ...
      // Because of dealing with portions to include/exclude, handle verse
      // first.
      ...
      ustring bookmark = resource_viewer_produce_anchor(
          anchor_book, anchor_chapter,
          convert_to_int(number_in_string(versenumber)));
      ...
      // Inline text, has been dealt with before (therefore should never occur
      // here).
      if (odttextparagraph)
...
void OpenDocument::generate_styles(xmlTextWriterPtr writer) {
  ...
  for (unsigned int i = 0; i < usfm->styles.size(); i++) {
    ...
    if (usfm->is_identifier(marker)) {
      ...
      case itBook:
        paragraph_style(writer, marker, usfm->name(), fontname, 12, 100, OFF,
                        OFF, OFF, OFF, "", 0, 0, 0, 0, 0, false,
                        usfm->userbool1());
        break;
      case itEncoding:
        break;
    ...
    if (usfm->is_starting_paragraph(marker)) {
      paragraph_style(writer, marker, usfm->name(), fontname, usfm->fontsize(),
                      100, usfm->italic(), usfm->bold(), usfm->underline(),
                      usfm->smallcaps(), usfm->justification(),
                      usfm->spacebefore(), usfm->spaceafter(),
                      usfm->leftmargin(), usfm->rightmargin(),
                      usfm->firstlineindent(), false, false);
    }
    if (usfm->is_inline_text(marker)) {
      span_style(writer, marker, usfm->name(), fontname, 100, usfm->italic(),
                 usfm->bold(), usfm->underline(), usfm->smallcaps(),
                 usfm->superscript(), usfm->color());
    }
    ...
    if (usfm->is_foot_endnote(marker)) {
      ...
      case fentContentWithEndmarker:
        span_style(writer, marker, usfm->name(), fontname, 100, usfm->italic(),
                   usfm->bold(), usfm->underline(), OFF, usfm->superscript(),
                   usfm->color());
        break;
      case fentParagraph:
        break;
    ...
    if (usfm->is_pagebreak(marker)) {
      paragraph_style(writer, marker, usfm->name(), fontname, 12, 100, OFF, OFF,
                      OFF, OFF, "", 0, 0, 0, 0, 0, false, true);
    }
  }
}
...
// This generates the file "styles.xml" in the OpenDocument.
{
  ...
  xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:page-width", "%.2fcm",
                                    settings->genconfig.paper_width_get());
  xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:page-height", "%.2fcm",
                                    settings->genconfig.paper_height_get());
  xmlTextWriterWriteAttribute(writer, BAD_CAST "style:print-orientation",
                              BAD_CAST "portrait");
  xmlTextWriterWriteAttribute(writer, BAD_CAST "style:num-format",
                              BAD_CAST "1");
  xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:margin-top", "%.2fcm",
                                    settings->genconfig.paper_top_margin_get());
  xmlTextWriterWriteFormatAttribute(
      writer, BAD_CAST "fo:margin-bottom", "%.2fcm",
      settings->genconfig.paper_bottom_margin_get());
  xmlTextWriterWriteFormatAttribute(
      writer, BAD_CAST "fo:margin-left", "%.2fcm",
      settings->genconfig.paper_inside_margin_get());
  xmlTextWriterWriteFormatAttribute(
      writer, BAD_CAST "fo:margin-right", "%.2fcm",
      settings->genconfig.paper_outside_margin_get());
  if (right_to_left) {
    xmlTextWriterWriteAttribute(writer, BAD_CAST "style:writing-mode",
                                BAD_CAST "rl-tb");
    xmlTextWriterWriteAttribute(writer, BAD_CAST "writing-mode",
                                BAD_CAST "rl-tb");
  }
  xmlTextWriterEndElement(writer);
  xmlTextWriterEndElement(writer);
  ...
  xmlTextWriterWriteAttribute(writer, BAD_CAST "style:name",
                              BAD_CAST "Standard");
  xmlTextWriterWriteAttribute(writer, BAD_CAST "style:page-layout-name",
                              BAD_CAST "pm1");
  xmlTextWriterEndElement(writer);
  xmlTextWriterEndElement(writer);
...
void OpenDocument::paragraph_style(
    xmlTextWriterPtr writer, const ustring &marker, const ustring &name,
    const ustring &fontname, double fontsize, int lineheight,
    const ustring &italic, const ustring &bold, const ustring &underline,
    const ustring &smallcaps, ustring justification, double spacebefore,
    double spaceafter, double leftmargin, double rightmargin,
    double firstlineindent, bool spancolumns, bool startpage) {
  // Style properties.
  xmlTextWriterStartElement(writer, BAD_CAST "style:style");
  {
    xmlTextWriterWriteAttribute(writer, BAD_CAST "style:name",
                                BAD_CAST marker.c_str());
    ustring marker_name = marker + " " + name;
    xmlTextWriterWriteAttribute(writer, BAD_CAST "style:display-name",
                                BAD_CAST marker_name.c_str());
    xmlTextWriterWriteAttribute(writer, BAD_CAST "style:family",
                                BAD_CAST "paragraph");
    xmlTextWriterWriteAttribute(writer, BAD_CAST "style:parent-style-name",
                                BAD_CAST "Standard");
    ...
    {
      if (lineheight != 100) {
        xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:line-height",
                                          "%d%%", lineheight);
      }
      if (!justification.empty()) {
        ...
        xmlTextWriterWriteAttribute(writer, BAD_CAST "fo:text-align",
                                    BAD_CAST justification.c_str());
        xmlTextWriterWriteAttribute(
            writer, BAD_CAST "style:justify-single-word", BAD_CAST "false");
      }
      if (spacebefore != 0) {
        xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:margin-top",
                                          "%.1fmm", spacebefore);
      }
      if (spaceafter != 0) {
        xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:margin-bottom",
                                          "%.1fmm", spaceafter);
      }
      if (leftmargin != 0) {
        xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:margin-left",
                                          "%.1fmm", leftmargin);
      }
      if (rightmargin != 0) {
        xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:margin-right",
                                          "%.1fmm", rightmargin);
      }
      if (firstlineindent != 0) {
        xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:text-indent",
                                          "%.1fmm", firstlineindent);
      }
      // Starting odd pages is not supported in OpenDocument 1.0.
      if (startpage) {
        xmlTextWriterWriteAttribute(writer, BAD_CAST "fo:break-before",
                                    BAD_CAST "page");
      }
    ...
    {
      xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:font-size",
                                        "%.1fpt", fontsize);
      xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:font-size-asian",
                                        "%.1fpt", fontsize);
      xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:font-size-complex",
                                        "%.1fpt", fontsize);
      if (!fontname.empty()) {
        xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "style:font-name",
                                          "%s", fontname.c_str());
      }
      if (italic == ON) {
        xmlTextWriterWriteAttribute(writer, BAD_CAST "fo:font-style",
                                    BAD_CAST "italic");
        xmlTextWriterWriteAttribute(writer, BAD_CAST "fo:font-style-asian",
                                    BAD_CAST "italic");
        xmlTextWriterWriteAttribute(writer, BAD_CAST "fo:font-style-complex",
                                    BAD_CAST "italic");
      }
      if (bold == ON) {
        xmlTextWriterWriteAttribute(writer, BAD_CAST "fo:font-weight",
                                    BAD_CAST "bold");
        xmlTextWriterWriteAttribute(writer, BAD_CAST "fo:font-weight-asian",
                                    BAD_CAST "bold");
        xmlTextWriterWriteAttribute(writer, BAD_CAST "fo:font-weight-complex",
                                    BAD_CAST "bold");
      }
      if (underline == ON) {
        xmlTextWriterWriteAttribute(
            writer, BAD_CAST "style:text-underline-style", BAD_CAST "solid");
        xmlTextWriterWriteAttribute(
            writer, BAD_CAST "style:text-underline-width", BAD_CAST "auto");
        xmlTextWriterWriteAttribute(writer,
                                    BAD_CAST "style:text-underline-color",
                                    BAD_CAST "font-color");
      }
      if (smallcaps == ON) {
        xmlTextWriterWriteAttribute(writer, BAD_CAST "fo:font-variant",
                                    BAD_CAST "small-caps");
      }
    }
    xmlTextWriterEndElement(writer);
...
void OpenDocument::span_style(xmlTextWriterPtr writer, const ustring &marker,
                              const ustring &name, const ustring &fontname,
                              double fontpercentage, ustring italic,
                              ustring bold, ustring underline,
                              ustring smallcaps, bool superscript,
                              unsigned int color) {
  // Open the style.
  xmlTextWriterStartElement(writer, BAD_CAST "style:style");
  {
    ...
    xmlTextWriterStartElement(writer, BAD_CAST "style:text-properties");
    {
      ...
      if ((underline == ON) || (underline == TOGGLE))
        ...
        underline = "none";
      xmlTextWriterWriteAttribute(writer, BAD_CAST "style:text-underline-style",
                                  BAD_CAST underline.c_str());
      xmlTextWriterWriteAttribute(writer, BAD_CAST "style:text-underline-width",
                                  BAD_CAST "auto");
      xmlTextWriterWriteAttribute(writer, BAD_CAST "style:text-underline-color",
                                  BAD_CAST "font-color");
      if ((smallcaps == ON) || (smallcaps == TOGGLE))
        ...
        smallcaps = "normal";
      xmlTextWriterWriteAttribute(writer, BAD_CAST "fo:font-variant",
                                  BAD_CAST smallcaps.c_str());
      if (superscript) {
        xmlTextWriterWriteAttribute(writer, BAD_CAST "style:text-position",
                                    BAD_CAST "super 58%");
      }
      if (color != 0) {
        xmlTextWriterWriteFormatAttribute(writer, BAD_CAST "fo:color", "#%s",
                                          color_decimal_to_hex(color).c_str());
      }
    }
    xmlTextWriterEndElement(writer);
  }
...
void OpenDocument::note_unformatted_markers(vector<ustring> &markers) {
  ...
  vector<ustring> temporal_markers(unformatted_markers.begin(),
                                   unformatted_markers.end());

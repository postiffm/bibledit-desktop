...
MaintenanceDialog::~MaintenanceDialog() { gtk_widget_destroy(dialog); }

int MaintenanceDialog::run() { return gtk_dialog_run(GTK_DIALOG(dialog)); }

gboolean MaintenanceDialog::on_navigation_policy_decision_requested(
  ...
  ((MaintenanceDialog *)user_data)
      ->navigation_policy_decision_requested(request, navigation_action,
                                             policy_decision);
  return true;
}

void MaintenanceDialog::navigation_policy_decision_requested(
    WebKitNetworkRequest *request, WebKitWebNavigationAction *navigation_action,
...
void MaintenanceDialog::load_webview(const gchar *url) {
  ...
  if (active_url == mechon_mamre_html_entry_url()) {
    ...
    htmlwriter.text_add(_(
        "Some actions may have great impact on Bibledit-Gtk. Use with care."));

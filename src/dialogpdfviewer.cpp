...
PDFViewerDialog::PDFViewerDialog(int dummy) {
  ...
  gtk_window_set_position(GTK_WINDOW(pdfviewerdialog),
  ...
  gtk_box_pack_start(GTK_BOX(dialog_vbox1), radiobutton_automatic, FALSE, FALSE,
                     0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_automatic),
                             radiobutton_automatic_group);
  radiobutton_automatic_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_automatic));
  ...
  gtk_box_pack_start(GTK_BOX(dialog_vbox1), radiobutton_manual, FALSE, FALSE,
                     0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_manual),
                             radiobutton_automatic_group);
  radiobutton_automatic_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_manual));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(pdfviewerdialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(pdfviewerdialog), okbutton,
                               GTK_RESPONSE_OK);
  ...
  if (settings->genconfig.pdf_viewer_automatic_get())
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(radiobutton_automatic),
                                 true);
  else
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(radiobutton_manual), true);
  gtk_entry_set_text(GTK_ENTRY(entry_path),
                     settings->genconfig.pdf_viewer_path_get().c_str());
  gtk_entry_set_text(GTK_ENTRY(entry_arguments),
                     settings->genconfig.pdf_viewer_arguments_get().c_str());
  gui();
}

PDFViewerDialog::~PDFViewerDialog() { gtk_widget_destroy(pdfviewerdialog); }

int PDFViewerDialog::run() {
  return gtk_dialog_run(GTK_DIALOG(pdfviewerdialog));
}

void PDFViewerDialog::on_radiobutton_toggled(GtkToggleButton *togglebutton,
                                             gpointer user_data) {
  ((PDFViewerDialog *)user_data)->on_radiobutton();
}

void PDFViewerDialog::on_radiobutton() { gui(); }

void PDFViewerDialog::on_okbutton_clicked(GtkButton *button,
                                          gpointer user_data) {
...
void PDFViewerDialog::on_okbutton() {
  extern Settings *settings;
  settings->genconfig.pdf_viewer_automatic_set(
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(radiobutton_automatic)));
  settings->genconfig.pdf_viewer_path_set(
      gtk_entry_get_text(GTK_ENTRY(entry_path)));
  settings->genconfig.pdf_viewer_arguments_set(
      gtk_entry_get_text(GTK_ENTRY(entry_arguments)));
}

void PDFViewerDialog::gui() {
  bool manual =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(radiobutton_manual));

...
FindNoteDialog::FindNoteDialog(int dummy) {
  ...
  gtk_window_set_position(GTK_WINDOW(findnotedialog),
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbutton_case),
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(findnotedialog), buttonfind,
                               GTK_RESPONSE_OK);
  gtk_widget_set_can_default(GTK_WIDGET(buttonfind), true);

  buttoncancel = gtk_button_new_from_stock("gtk-cancel");
  gtk_widget_show(buttoncancel);
  gtk_dialog_add_action_widget(GTK_DIALOG(findnotedialog), buttoncancel,
                               GTK_RESPONSE_CANCEL);
  ...
  g_signal_connect((gpointer)buttonfind, "clicked",
                   G_CALLBACK(findnotedialog_on_buttonfind_clicked),
                   gpointer(this));
...
FindNoteDialog::~FindNoteDialog() { gtk_widget_destroy(findnotedialog); }

int FindNoteDialog::run() { return gtk_dialog_run(GTK_DIALOG(findnotedialog)); }

void FindNoteDialog::findnotedialog_on_buttonfind_clicked(GtkButton *button,
                                                          gpointer user_data) {
...
void FindNoteDialog::on_buttonfind_clicked() {
  ...
  try {
    ...
    settings->session.search_case_sensitive =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(checkbutton_case));

    // Get the string to search for.
    // We need to normalize the search expression, as prescribed, when comparing
    // strings.
    ...
    for (unsigned int i = 0; i < reader.ustring0.size(); i++) {
      ...
      if (casefolded_note.find(searchword_casefold) != string::npos) {
        ...
        int date_modified;
        ustring logbook;
        notes_read_one_from_file(id, note, project, fullreference, category,
...
void FindNoteDialog::on_word_entry_changed(GtkEditable *editable,
                                           gpointer user_data) {

...
DisplayProjectNotes::DisplayProjectNotes(const ustring &reference,
  ...
  // Start main thread.
  // New g_thread_new ("displayprojectnotes", GThreadFunc (thread_start),
  // gpointer(this));
...
void DisplayProjectNotes::stop() { mystop = true; }

void DisplayProjectNotes::show_buffer() {
  // Displays the notes text buffer.
  webkit_web_view_load_string(WEBKIT_WEB_VIEW(webview), note_buffer.c_str(),
                              NULL, NULL, NULL);
}
...
void DisplayProjectNotes::thread_main(gpointer data) {
  ...
  if (!mystop) {
    notes_display(note_buffer, ids_to_display, id_to_scroll_to, cursor_offset,
                  mystop, extra_note_id);
  }
  // Finish off.
  ready = true;
}

...
void keyterms_ensure_user_database() {
  ...
  if (g_file_test(keyterms_get_user_filename().c_str(),
...
void keyterms_import_textfile_flush(sqlite3 *db, unsigned int category_id,
                                    ustring &keyterm, vector<ustring> &comments,
  ...
  if (!keyterm.empty()) {
    ...
    // Store the keyterm.
    sql = g_strdup_printf(
        "insert into keyterm values (%d, %d, '%s', '%s', '%s');", keyterm_id,
        category_id, keyterm.c_str(), keyterm.casefold().c_str(),
        comment.c_str());
    ...
    for (unsigned int i = 0; i < references.size(); i++) {
      sql = g_strdup_printf("insert into reference values (%d, %d, %d, %d);",
                            keyterm_id, references[i].book_get(),
                            references[i].chapter_get(),
                            convert_to_int(references[i].verse_get()));
      rc = sqlite3_exec(db, sql, NULL, NULL, &error);
      g_free(sql);
      if (rc)
        throw runtime_error(sqlite3_errmsg(db));
...
void keyterms_import_textfile_flags_down(bool &flag1, bool &flag2,
                                         bool &flag3) {
  flag1 = false;
  flag2 = false;
  flag3 = false;
...
ustring keyterms_reference_start_markup() { return "_reference_start_"; }

ustring keyterms_reference_end_markup() { return "_reference_end_"; }
...
// Imports a keyterms textfile.
{
  ...
  try {
    ...
    sql = g_strdup_printf("insert into category values (%d, '%s');",
                          category_id, category.c_str());
    rc = sqlite3_exec(db, sql, NULL, NULL, &error);
    g_free(sql);
    if (rc)
      throw runtime_error(sqlite3_errmsg(db));
    ...
    for (unsigned int i = 1; i < rt.lines.size(); i++) {
      try {
        ...
        if (line.find("//") != string::npos)
          continue;
        if (line.substr(0, 1) == "[") {
          keyterms_import_textfile_flags_down(readkeyterm, readcomments,
                                              readreferences);
        }
        if (readkeyterm) {
          keyterm = line;
        ...
        if (readreferences) {
          ...
          if (reference_discover(oldRef, line, newRef)) {
            ...
            comments.push_back(keyterms_reference_start_markup() +
                               newRef.human_readable("") +
                               keyterms_reference_end_markup());
          }
        }
        if (line == "[keyterm]") {
          keyterms_import_textfile_flush(db, category_id, keyterm, comments,
                                         references);
          readkeyterm = true;
        }
        if (line == "[comments]") {
...
void keyterms_import_ktref_db(const ustring &textfile, ustring category) {
  ...
  try {
    ...
    // Lines with text that does not start with a backslash will be joined to
    // the previous one.
...
/*
 ...
 digit 1: calculate the ansi code and subtract 32 from it (e.g. ! = 1 =
 Genesis).
 digit 2: if " store number 90.
 digit 3: calculate the ansi code and subtract 32 from it; add the stored number
 of digit 2 to it (e.g. !! = chapter 1; "! = chapter 91).
 digit 4: if " store number 90.
 digit 5: calculate the ansi code and subtract 32 from it; add the stored number
 of digit 4 to it (e.g. !! = verse 1; "! = verse 91).
 digit 6: calculate the ansi code and subtract 32 from it (position of word
 within verse).
 digit 7: calculate the ansi code and subtract 32 from it (position of morpheme
 within word).
 */
{
  ...
  for (unsigned int i = 0; i < parse.words.size(); i++) {
    ...
    *digit5 -= 32;
    Reference reference(*digit1, digit2 + *digit3,
                        convert_to_string(digit4 + *digit5));
    references.push_back(reference);
    comments.push_back(keyterms_reference_start_markup() +
                       reference.human_readable("") +
                       keyterms_reference_end_markup());
    g_free(digit1);
    g_free(digit3);
    g_free(digit5);
...
void keyterms_import_ktbh_txt(const ustring &textfile, ustring category) {
  ...
  try {
    ...
    // Read the text. Any lines not starting with \ should be joined to the
    // previous one.
    vector<ustring> lines;
    {
...
// If all collections have been deleted, it reverts to the original database of
// the package.
{
  ...
  {
    SqliteReader reader(0);
    sql = g_strdup_printf("select id from category where name = '%s';",
  ...
  {
    SqliteReader reader(0);
    sql = g_strdup_printf("select id from keyterm where category = %d;",
                          collection_id);
    sqlite3_exec(db, sql, reader.callback, &reader, NULL);
    g_free(sql);
  ...
  // Delete the category.
  sql = g_strdup_printf("delete from category where name = '%s';",
                        double_apostrophy(collection).c_str());
  sqlite3_exec(db, sql, NULL, NULL, NULL);
  g_free(sql);

  // Delete the keyterms.
  sql = g_strdup_printf("delete from keyterm where category = %d;",
                        collection_id);
  sqlite3_exec(db, sql, NULL, NULL, NULL);
  g_free(sql);
  ...
  for (unsigned int i = 0; i < keyterm_ids.size(); i++) {
    sql =
        g_strdup_printf("delete from reference where id = %d;", keyterm_ids[i]);
    sqlite3_exec(db, sql, NULL, NULL, NULL);
    g_free(sql);
  }
...
void keyterms_get_terms(const ustring &searchterm, const ustring &collection,
                        vector<ustring> &terms, vector<unsigned int> &ids)
...
// If the collection is empty, get them from all collections.
{
  ...
  try {
    ...
    if (!collection.empty()) {
      ...
      sql = g_strdup_printf("select id from category where name = '%s';",
                            double_apostrophy(collection).c_str());
      rc = sqlite3_exec(db, sql, readercollection.callback, &readercollection,
                        &error);
      g_free(sql);
      if (rc)
        throw runtime_error(sqlite3_errmsg(db));
    ...
    sql.append(";");
    // Query the db.
    rc = sqlite3_exec(db, sql.c_str(), readerterms.callback, &readerterms,
                      &error);
    if (rc)
      throw runtime_error(sqlite3_errmsg(db));
    // Return results.
...
bool keyterms_get_data(unsigned int id, ustring &category, ustring &comments,
  ...
  try {
    ...
    sql = g_strdup_printf(
        "select category, comment from keyterm where id = %d;", id);
    ...
    sql = g_strdup_printf("select name from category where id = %d;",
                          convert_to_int(reader1.ustring0[0]));
    ...
    sql = g_strdup_printf(
        "select book, chapter, verse from reference where id = %d;", id);
    rc = sqlite3_exec(db, sql, reader3.callback, &reader3, &error);
    g_free(sql);
    if (rc)
      throw runtime_error(sqlite3_errmsg(db));
    for (unsigned int i = 0; i < reader3.ustring0.size(); i++) {
      Reference reference(convert_to_int(reader3.ustring0[i]),
                          convert_to_int(reader3.ustring1[i]),
                          reader3.ustring2[i]);
      references.push_back(reference);
    }
  } catch (exception &ex) {
...
// If the database is not there, it will be created on the fly.
{
  ...
  if (!g_file_test(filename.c_str(), G_FILE_TEST_IS_REGULAR)) {
    ...
    try {
      ...
      sqlite3_busy_timeout(db, 1000);
      char *sql;
      sql = g_strdup_printf("create table renderings (keyword string, "
                            "collection string, rendering string, wholeword "
                            "integer, casesensitive integer);");
      rc = sqlite3_exec(db, sql, NULL, NULL, &error);
      g_free(sql);
      if (rc)
        throw runtime_error(sqlite3_errmsg(db));
...
void keyterms_retrieve_renderings(const ustring &project,
  ...
  try {
    ...
    char *sql;
    SqliteReader reader(0);
    sql = g_strdup_printf("select rendering, wholeword, casesensitive from "
                          "renderings where keyword = '%s' and collection = "
                          "'%s';",
...
void keyterms_store_renderings(const ustring &project, const ustring &keyterm,
  ...
  try {
    ...
    char *sql = g_strdup_printf(
        "delete from renderings where keyword = '%s' and collection = '%s';",
        double_apostrophy(keyterm).c_str(),
        double_apostrophy(collection).c_str());
    rc = sqlite3_exec(db, sql, NULL, NULL, &error);
    g_free(sql);
    for (unsigned int i = 0; i < renderings.size(); i++) {
      ...
      sql = g_strdup_printf(
          "insert into renderings values ('%s', '%s', '%s', %d, %d);",
          s1.c_str(), s2.c_str(), s3.c_str(), i1, i2);
      rc = sqlite3_exec(db, sql, NULL, NULL, &error);
      g_free(sql);
      if (rc)
        throw runtime_error(sqlite3_errmsg(db));
...
void keyterms_export(const ustring &directory, bool gui) {
  ...
  for (unsigned int cat = 0; cat < categories.size(); cat++) {
    // Output data.
    ustring paratext_filename =
        gw_build_filename(directory, categories[cat] + ".lex");
...
vector<int> keyterms_get_terms_in_verse(const Reference &reference) {
  ...
  sqlite3_busy_timeout(db, 1000);
  SqliteReader reader(0);
  char *sql = g_strdup_printf("select id from reference where book = %d and "
                              "chapter = %d and verse = '%s';",
...
void keyterms_export_renderings(const ustring &collection,
                                bool include_terms_wo_renderings)
// Exports keyterms and their renderings to html.
// collection: May restrict the export to one collection.
// include_terms_wo_rendering: Whether to include the keyterms that have no
// renderings.
{
  // The project.
  ...
  // Get keyterms and renderings.
  {
    ...
    for (unsigned int i = 0; i < collections.size(); i++) {
      ...
      for (unsigned int i2 = 0; i2 < keyterms.size(); i2++) {
        ...
        vector<bool> dummy;
        keyterms_retrieve_renderings(project, keyterms[i2], collections[i],
                                     rendering, dummy, dummy);

...
ListviewMDialog::ListviewMDialog(const ustring &title, vector<ustring> &list,
                                 bool sortlist, gchar *help)
...
// It returns the selected rows in "foci".
{
  ...
  label1 =
      gtk_label_new(_("Select one or more items, and then press OK or Enter"));
  ...
  g_signal_connect_after((gpointer)treeview, "button_release_event",
                         G_CALLBACK(on_treeview_button_event), gpointer(this));
  g_signal_connect_after((gpointer)treeview, "key_release_event",
                         G_CALLBACK(on_treeview_key_event), gpointer(this));
  g_signal_connect((gpointer)treeview, "row_activated",
                   G_CALLBACK(on_treeview_row_activated), gpointer(this));
...
ListviewMDialog::~ListviewMDialog() { gtk_widget_destroy(dialog); }

int ListviewMDialog::run() { return gtk_dialog_run(GTK_DIALOG(dialog)); }

gboolean ListviewMDialog::on_treeview_button_event(GtkWidget *widget,
...
gboolean ListviewMDialog::on_treeview_key_event(GtkWidget *widget,
                                                GdkEventKey *event,
                                                gpointer user_data) {
  ((ListviewMDialog *)user_data)->on_treeview_cursor();
  return FALSE;
...
void ListviewMDialog::on_treeview_row_activated(GtkTreeView *treeview,
                                                GtkTreePath *path,

...
ustring scripts_straight_through() { return "straight through"; }
    ...
    for (int scripttype = stSed; scripttype < stEnd; scripttype++) {
      ...
      if (!source_suffix.empty()) {
        if (g_str_has_suffix(rf.files[i].c_str(), source_suffix.c_str())) {
          rf.files[i].erase(rf.files[i].length() - source_suffix.length(),
                            source_suffix.length());
        }
      }
      if (!binary_suffix.empty()) {
        if (g_str_has_suffix(rf.files[i].c_str(), binary_suffix.c_str())) {
          rf.files[i].erase(rf.files[i].length() - binary_suffix.length(),
                            binary_suffix.length());
        }
      }
    }
  }
...
ustring script_filter(const ustring &scriptname, bool straightthrough,
                      const ustring &inputfile, const ustring &outputfile)
/*
 ...
 */
{
  ...
  // If the rules file does not exist, or the script is of an unknown type, pass
  // it straight through.
  if (!g_file_test(scriptfile.c_str(), G_FILE_TEST_IS_REGULAR) ||
      (scripttype == stEnd)) {
    unix_cp(inputfile, outputfile);
  ...
  case stFree: {
    ...
    replace_text(scriptdata, script_free_input_identifier(),
                 shell_quote_space(encodedinputfile));
    ...
    replace_text(scriptdata, script_free_output_identifier(),
                 shell_quote_space(outputfile));
    // Write temporal script.
    g_file_set_contents(script_temporal_script_file().c_str(),
                        scriptdata.c_str(), -1, NULL);
  ...
  int result = system(
      command
          .c_str()); // This one is too unpredictable to be used with GwSpawn.
  ...
  // Handle error.
  gchar *contents;
  g_file_get_contents(script_temporal_error_file().c_str(), &contents, NULL,
                      NULL);
  if (contents) {
    error = contents;
    g_free(contents);
...
ustring script_get_path(const ustring &script, ScriptType *scripttype,
                        bool binary)
// Gets the full path to an existing "script".
// If "binary" is true, then, if possible, it gives the path, not to the
// original
// script, but to the binary form of it. Right now this is used for TECkit
// support
...
// The "scripttype" is filled with the right type of script.
{
  ...
  for (unsigned int i = 0; i < stEnd; i++) {
    ...
    ustring path = gw_build_filename(Directories->get_scripts(),
                                     script_prefix(script_type) + script +
                                         script_suffix(script_type, binary));
...
{
  return gw_build_filename(Directories->get_scripts(),
                           script_prefix(scripttype) + script +
                               script_suffix(scripttype, binary));
}
...
const gchar *script_sed_binary() { return "sed"; }

const gchar *script_teckit_compiler() { return "teckit_compile"; }

const gchar *script_teckit_txtconverter() { return "txtconv"; }

const gchar *script_free_input_identifier() { return "inputfile"; }

const gchar *script_free_output_identifier() { return "outputfile"; }

void script_encode_usfm_file(const ustring &filename)
// Encodes a USFM file. The purpose is that the USFM marked are not changed by
// the script.

...
// Dialog to show the commands used in the text2pdf object.
{
  ...
  gtk_window_set_position(GTK_WINDOW(text2pdfdialog),
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(text2pdfdialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(text2pdfdialog), okbutton,
                               GTK_RESPONSE_OK);
  gtk_widget_set_can_default(GTK_WIDGET(okbutton), true);

  g_signal_connect((gpointer)okbutton, "clicked",
                   G_CALLBACK(on_okbutton_clicked), gpointer(this));
  g_signal_connect((gpointer)text2pdfdialog, "delete_event",
                   G_CALLBACK(on_text2pdfdialog_delete_event), gpointer(this));
  ...
  // Set buffer unmodified so as to prevent time-consuming actions being taken
  // on it if there was no change.
...
Text2PdfDialog::~Text2PdfDialog() { gtk_widget_destroy(text2pdfdialog); }

int Text2PdfDialog::run() { return gtk_dialog_run(GTK_DIALOG(text2pdfdialog)); }

void Text2PdfDialog::on_okbutton_clicked(GtkButton *button,
                                         gpointer user_data) {
...
gboolean Text2PdfDialog::on_text2pdfdialog_delete_event(GtkWidget *widget,
                                                        GdkEvent *event,
                                                        gpointer user_data)
// Event handler.
{

...
{
  return gw_build_filename(project_data_directory_book(project, book),
                           reporting_status_filename());
}

void reporting_check_tasks_and_durations(vector<ustring> &tasks,
...
// If the vectors are not of the same size, their sizes will be adjusted.
{
  ...
  if (tasks.empty()) {
    ...
    for (unsigned int i = 0;
         i < (sizeof(default_values) / sizeof(*default_values)); i++) {
    ...
    if (tasks.size() != durations->size()) {
      ...
      for (unsigned int i = 0; i < tasks.size(); i++) {
        ...
        for (unsigned int i2 = 0;
             i2 < (sizeof(default_values) / sizeof(*default_values)); i2++) {
...
void reporting_durations_to_percentages(vector<double> &durations,
...
unsigned int
reporting_calculate_percentage_ready(vector<int> &tasks_done,
...
unsigned int reporting_status_string_to_int(const ustring &status,
                                            vector<ustring> *allstatus)
...
void reporting_derive_parent_status_from_children(
    unsigned int taskcount, ProjectStatusRecord *parent,
    vector<ProjectStatusRecord *> children)
...
void reporting_merge_child_status_into_parent(unsigned int taskcount,
                                              unsigned int childnumber,
                                              ProjectStatusRecord &parent,
                                              const ProjectStatusRecord &child)
...
void reporting_derive_children_status_from_parent(
    unsigned int taskcount, vector<ProjectStatusRecord *> children,
    ProjectStatusRecord *parent)
// Takes pointers to an existing status record, and derives the children's
// status
// from their parent.
{
...
unsigned int ProjectStatusRecord::calculate_percentage(
    const vector<unsigned int> &percentages) {
...
ProjectStatusChapter::ProjectStatusChapter(unsigned int chapter_in,
                                           unsigned int highestverse,
                                           GKeyFile *keyfile,
                                           vector<ustring> *alltasks_in)
/*
 ...
 */
{
  ...
  bool chapterrecord_available = false;
  {
    gsize length;
    gchar **stringlist =
        g_key_file_get_string_list(keyfile, convert_to_string(chapter).c_str(),
                                   chapter_key(), &length, NULL);
    if (stringlist) {
  ...
  for (unsigned int i = 0; i <= highestverse; i++) {
    ...
    if (chapterrecord_available) {
      ...
      gsize length;
      gchar **stringlist = g_key_file_get_string_list(
          keyfile, convert_to_string(chapter).c_str(),
          convert_to_string(i).c_str(), &length, NULL);
      if (stringlist) {
        for (unsigned int i2 = 0; i2 < length; i2++) {
          for (unsigned int i3 = 0; i3 < alltasks->size(); i3++) {
            if (stringlist[i2] == alltasks->at(i3)) {
...
// This status is derived from all verses.
{
  ...
  reporting_derive_parent_status_from_children(alltasks->size(), &statusrecord,
                                               children_records);
...
// Propagates the "state" to all the verses in the chapter.
{
  ...
  reporting_derive_children_status_from_parent(alltasks->size(), verses_status,
                                               &state);
}
...
void ProjectStatusChapter::set_verse(unsigned int verse,
                                     const ProjectStatusRecord &state)
...
// Save the status of the chapter to the keyfile.
{
  ...
  if (chapter_consistent) {
    ...
    if (!values.empty()) {
      ...
      g_key_file_set_string_list(keyfile, group.c_str(), chapter_key(),
                                 (const gchar **const)strs, values.size());
      for (unsigned int i2 = 0; i2 < values.size(); i2++) {
        g_free(strs[i2]);
      }
      g_free(strs);
    ...
    for (unsigned int i = 0; i < status.size(); i++) {
      ...
      g_key_file_set_string_list(keyfile, group.c_str(), name.c_str(),
                                 (const gchar **const)strs, values.size());
      for (unsigned int i2 = 0; i2 < values.size(); i2++) {
        g_free(strs[i2]);
      }
...
ProjectStatusBook::ProjectStatusBook(const ustring &project_in,
  ...
  g_key_file_load_from_file(keyfile,
                            reporting_status_filename(project, book).c_str(),
  ...
  for (unsigned int i = 0; i < chps.size(); i++) {
    unsigned int highestverse = convert_to_int(
        versification_get_last_verse(versification, book, chps[i]));
    if (highestverse == 1)
      highestverse = 0;
    ProjectStatusChapter *chapter =
        new ProjectStatusChapter(chps[i], highestverse, keyfile, alltasks);
...
// Save the status of the book.
{
  ...
  if (data) {
    g_file_set_contents(reporting_status_filename(project, book).c_str(), data,
                        -1, NULL);
    g_free(data);
  }
  // Free memory.
...
ProjectStatus::ProjectStatus(const ustring &project_in,
                             const vector<ustring> &alltasks_in, bool gui) {
  // Save project.
  project = project_in;
  alltasks = alltasks_in;
...
unsigned int
reporting_get_percentage_ready_project(ProjectStatus *projectstatus)
// Calculates the percentage ready for the whole project.
{
  ...
  for (unsigned int i = 0; i < projectstatus->books.size(); i++) {
    ...
    for (unsigned int i = 0; i < projectstatusbook->chapters.size(); i++) {
      ProjectStatusChapter *projectstatuschapter =
          projectstatusbook->chapters[i];
      for (unsigned int i = 0; i < projectstatuschapter->status.size(); i++) {
        unsigned int percentage =
            projectstatuschapter->status[i].calculate_percentage(percentages);
        verse_count++;
...
unsigned int
reporting_get_percentage_ready_book(ProjectStatusBook *projectstatusbook) {
  extern Settings *settings;
  vector<ustring> tasks = settings->genconfig.project_tasks_names_get();
  vector<double> durations;
  reporting_check_tasks_and_durations(tasks, &durations);
...
void reporting_get_tasks_done_per_book(ProjectStatus *projectstatus,
                                       vector<ustring> &headers,
                                       vector<VectorUstring> &texts)
...
void reporting_get_tasks_done_per_chapter(ProjectStatusBook *projectstatusbook,
                                          vector<ustring> &headers,
                                          vector<VectorUstring> &texts) {
  // Get the available tasks to be done.
  extern Settings *settings;
...
void reporting_produce_status_report(const ustring &project, bool planning,
                                     bool perc_done_project,
                                     bool perc_done_book, bool tasks_book,
                                     bool tasks_chapter, bool csv_export)
// This produces a status report.
{
  ...
  // Current date.
  htmlwriter.paragraph("Produced on " +
                       date_time_julian_human_readable(
                           date_time_julian_day_get_current(), true) +
                       ".");
  ...
  if (perc_done_project) {
    ...
    unsigned int percentage =
        reporting_get_percentage_ready_project(&projectstatus);
    percentages.push_back(percentage);
    ...
    if (csv_export) {
      ...
      ustring filename =
          gw_build_filename(Directories->get_temp(), "project_complete.csv");
  ...
  if (perc_done_book) {
    ...
    for (unsigned int i = 0; i < projectstatus.books.size(); i++) {
      ...
      percentages.push_back(
          reporting_get_percentage_ready_book(projectstatusbook));
    }
    ...
    if (csv_export) {
      ...
      for (unsigned int i = 0; i < texts.size(); i++) {
        csv.push_back("\"" + texts[i] + "\"," +
                      convert_to_string(percentages[i]));
      }
      ustring filename =
          gw_build_filename(Directories->get_temp(), "books_complete.csv");
      write_lines(filename, csv);
      htmlwriter.hyperlinkedparagraph("The above data in csv format", filename);
      htmlwriter.paragraph("");
    }
  ...
  if (tasks_book) {
    ...
    reporting_get_tasks_done_per_book(&projectstatus, column_headers,
                                      cell_texts);
    vector<bool> centers;
    ...
    htmlwriter.table("Tasks per book - Legend", column_headers, legend, "",
                     NULL, -1);
    htmlwriter.paragraph("");
    htmlwriter.table("Tasks per book - Data", shortened_column_headers,
                     cell_texts, "", &centers, -1);
    htmlwriter.paragraph("");
  }
  ...
  if (tasks_chapter) {
    for (unsigned int i = 0; i < projectstatus.books.size(); i++) {
      ...
      vector<ustring> column_headers;
      vector<VectorUstring> cell_texts;
      reporting_get_tasks_done_per_chapter(projectstatusbook, column_headers,
                                           cell_texts);
      vector<bool> centers;
      ...
      htmlwriter.table("Tasks per chapter for " +
                           books_id_to_english(projectstatus.books[i]->book),
                       column_headers, cell_texts, "", &centers, -1);
      htmlwriter.paragraph("");
    }
  }
}

void reporting_get_tasks_undone(ProjectStatus *projectstatus,
...
unsigned int
reporting_get_task_percentage_complete(ProjectStatus *projectstatus,
                                       unsigned int book, const ustring &task)

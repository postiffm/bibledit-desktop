...
class Editor2
    : public ChapterView // Editor2 has to implement the ChapterView interface
{
public:
...
private:
  ...
  void apply_editor_action(EditorAction *action,
                           EditorActionApplication application = eaaInitial);
...
private:
  bool usfm_starts_new_paragraph(ustring &line, const ustring &marker,
  ...
  bool editor_starts_character_style(ustring &line, ustring &character_style,
                                     const ustring &marker_text,
                                     size_t marker_pos, size_t marker_length,
                                     bool is_opener, bool marker_found);
  bool editor_ends_character_style(ustring &line, ustring &character_style,
                                   const ustring &marker_text,
                                   size_t marker_pos, size_t marker_length,
                                   bool is_opener, bool marker_found);
  bool text_starts_note_raw(ustring &line, ustring &character_style,
                            const ustring &marker_text, size_t marker_pos,
                            size_t marker_length, bool is_opener,
                            bool marker_found, ustring &raw_note);
  void get_next_note_caller_and_style(EditorNoteType type, ustring &caller,
                                      ustring &style, bool restart);
  unsigned int note_style_num;
  void editor_start_note_raw(ustring raw_note, const ustring &marker_text);
  void editor_text_fallback(ustring &line, ustring &character_style,
  ...
  ustring usfm_get_text(GtkTextBuffer *textbuffer, GtkTextIter startiter,
                        GtkTextIter enditer);

  // Textview keyboard key pressing.
  static gboolean on_textview_key_press_event(GtkWidget *widget,
                                              GdkEventKey *event,
                                              gpointer user_data);
  ...
  // Next two obsolete. We should NOT be processing things when the user
  // releases a key, after holding
  // it down for an indeterminate length of time (like backspace or delete in
  // particular)
  // static gboolean on_textview_key_release_event(GtkWidget *widget,
  // GdkEventKey *event, gpointer user_data);
  // void textview_key_release_event(GtkWidget *widget, GdkEventKey *event);
  bool textbuffer_delete_range_was_fired;
  static void on_textbuffer_changed(GtkTextBuffer *textbuffer,
                                    gpointer user_data);
  void textbuffer_changed(GtkTextBuffer *textbuffer);

  // Textview mouse button pressing signal and delay.
  static gboolean on_textview_button_press_event(GtkWidget *widget,
                                                 GdkEventButton *event,
                                                 gpointer user_data);
  gboolean textview_button_press_event(GtkWidget *widget,
                                       GdkEventButton *event);
  ...
  static void on_textview_move_cursor(GtkTextView *textview,
                                      GtkMovementStep step, gint count,
                                      gboolean extend_selection,
                                      gpointer user_data);
  void textview_move_cursor(GtkTextView *textview, GtkMovementStep step,
                            gint count);
...
public:
  ...
  void create_or_update_text_style(Style *style, bool paragraph, bool plaintext,
  ...
  static void on_buffer_insert_text_after(GtkTextBuffer *textbuffer,
                                          GtkTextIter *pos_iter, gchar *text,
                                          gint length, gpointer user_data);
  void buffer_insert_text_after(GtkTextBuffer *textbuffer,
                                GtkTextIter *pos_iter, gchar *text);
  static void on_buffer_delete_range_before(GtkTextBuffer *textbuffer,
                                            GtkTextIter *start,
                                            GtkTextIter *end,
                                            gpointer user_data);
  void buffer_delete_range_before(GtkTextBuffer *textbuffer, GtkTextIter *start,
                                  GtkTextIter *end);
  static void on_buffer_delete_range_after(GtkTextBuffer *textbuffer,
                                           GtkTextIter *start, GtkTextIter *end,
                                           gpointer user_data);
  void buffer_delete_range_after(GtkTextBuffer *textbuffer, GtkTextIter *start,
                                 GtkTextIter *end);
...
private:
  ...
  static void on_button_spelling_recheck_clicked(GtkButton *button,
                                                 gpointer user_data);
...
public:
  ...
  ustring get_verse_number_at_iterator(GtkTextIter iter,
                                       const ustring &verse_marker,
...
private:
  ...
  void combine_paragraphs(EditorActionCreateParagraph *first_paragraph,
...
public:
private:
  static gboolean on_motion_notify_event(GtkWidget *textview,
                                         GdkEventMotion *event,
                                         gpointer user_data);
  gboolean motion_notify_event(GtkWidget *textview, GdkEventMotion *event);
  bool previous_hand_cursor;

  static gboolean on_caller_button_press_event(GtkWidget *widget,
                                               GdkEventButton *event,
                                               gpointer user_data);

...
Httpd::~Httpd() {
  ...
  g_usleep(200000);
// Just to be sure, and solve a problem of sockets not being closed, which has
// been
...
void Httpd::thread_start(gpointer data) { ((Httpd *)data)->thread_main(); }
...
// Listen on socket.
  ...
  // This new system solves that. The socket is non-blocking, and if no
  // connection
  ...
  if (amount > 0) {
    ...
    if (parse.words.size() >= 2) {
      ...
      }
      {
        ...
        filename = gw_build_filename(Directories->get_package_data(),
                                     gw_path_get_basename(filename));
        // Decide what action to take.
        if ((gw_path_get_basename(filename) == "search.html") &&
            (!command.empty())) {
          send_search(fd, filename, command);
        } else if ((gw_path_get_basename(filename) ==
                    "bibledit_loads_references.html") &&
                   (!command.empty())) {
          // E.g.:
          // http://localhost:51516/bibledit_loads_references.html?search-whole-word=word
...
void Httpd::log(const ustring &message) { gw_message("Webserver: " + message); }
...
void Httpd::send_file(int fd, const ustring &filename) {
  if (g_file_test(filename.c_str(), G_FILE_TEST_IS_REGULAR)) {
    ...
    if (g_str_has_prefix(basename.c_str(), "olh_")) {
      ...
      s.insert(pos, "<p>See also the <a href=\"index.html\">general online "
                    "help.</a></p>\n");
#ifdef WIN32

...
int main(int argc, char *argv[]) {
  ...
  // The GLib threading system is automatically initialized at the start of your
  // program.
  // Initialize g types.
...
//}
  ...
  // Redirect stdout and stderr to file.
  {
    ...
    // if (global_debug_level) { printf("Redirecting stdout/stderr to
    // logfile.\n"); }
  ...
  eg by os.system in an activity.py, appears as an extra icon in the "wheel" on
  the home screen,
  and sugar doesn't see it as being connected in any way with the original
  ...
  This uses the XEmbed protocol behind the scenes, and requires a window ID to
  be passed to Bibledit.
  ...
  If that argument is present, it then creates a plug for its main window
  instead of a normal top-level window.
  */
  // Accelerators.
  ...
  gtk_window_set_default_icon_from_file(
      gw_build_filename(Directories->get_package_data(), "bibledit.xpm")
          .c_str(),
      NULL);
  gw_message("Set up window icon fallback");
  // Start the gui.
  MainWindow *mainwindow =
      new MainWindow(xembed, accelerator_group, settings, urltransport, vcs);

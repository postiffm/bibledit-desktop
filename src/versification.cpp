...
void versification_create_book_template(const ustring &versification,
                                        unsigned int book,
                                        vector<ustring> &booktemplate) {
  vector<unsigned int> chapters =
      versification_get_chapters(versification, book);
  chapters.insert(chapters.begin(), 0);
  for (unsigned int i = 0; i < chapters.size(); i++) {
    versification_create_chapter_template(versification, book, chapters[i],
                                          booktemplate);
  }
}

void versification_create_chapter_template(const ustring &versification,
                                           unsigned int book,
                                           unsigned int chapter,
                                           vector<ustring> &chaptertemplate) {
...
vector<unsigned int> versification_get_chapters(const ustring &versification,
                                                unsigned int book) {
  vector<unsigned int> chapters;
  vector<unsigned int> verses;
  versification_get_chapters_verses(versification, book, chapters, verses);
...
ustring versification_get_last_verse(const ustring &versification,

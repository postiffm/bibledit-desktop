...
ustring spelling_global_dictionary() { return _("Global Dictionary"); }

const gchar *spelling_project_dictionary_prefix() { return _("Project "); }
...
ustring spelling_project_dictionary(const ustring &project) {
  return spelling_project_dictionary_prefix() + project +
         spelling_project_dictionary_suffix();
}
...
bool spelling_dictionary_editable(const ustring &dictionary) {
  if (dictionary == spelling_global_dictionary())
    return true;
  if (g_str_has_prefix(dictionary.c_str(),
                       spelling_project_dictionary_prefix()))
    if (g_str_has_suffix(dictionary.c_str(),
                         spelling_project_dictionary_suffix()))
      return true;
  return false;
}
  ...
  if (g_str_has_prefix(dictionary.c_str(),
    ...
    if (g_str_has_suffix(dictionary.c_str(),
      ...
      ustring filename = gw_build_filename(
          project_data_directory_project(project), "shared_dictionary");
      return filename;
    }
  }
  return "";
...
const gchar *spelling_tag_name() { return _("misspelling"); }
...
As far as I can tell, Unicode does not specify any hyphen character that must
not act as a word boundary
(except for the soft hyphen, but that would not fulfill your needs because it is
not visible except when it is followed by a line break);
pretty much any of the hyphen characters may be tailored not to act as a word
boundary, though.
I would say that should be something that the localization of the spell checker
should take care of,
so if none of the characters works (see the list below—in particular notice that
the actual hyphen Unicode character is not what you get
when you type a hyphen on your keyboard; that is instead the hyphen-minus
character), you may want to submit this as a
bug/feature request with the project for whatever spell checking library is used
by Bibledit
(I'm assuming BE uses one of the many open-source spelling libraries and not a
home-grown one).
In particular, I would say that a person who uses a "non-breaking hyphen"
probably typically expects its "non-breaking" aspect to apply to words
as well as lines (although in reality the Unicode standard requires only that a
non-breaking hyphen prevent line breaks).
...
Since Pango routines are used for determining the word boundaries, the right
thing to do is to fix Pango.

  */
}

void SpellingChecker::check_word(GtkTextBuffer *textbuffer, GtkTextIter *start,
                                 GtkTextIter *end) {
  ...
  if (correct) {
    ...
    for (unsigned int i = 0; i < dicts.size(); i++) {
      if (correct)
        continue;
      correct = (enchant_dict_check(dicts[i], word.c_str(),
                                    strlen(word.c_str())) == 0);
    }
    if (correct)
...
// This routine attaches the spelling checker to a textview.
{
  ...
  g_signal_connect(G_OBJECT(textview), "populate-popup",
                   G_CALLBACK(on_populate_popup), gpointer(this));
  g_signal_connect(G_OBJECT(textview), "popup-menu",
                   G_CALLBACK(on_popup_menu_event), gpointer(this));
}
...
gboolean SpellingChecker::on_button_press_event(GtkWidget *widget,
...
void SpellingChecker::button_press_event(GtkWidget *widget,
                                         GdkEventButton *event) {
  if (event->button == 3) {
    gint x, y;
    gtk_text_view_window_to_buffer_coords(GTK_TEXT_VIEW(widget),
                                          GTK_TEXT_WINDOW_TEXT, int(event->x),
                                          int(event->y), &x, &y);
    gtk_text_view_get_iter_at_location(GTK_TEXT_VIEW(widget),
                                       &right_clicked_iter, x, y);
  }
}

void SpellingChecker::on_populate_popup(GtkTextView *textview, GtkMenu *menu,
                                        gpointer user_data) {
...
gboolean SpellingChecker::on_popup_menu_event(GtkTextView *view,
                                              gpointer user_data)
...
void SpellingChecker::build_suggestion_menu(GtkWidget *menu,
  ...
  vector<ustring> replacements;
  {
    ...
    for (unsigned int d = 0; d < dicts.size(); d++) {
      size_t n_suggs;
      char **suggestions =
          enchant_dict_suggest(dicts[d], word, strlen(word), &n_suggs);
      if (suggestions) {
  ...
  if (replacements.empty()) {
    ...
    for (unsigned int i = 0; i < replacements.size(); i++) {
      ...
      g_signal_connect(G_OBJECT(mi), "activate", G_CALLBACK(on_replace_word),
                       gpointer(this));
      gtk_widget_show(mi);
      gtk_menu_shell_insert(GTK_MENU_SHELL(menu), mi, position);
      position++;
  ...
  g_signal_connect(G_OBJECT(mi), "activate", G_CALLBACK(on_add_to_dictionary),
                   gpointer(this));
  gtk_widget_show_all(mi);
  gtk_menu_shell_insert(GTK_MENU_SHELL(mainmenu), mi, position);
  position++;
  ...
  g_signal_connect(G_OBJECT(mi), "activate", G_CALLBACK(on_ignore_all),
                   gpointer(this));
  gtk_widget_show_all(mi);
  gtk_menu_shell_insert(GTK_MENU_SHELL(mainmenu), mi, position);
  position++;
...
void SpellingChecker::right_clicked_word_get_extends(GtkTextIter *start,
                                                     GtkTextIter *end)
...
void SpellingChecker::on_add_to_dictionary(GtkWidget *menuitem,
                                           gpointer user_data) {
...
// Replaces the misspelled word in the text with the word that is chosen in the
// menu.
// An algorithm is used that gives the replacement the same styles as the
// original.
{
  ...
  enchant_dict_store_replacement(dicts[0], oldword, strlen(oldword), newword,
                                 strlen(newword));
  ...
  /*
    // Apply the tags of the old word to the new. This has been disabled for
    just now. It should use EditorActions.
    ...
    for (unsigned int i = 0; i < unewword.length(); i++) {
      ...
      if (!paragraphstyle.empty()) {
        gtk_text_buffer_apply_tag_by_name(buffer, paragraphstyle.c_str(),
    &start, &end);
      }
      if (!characterstyle.empty())
        gtk_text_buffer_apply_tag_by_name(buffer, characterstyle.c_str(),
    &start, &end);
    }
    */
}
...
bool SpellingChecker::move_cursor_to_spelling_error(GtkTextBuffer *textbuffer,
                                                    bool next, bool extremity)

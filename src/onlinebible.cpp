...
void import_online_bible(WindowsOutpost *windows_outpost,
                         const ustring &online_bible,
                         const ustring &bibledit_bible,
  ...
  response = windows_outpost->OnlineBibleDirectCommandResponseGet(
      "GetBookAbbreviationsForVersion " + online_bible);
  ...
  for (unsigned int bk = 0; bk < books.size(); bk++) {
    ...
    if (book_id == 0) {
      summary_messages.push_back(_("Could not find a matching book for ") +
                                 book_olb);
      continue;
    }
    /*
    ...
    response = windows_outpost->OnlineBibleDirectCommandResponseGet(
        "GetChapterCountForBook " + online_bible + " " + book_olb);
    if (online_bible_ok_reply_validate(response)) {
    ...
    for (unsigned int ch = 1; ch <= chapter_count; ch++) {
      ...
      {
        vector<ustring> lines =
            project_retrieve_chapter(bibledit_bible, book_id, ch);
        if (!lines.empty()) {
          continue;
        }
      }
      ...
      response = windows_outpost->OnlineBibleDirectCommandResponseGet(
          "GetVerseCountForChapter " + online_bible + " " + book_olb + " " +
      ...
      for (unsigned int vs = 1; vs <= verse_count; vs++) {
        ...
        // Progress update.
        progresswindow.set_text(book_olb + " " + convert_to_string(ch) + ":" +
                                convert_to_string(vs));

        // Did the user cancel the import? (Could be, because it takes a huge
        // amount of time).
        if (progresswindow.cancel) {
          summary_messages.push_back(_("The rest of the import was cancelled"));
          summary_messages.push_back(
              _("What has been imported already remains"));
          return;
        }

        // Get and store the verse text.
        response = windows_outpost->OnlineBibleDirectCommandResponseGet(
            "GetVerseText " + online_bible + " \"" + book_olb + " " +
            convert_to_string(ch) + ":" + convert_to_string(vs) + "\"");
        if (!online_bible_ok_reply_validate(response)) {
          response = _("Verse text could not be imported");
        }
        chapter_contents.push_back("\\v " + convert_to_string(vs) + " " +
                                   response);
      }

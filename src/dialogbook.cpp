...
BookDialog::BookDialog(const ustring &project) {
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(bookdialog), cancelbutton1,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton1), true);

  okbutton1 = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton1);
  gtk_dialog_add_action_widget(GTK_DIALOG(bookdialog), okbutton1,
                               GTK_RESPONSE_OK);
  ...
  for (unsigned int i = 0; i < books.size(); i++) {
    ...
    gtk_list_store_set(
        model, &iter, COLUMN_BOOK,
        books_id_to_name(projectconfig->language_get(), books[i]).c_str(), -1);
  }

  new DialogAutoScaler(bookdialog, G_MAXINT);
}

BookDialog::~BookDialog() { gtk_widget_destroy(bookdialog); }

int BookDialog::run() { return gtk_dialog_run(GTK_DIALOG(bookdialog)); }
...
void BookDialog::on_button_standard_order_clicked(GtkButton *button,
                                                  gpointer user_data) {

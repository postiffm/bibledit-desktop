...
private:
  ...
  ustring verse; // this is not just a verse integer, but could be 16-18, for
                 // example
public:
  Reference();
  Reference(
      int dummy); // not sure why this was pre-existent, but I kept it for now
  Reference(unsigned int book_in, unsigned int chapter_in,
  ...
  void assign(unsigned int book_in, unsigned int chapter_in,
              const ustring &verse_in);
  void clear(void);
};

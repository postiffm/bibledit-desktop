...
WindowEditor::WindowEditor(const ustring &project_name, GtkWidget *parent_layout, GtkAccelGroup *accelerator_group, bool startup) : FloatingWindow(parent_layout, widEditor, project_name, startup)
// Text editor.
{
...
// Let the editor go to a reference.
{
  if (editor2 || usfmview) {
    ...
    if (new_book) {
      new_chapter = true;
      if (editor2)
        editor2->book_set(reference.book);
      if (usfmview)
    ...
    if (new_chapter) {
      ...
      if (editor2)
        editor2->chapter_load(reference.chapter);
      if (usfmview)
    ...
    if (new_book || new_chapter || new_verse) {
      if (editor2)
        editor2->go_to_verse(reference.verse, false);
      if (usfmview)

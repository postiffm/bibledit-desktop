...
BulkSpellingDialog::BulkSpellingDialog(const vector<ustring> words) {
  ...
  gtk_builder_add_from_file(
      gtkbuilder, gw_build_filename(Directories->get_package_data(),
                                    "gtkbuilder.bulkspellingdialog.xml")
                      .c_str(),
      NULL);
  ...
  gtk_label_set_text(GTK_LABEL(label),
                     _("All the words that are misspelled are visible "
  ...
  for (unsigned int i = 0; i < words.size(); i++) {
    ...
    g_signal_connect((gpointer)spellbutton, "clicked",
                     G_CALLBACK(on_spellbutton_clicked), gpointer(this));
  }
...
int BulkSpellingDialog::run() { return gtk_dialog_run(GTK_DIALOG(dialog)); }

void BulkSpellingDialog::on_okbutton_clicked(GtkButton *button,
                                             gpointer user_data) {
...
void BulkSpellingDialog::on_spellbutton_clicked(GtkButton *button,
                                                gpointer user_data) {

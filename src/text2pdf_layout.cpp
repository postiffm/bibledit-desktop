...
T2PLayoutContainer::T2PLayoutContainer(PangoRectangle rectangle_in,
                                       T2PArea *parent_in, cairo_t *cairo)
...
void T2PLayoutContainer::layout_text(T2PInputParagraph *paragraph,
                                     unsigned int line_number, string &text)
...
// PangoLayout works with byte indexes, therefore we work with "string" instead
// of with "ustring".
{
  // Store paragraph.
  ...
  // If there's a note, then let the layout wrap on word, rather than on word
  // and char.
  // This is required because of the system of laying out notes, where notes are
  // laid out one after the other,
  // so there may be cases that very little space at the end remains. This
  // little space would cause a wrap at the
  // character level, which is undesirable in notes.
  if (has_note)
    pango_layout_set_wrap(layout, PANGO_WRAP_WORD);
  // If the text starts with a space, shifting of the attributes has been
  // noticed.
  ...
  // Get the first line in the layout, and ensure that there's only one in the
  // layout.
  // Store any remnant of the input text.
  // Very often the last character of the relevant line in the layout is a space
  // because of the wrapping.
  // This space is removed to give a better visual justification.
  // PangoLayoutLine->start_index gives the start of line as byte index into
  // layout->text. Note the "byte".
  // PangoLayoutLine->length gives the length of line in bytes. Note the
  // "bytes".
  ...
  if (parent)
    ((T2PBlock *)parent)
        ->set_widow_orphan_data(line_number, last_line_of_paragraph_loaded);

  // Store any notes that belong to the bit of text that was laid out.
  if (paragraph)
    note_paragraphs =
        paragraph->get_notes(original_length_of_text, line.length());

  // In normal cases the width of the layout container is set to the actual
  // width of the laid-out text.
  // But in the special case that there's a note, and that some text is left to
  // be put in the next layout container,
  // the size won't be set. In this case it is made to appear as if no space if
  // left for anything.
  // As a result the next layout container won't be placed after this note, but
  // rather on a new line, below it.
  if (has_note && !text.empty()) {
    // Don't modify the size because we're in a note, and there's still text
    // left for the next layout.
  } else {
...
// Print the content.
{
  ...
  // Move into position.
  cairo_move_to(cairo, pango_units_to_points(rectangle.x),
                pango_units_to_points(rectangle.y));
  // Show text.
  if (layout) {
...
void T2PLayoutContainer::index_white_space(const string &text,
                                           vector<guint> &offsets)
...
void T2PLayoutContainer::indentation_width_margins_alignment(bool first_line,
                                                             bool right_to_left)
/*
 Handles the indentation, the width, the margins, and the alignment of the
 paragraph.
 first_line: whether the layout contains the first line of the paragraph.
 right_to_left: whether text runs from right to left.

 Things to test are positive and negative indentation, with any combination of
 the alignment.
 ...
 */
{
  ...
  if (first_line && input_paragraph->first_line_indent_mm != 0) {
    int first_line_indent_pango_units =
        millimeters_to_pango_units(input_paragraph->first_line_indent_mm);
  ...
  if (input_paragraph->left_margin_mm != 0) {
    int left_margin_pango_units =
        millimeters_to_pango_units(input_paragraph->left_margin_mm);
    rectangle.width -= left_margin_pango_units;
    rectangle.x += left_margin_pango_units;
  }
  // Right margin.
  if (input_paragraph->right_margin_mm != 0) {
    int right_margin_pango_units =
        millimeters_to_pango_units(input_paragraph->right_margin_mm);
...
void T2PLayoutContainer::justify(const string &line, bool last_line,
                                 PangoAttrList *attrs)
/*
 ...
 - if there are no spaces in the line, that then the letters are spaced instead
 of the spaces.
 */
{
  // If the last line of the paragraph is loaded, no justification should be
  // done.
  if (last_line)
    return;
  ...
  // so that the total text becomes as wide as is desired to give the effect of
  // justification.
  ...
  if (start_indexes.empty()) {
    ...
    int white_space_spacing =
        (desired_width - actual_width) / start_indexes.size();
...
// Sets the font name and size.
{
  ...
  desc = pango_font_description_from_string(
      input_paragraph ? input_paragraph->font_name.c_str() : "");
  pango_font_description_set_absolute_size(
      desc, input_paragraph ? input_paragraph->font_size_points * PANGO_SCALE
                            : 12 * PANGO_SCALE);
  ...
  if (input_paragraph) {
    ...
    do {
      if (input_paragraph->inline_get_font_size(
              index, in_range, font_size_pango_units, start_index, end_index)) {
        PangoAttribute *attr;
...
// Sets the italics, if any.
{
  ...
  bool italic;
  do {
    if (input_paragraph->inline_get_italic(index, in_range, italic, start_index,
                                           end_index)) {
      if (italic) {
        PangoAttribute *attr;
...
// Sets bold text.
{
  ...
  bool bold;
  do {
    if (input_paragraph->inline_get_bold(index, in_range, bold, start_index,
                                         end_index)) {
      if (bold) {
        PangoAttribute *attr;
...
// Sets the underline attribute.
{
  ...
  bool underline;
  do {
    if (input_paragraph->inline_get_underline(index, in_range, underline,
...
/*
 Sets the small caps. But this was not yet implemented in Pango at the time of
 programming this function.

 Small capitals (usually abbreviated small caps) are uppercase (capital)
 characters set at the same height as surrounding
 lowercase (small) letters or text figures.

 Typically, the height of a small capital will be one ex, the same height as
 most lowercase characters in the font;
 classically, small caps were very slightly taller than x-height. Well-designed
 small capitals are not simply scaled-down
 versions of normal capitals; they normally retain the same stroke weight as
 other letters, and a wider aspect ratio
 to facilitate readability.

 Many word processors and text formatting systems include an option to format
 text in caps and small caps;
 this leaves uppercase letters as they are but converts lowercase letters to
 small caps.
 How this is implemented depends on the typesetting system; some can use true
 small caps associated with a font,
 making text look proportional, but most modern digital fonts do not have a
 small-caps case, so the typesetting
 system simply reduces the uppercase letters by a fraction, making them look out
 of proportion.
 (Often, in text, the next bolder version of the small caps generated by such
 systems will match well with the normal
 weights of capitals and lower case, especially when such small caps are
 extended about 5% or letterspaced a half point
 or a point.)

 In typography, the x-height or corpus size refers to the distance between the
 baseline and the mean line in a typeface.
 Typically, this is the height of the letter x in the font (which is where the
 terminology came from),
 as well as the u, v, w, and z. (The a, c, e, m, n, o, r and s all tend to
 exceed the x-height slightly,
 due to overshoot.) However, in modern typography, the x-height is simply a
 design characteristic of the font,
 and while an x is usually exactly one x-height in height, in some more
 decorative or script designs,
 ...
 */
{
  ...
  bool small_caps;
  do {
    if (input_paragraph->inline_get_small_caps(index, in_range, small_caps,
                                               start_index, end_index)) {
      if (small_caps) {
        PangoAttribute *attr;
...
// Sets the superscript markup.
{
  ...
  bool superscript;
  do {
    if (input_paragraph->inline_get_superscript(
            index, in_range, superscript, start_index, end_index,
            input_paragraph->text.length())) {
      {
        // In professional typesetting software, the superscript character is
        // raised by 33%.
        PangoAttribute *attr;
        attr = pango_attr_rise_new(
            int(input_paragraph->font_size_points * PANGO_SCALE * 0.33));
        attr->start_index = start_index;
        attr->end_index = end_index;
        pango_attr_list_insert(attrs, attr);
      }
      {
        PangoAttribute *attr;
        // In professional typesetting software, the size of the superscript
        // character is 58%
      ...
      {
        // To keep the superscript character visually similar to the rest of the
        // font,
        // the weight should be slightly heavier than the reduced-size character
        // would be.
        PangoAttribute *attr;
        attr = pango_attr_weight_new(
            (PangoWeight)((PANGO_WEIGHT_NORMAL + PANGO_WEIGHT_SEMIBOLD) / 2));
        attr->start_index = start_index;
        attr->end_index = end_index;
        pango_attr_list_insert(attrs, attr);
      }
...
// Sets the colour.
{
  ...
  int colour;
  do {
    if (input_paragraph->inline_get_colour(index, in_range, colour, start_index,
                                           end_index)) {
      ustring colour_string = "#" + color_decimal_to_hex(colour);
      PangoColor pangocolor;
      pango_color_parse(&pangocolor, colour_string.c_str());
      PangoAttribute *attr;
      attr = pango_attr_foreground_new(pangocolor.red, pangocolor.green,
                                       pangocolor.blue);
      attr->start_index = start_index;
      attr->end_index = end_index;
      pango_attr_list_insert(attrs, attr);
...
// Sets the strike-through markup, if any.
{
  ...
  bool strike_through;
  do {
    if (input_paragraph->inline_get_strike_through(
            index, in_range, strike_through, start_index, end_index)) {
      if (strike_through) {
        PangoAttribute *attr;
...
// This undoes the laying out of text. Used for putting the text in this layout
// back into the original input paragraph.
{
  if (input_paragraph) {
...
void T2PLayoutContainer::layout_drop_caps(T2PInputParagraph *paragraph,
                                          double context_font_size)
// Lays out a text in drop-caps format.
{
  ...
  // The drop-caps text gets a visual height of two lines of the paragraph.
  {
    ...
    PangoFontDescription *desc;
    desc =
        pango_font_description_from_string(input_paragraph->font_name.c_str());
    pango_font_description_set_absolute_size(
        desc, input_paragraph->font_size_points * PANGO_SCALE);
    pango_layout_set_font_description(layout, desc);
    ...
    // The ink of the drop-caps text should start at a certain y below the
    // baseline.
    int ink_y_top = ink_rect.y;

    // The ink of the drop-caps text should end at a certain y below the
    // baseline.

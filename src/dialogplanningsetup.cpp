...
PlanningSetupDialog::PlanningSetupDialog(int dummy) {
  ...
  startdate = settings->projectconfig(settings->genconfig.project_get())
                  ->planning_project_start_get();
  ...
  gtk_builder_add_from_file(
      gtkbuilder, gw_build_filename(Directories->get_package_data(),
                                    "gtkbuilder.planningsetupdialog.xml")
                      .c_str(),
      NULL);
  ...
  g_signal_connect((gpointer)button_start, "clicked",
                   G_CALLBACK(on_button_start_clicked), gpointer(this));

  button_tasks = GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "button_tasks"));
  shortcuts.button(button_tasks);
  g_signal_connect((gpointer)button_tasks, "clicked",
                   G_CALLBACK(on_button_tasks_clicked), gpointer(this));

  button_time = GTK_WIDGET(gtk_builder_get_object(gtkbuilder, "button_time"));
  shortcuts.button(button_time);
  g_signal_connect((gpointer)button_time, "clicked",
                   G_CALLBACK(on_button_time_clicked), gpointer(this));
...
int PlanningSetupDialog::run() { return gtk_dialog_run(GTK_DIALOG(dialog)); }

void PlanningSetupDialog::on_button_start_clicked(GtkButton *button,
                                                  gpointer user_data) {
...
void PlanningSetupDialog::on_button_tasks_clicked(GtkButton *button,
                                                  gpointer user_data) {
...
void PlanningSetupDialog::on_button_tasks() {
  ...
  EditListDialog dialog(&tasks, _("Tasks"),
                        _("of tasks - add, remove, or re-order them"), true,
...
void PlanningSetupDialog::on_button_time_clicked(GtkButton *button,
                                                 gpointer user_data) {
...
void PlanningSetupDialog::on_okbutton_clicked(GtkButton *button,
                                              gpointer user_data) {
...
void PlanningSetupDialog::on_okbutton() {
  ...
  settings->projectconfig(settings->genconfig.project_get())
      ->planning_project_start_set(startdate);
...
void PlanningSetupDialog::gui() {
  gtk_button_set_label(
      GTK_BUTTON(button_start),
      date_time_julian_human_readable(startdate, false).c_str());
}

...
OsisRoot::OsisRoot(WriteText *wt, const ustring &name,
...
// Add the root data for the osis file.
{
  ...
  mywt->text("<osis "
             ...
             "namespace osisCore.2.1.xsd\">\n");
  mywt->text("\n");
  mywt->text("<osisText osisIDWork=\"" + name +
...
OsisChapter::OsisChapter(WriteText *wt, const ustring &book,
...
OsisVerse::OsisVerse(WriteText *wt, const ustring &book, unsigned int chapter,
                     const ustring &verse)
// Add the verse data for the osis file.
// Segmented verses like "6a", e.g. in verse "2-6a" are supposed to be given
// like:
...
// Open a paragraph.
{
  ...
  // It seems that the newer osis2mod compiler does not support these sections
  // anymore.
  //  mywt->text ("<p>");
}

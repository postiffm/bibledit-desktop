...
// these words overlap, combine these words into one string to be highlighted.
{
  ...
  if (positions.size() < 2)
    return;
  // We presume that the positions have been sorted. That's only how it can
  // work.
  ...
  do {
    ...
    for (unsigned int i = 0; i < positions.size() - 1; i++) {
      // Let the two iterators point to the second position that might have to
      // be removed.
      iterpos++;
      iterlen++;
      ...
      if ((second_bound >= lower_bound) && (second_bound <= upper_bound)) {
        // There is overlap. Get the highest upper boundary of the two
        // positions.
        unsigned int highest_bound =
            MAX(upper_bound, second_bound + lengths[i + 1]);
...
void xml_html_insert_emphasis(ustring &line, vector<size_t> &positions,
...
*/
{
  ...
  while (offposition != string::npos) {
    ...
    // Do not repeat on the & just removed and inserted, but start searching
    // after it.
...
void xml_positions_push_up(size_t position, size_t up,
...
ustring xml_text_embed_in_tags(int level, const ustring tag,
                               const ustring &text) {
  // Handle < and > and &.
  ustring text2(text);

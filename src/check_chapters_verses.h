...
public:
  CheckChaptersVerses(const ustring &project, const vector<unsigned int> &books,
                      bool gui);
  ~CheckChaptersVerses();
...
private:
  ...
  void verses_check(unsigned int book, unsigned int chapter,
                    const vector<ustring> &verses);
  void message(unsigned int book, unsigned int chapter, const ustring &verse,

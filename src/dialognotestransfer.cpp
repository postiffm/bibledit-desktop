...
NotesTransferDialog::NotesTransferDialog(int dummy) {
  ...
  gtk_window_set_title(GTK_WINDOW(notestransferdialog),
                       _("Transfer To Project Notes"));
  gtk_window_set_position(GTK_WINDOW(notestransferdialog),
  ...
  label1 = gtk_label_new(_("This will transfer all text from the currently "
                           "opened project into the Project notes."));
  ...
  dialog_action_area1 =
      gtk_dialog_get_action_area(GTK_DIALOG(notestransferdialog));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(notestransferdialog), cancelbutton,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton), true);

  okbutton = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton);
  gtk_dialog_add_action_widget(GTK_DIALOG(notestransferdialog), okbutton,
                               GTK_RESPONSE_OK);
...
void NotesTransferDialog::on_okbutton_clicked(GtkButton *button,
                                              gpointer user_data) {
  ...
  for (unsigned int bk = 0; bk < books.size(); bk++) {
    ...
    for (unsigned int ch = 0; ch < chapters.size(); ch++) {
      ...
      for (unsigned int vs = 0; vs < verses.size(); vs++) {
        ...
        clean_note(text);
        if (!text.empty()) {
          transfer_note(project, books[bk], chapters[ch], verses[vs], text,
                        category);
        }
      }
    }
...
void NotesTransferDialog::transfer_note(const ustring &project,
                                        unsigned int book, unsigned int chapter,
                                        const ustring &verse, ustring &text,
                                        const ustring &category) {
  int id = notes_database_get_unique_id();
  ustring references =
      books_id_to_osis(book) + "." + convert_to_string(chapter) + "." + verse;
  int date_created = date_time_julian_day_get_current();
  ustring user = g_get_real_name();
  ustring logbook;
  notes_store_one_in_file(id, text, project, references, category, date_created,
                          user, date_time_julian_day_get_current(), logbook);
}

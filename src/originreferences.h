...
public:
  OriginReferences(const ustring &project, bool affectfootnotes,
                   bool affectendnotes, bool affectxrefs,
                   OriginReferencesActionType action, bool includebook,
                   bool includechapter, const ustring &separator,
                   bool includeverse, const ustring &suffix, bool gui);
  ~OriginReferences();

protected:
private:
  void get_relevant_markers();
  void handle_notes(ustring &line, int book, int chapter, const ustring &verse,
                    const ustring &note_opener, const ustring &note_closer,
                    const ustring &origin_ref_opener,
                    const ustring &origin_ref_closer,
                    const ustring &text_opener, const ustring &text_closer);
  void remove_reference(ustring &line, const ustring &opener,
                        const ustring &closer);
  void add_reference(ustring &line, const ustring &opener,
                     const ustring &reference);
  void add_text_marker(ustring &line, const ustring &referenceopener,
                       const ustring &referencecloser,
                       const ustring &textopener, const ustring &textcloser);

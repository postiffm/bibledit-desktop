...
RevertDialog::RevertDialog(Reference *reference) {
  ...
  gtk_window_set_position(GTK_WINDOW(revertdialog),
                          GTK_WIN_POS_CENTER_ON_PARENT);
  gtk_window_set_type_hint(GTK_WINDOW(revertdialog),
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolledwindowrevisions),
                                 GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
  gtk_scrolled_window_set_shadow_type(
      GTK_SCROLLED_WINDOW(scrolledwindowrevisions), GTK_SHADOW_IN);
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeviewrevisions),
  ...
  radiobutton_current =
      gtk_radio_button_new_with_mnemonic(NULL, _("View current version"));
  gtk_widget_show(radiobutton_current);
  gtk_box_pack_start(GTK_BOX(vbox2), radiobutton_current, FALSE, FALSE, 0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_current),
                             radiobutton_current_group);
  radiobutton_current_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_current));

  shortcuts.button(radiobutton_current);

  radiobutton_previous =
      gtk_radio_button_new_with_mnemonic(NULL, _("View previous revision"));
  gtk_widget_show(radiobutton_previous);
  gtk_box_pack_start(GTK_BOX(vbox2), radiobutton_previous, FALSE, FALSE, 0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_previous),
                             radiobutton_current_group);
  radiobutton_current_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_previous));

  shortcuts.button(radiobutton_previous);

  radiobutton_changes = gtk_radio_button_new_with_mnemonic(
      NULL, _("View changes if reverting to previous revision"));
  gtk_widget_show(radiobutton_changes);
  gtk_box_pack_start(GTK_BOX(vbox2), radiobutton_changes, FALSE, FALSE, 0);
  gtk_radio_button_set_group(GTK_RADIO_BUTTON(radiobutton_changes),
                             radiobutton_current_group);
  radiobutton_current_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(radiobutton_changes));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(revertdialog), cancelbutton1,
                               GTK_RESPONSE_CANCEL);
  gtk_widget_set_can_default(GTK_WIDGET(cancelbutton1), true);

  okbutton1 = gtk_button_new_from_stock("gtk-ok");
  gtk_widget_show(okbutton1);
  gtk_dialog_add_action_widget(GTK_DIALOG(revertdialog), okbutton1,
                               GTK_RESPONSE_OK);
  ...
  g_signal_connect((gpointer)comboboxbook, "changed",
                   G_CALLBACK(on_comboboxbook_changed), gpointer(this));
  g_signal_connect((gpointer)comboboxchapter, "changed",
                   G_CALLBACK(on_comboboxchapter_changed), gpointer(this));
  ...
  g_signal_connect((gpointer)radiobutton_current, "toggled",
                   G_CALLBACK(on_radiobutton_toggled), gpointer(this));
  g_signal_connect((gpointer)radiobutton_previous, "toggled",
                   G_CALLBACK(on_radiobutton_toggled), gpointer(this));
  g_signal_connect((gpointer)radiobutton_changes, "toggled",
                   G_CALLBACK(on_radiobutton_toggled), gpointer(this));
  ...
  combobox_set_string(comboboxbook,
                      books_id_to_name(language, reference->book_get()));
...
int RevertDialog::run() { return gtk_dialog_run(GTK_DIALOG(revertdialog)); }

void RevertDialog::on_comboboxbook_changed(GtkComboBox *combobox,
                                           gpointer user_data) {
...
void RevertDialog::on_comboboxchapter_changed(GtkComboBox *combobox,
                                              gpointer user_data) {
...
void RevertDialog::on_chapter_changed() {
  ...
  ustring chapterdirectory =
      project_data_directory_chapter(project, book_get(), chapter_get());
  ustring path =
      project_data_filename_chapter(project, book_get(), chapter_get(), false);
...
void RevertDialog::on_treeviewrevisions_row_activated(GtkTreeView *treeview,
                                                      GtkTreePath *path,
...
void RevertDialog::on_revision_activated() {
  ...
  ustring data =
      snapshots_get_chapter(project, book_get(), chapter_get(), second);
...
// to change the data, and if Yes is responded with, proceed with loading the
// change.
{
  ...
  int result = gtkw_dialog_question(revertdialog,
                                    _("Are you sure you wish to revert this "
                                      "chapter to the previous revision?"));
...
void RevertDialog::on_radiobutton_toggled(GtkToggleButton *togglebutton,
                                          gpointer user_data) {

...
ustring Usfm::name() { return styles[last_style_pointer].name; }

double Usfm::fontsize() { return styles[last_style_pointer].fontsize; }

ustring Usfm::italic() { return styles[last_style_pointer].italic; }

ustring Usfm::bold() { return styles[last_style_pointer].bold; }

ustring Usfm::underline() { return styles[last_style_pointer].underline; }

ustring Usfm::smallcaps() { return styles[last_style_pointer].smallcaps; }

bool Usfm::superscript() { return styles[last_style_pointer].superscript; }

ustring Usfm::justification() {
  return styles[last_style_pointer].justification;
}

double Usfm::spacebefore() { return styles[last_style_pointer].spacebefore; }

double Usfm::spaceafter() { return styles[last_style_pointer].spaceafter; }

double Usfm::leftmargin() { return styles[last_style_pointer].leftmargin; }

double Usfm::rightmargin() { return styles[last_style_pointer].rightmargin; }

double Usfm::firstlineindent() {
  return styles[last_style_pointer].firstlineindent;
}

bool Usfm::spancolumns() { return styles[last_style_pointer].spancolumns; }

unsigned int Usfm::color() { return styles[last_style_pointer].color; }

bool Usfm::userbool1() { return styles[last_style_pointer].userbool1; }

bool Usfm::userbool2() { return styles[last_style_pointer].userbool2; }

bool Usfm::userbool3() { return styles[last_style_pointer].userbool3; }

int Usfm::userint1() { return styles[last_style_pointer].userint1; }

int Usfm::get_style_pointer(const ustring &usfm)
// Returns a pointer to the style relevant for "usfm", or -1 is the style is not
// found.
{
  // Optimization to speed up finding the pointer: if found before, reuse the
  // result.
  if (usfm != last_marker) {
...
bool usfm_is_id(const ustring &marker) { return (marker == "id"); }

bool usfm_is_chapter(const ustring &marker) { return (marker == "c"); }

bool usfm_is_verse(const ustring &marker) { return (marker == "v"); }

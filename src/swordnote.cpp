...
// Stores the properties for all the footnote related styles.
{
  ...
  for (unsigned int i = 0; i < usfm.styles.size(); i++) {
    if (usfm.styles[i].type == stFootEndNote) {
      ...
      case fentEndnote: {
        ...
        endnote_numbering_restart = nnrtNever;
        EndnotePositionType endnoteposition =
      ...
      case fentStandardContent: {
        // Standard content.
        standardparagraph_marker_open =
            usfm_get_full_opening_marker(usfm.styles[i].marker);
        standardparagraph_marker_close =
      ...
      case fentParagraph: {
        extraparagraph_marker_open =
            usfm_get_full_opening_marker(usfm.styles[i].marker);
        extraparagraph_marker_close =
            usfm_get_full_closing_marker(usfm.styles[i].marker);
        footnote_markers.insert(usfm.styles[i].marker);
        break;
      }
  ...
  footnotecaller =
      new NoteCaller(footnote_numbering_type, footnote_numbering_user_sequence);
  endnotecaller =
      new NoteCaller(endnote_numbering_type, endnote_numbering_user_sequence);
  xrefcaller =
      new NoteCaller(xref_numbering_type, xref_numbering_user_sequence);
}
...
void SwordNote::transform2(ustring &opener, ustring &closer, NoteCaller *caller,
                           ustring &line)
// Replace all note related content with corresponding osis xml code.
{
  ...
  while (opening_position != string::npos) {
    ...
    ustring note;
    note = line.substr(opening_position + opener.length(),
                       closing_position - opening_position - closer.length());
...
ustring SwordNote::transform_main_parts(NoteCaller *caller,
                                        const ustring &line) {
  // Variables.
  ustring sword_code;
  ...
  // As Sword content, as displayed in BibleTime, does not format notes, just

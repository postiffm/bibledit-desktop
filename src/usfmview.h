...
class USFMView
    : public ChapterView // USFMView has to implement the ChapterView interface
{
  ustring project;
...
public:
  ...
  void create_or_update_formatting_data() {
    gw_warning("Cannot create or update formatting data in usfmview");
  }
  void apply_style(const ustring &marker) {
    gw_warning("Cannot apply styles in usfmview");
  }
  bool can_undo();
  void undo();
  bool can_redo();
...
public:
  ...
  inline EditorTextViewType last_focused_type() {
    return etvtBody;
  } // This is a no-op for usfmview
private:
